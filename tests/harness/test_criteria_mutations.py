...
被测判据（→ TODO 条目 → 审计表 B 编号）：
  M1 事件 1-4 step 模板        → #84 / B2（test_hit_table.ev14_template_problems）
  M2 dump 未用字段值域白名单    → #86 / B3（test_hit_table.dump_sentinel_problems）
  M3 STR 段冷/暖前缀契约        → #87 / B4（test_cir_warm_path.str_contract_equal）
  M4 `--dump-tk-terms` 剔除面白名单 → #88 / B5（test_ccr_types.dump_strip_face_problems）
  M5 零 diff 腿指令边界锁步     → #90 / B1（test_mw_task2.region_equal_mask_calls）
  M6 慢路径块体逐指令模板       → #85 / B6（test_mw_task2.parse_slow_block）
...
def old_ev14_prefix_ok(events) -> bool:
    """#84 旧形态：`stream[:2] == want`（只比 REX+opcode 前 2B）。"""
...
def old_str_size_only_ok(cold_seg: bytes, warm_seg: bytes) -> bool:
    """#87 旧形态：STR 段只登记尺寸、**不入判据**（恒绿——差异被显式豁免）。"""
    return True


def old_strip_compare_ok(out_flag: str, out_plain: str) -> bool:
    """#88 旧形态：黑盒逐行过滤（`[df-tk-terms]` 头 + `^-?\\d+\\t` 数据行）后比对。"""
    import re
...
def old_region_equal_mask_calls(a: bytes, b: bytes):
    """#90 旧形态：遇 0xE8 字节即跳 5B（含**非指令首**的 0xE8 ⇒ 其后 4B 盲窗）。"""
    if len(a) != len(b):
        return False
    i = 0
    while i < len(a):
...
def main() -> int:
    ...
    # ── M1（#84/B2）事件 1-4 step 模板：4 个坏实现 ──────────────────────
    ...
    for label, bad_line in [
        ...
        cases.append((f"M2/#86 dump 值域：{label}",
    ...
    cases.append(("M5/#90 零 diff：改 0xE8 数据字节后的帧常数 imm32（旧盲窗）",
    ...
    ok_pos, why_pos, ncalls = MW.region_equal_mask_calls(t_reg, span,
                                                         bytes(legal), span)
    cases.append(("M5/#90 正控：真 call rel32 位移变化（合法）仍绿",
                  ok_pos, [] if ok_pos else [why_pos],
                  {"expect_strong_clean": True}))
    print(f"       （M5 正控：真 call 站点 @{call_off}，豁免面计数 = {ncalls}"
          f"（>0 = 探针触发非空转））")

    # ── M6（#85/B6）慢路径块体模板：3 个坏实现 + 1 个正控 ───────────────
    ...
    try:
        ...
        cases.append(("M6/#85 正控：真实块体可解析", True, [f"正控自身失败: {e}"]))
    for label, mut in [
        ...
        try:
            ...
            probs = [str(e)]
        cases.append((f"M6/#85 块体模板：{label}",
