# 暖态探针语料（#60 批 T3：判据网「暖态腿」的载体）

**来源**：#60 批 T1 的定路径探针矩阵（`/tmp/fct4/task1-report.md` §3.3，2026-09-15 实跑实拷入仓）。
...
⇒ 键唯一 ⇒ **结构性无暖态** ⇒ 腿恒绿而空洞（T1 方法学发现；突变 M3 实证：缺陷在场时
唯一路径版腿 `FAIL=0` + 「暖态生效档数 = 0」）。故本语料**必须按入仓路径就地编译**，
...
| 6 | `plain.cr` | **正控**（普通程序） | rc=0 → rc=0 ∧ **`plain.cr::main.cir` 条目存在 ∧ 二跑真命中**（防腿恒绿空洞） | T2 正控 |
| 7 | `alloc_witness.cr` | **见证面**（数组字面量 + 取址 ⇒ ir_gen 期分配复合行） | rc=0 → rc=0 ∧ **自档 `main` 无条目**（#60 T2 见证规则） | T2 §3.2 |
...
（腿绿但根本没走到暖态，例如路径不固定/缓存被关），二者都不可接受。

## 与既有 runner 的关系（#60 T3 落位）

- `tools/baseline/warm_leg.sh`：腿本体（两层：广度层 = 父 runner 传入清单；牙齿层 = 本语料）。
- `tools/baseline/warm_run.sh`：牙齿层专用入口（= `--with-corpus`）。
- `parity_run.sh` / `probes_run.sh`：各加**广度层**调用（同一清单单一真源 `<outdir>/corpus.tsv`）；
