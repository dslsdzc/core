...
# ══════════════════════════════════════════════════════════════════════
# 判据侧 x86-64 指令长度解码（B1/#2026-09-16-28 + B6/#2026-09-16-23 修复，2026-09-16 判据网加固批）
#
# 缘起（判据强度审计 §0/表 B，TODO #2026-09-16-28/#2026-09-16-23）：
#   - 零 diff 腿原实现「遇 0xE8 字节即跳 5 字节」——0xE8 出现在 disp8/imm32
#     等**非指令首**位置（值 −24 = 0xE8 极常见）时，其后 4 字节**永不比较**
#     = 盲窗（改帧常数低 4B 即可骗过，见 #2026-09-16-28）。
#   - 慢路径块原判据只有 2B 前缀锚（4D 19）+ 目标序（#2026-09-16-23）。
...
CASES = [
    ...
    # B6/#2026-09-16-23 修复附例（判据网加固批 2026-09-16）：**sub 站点**慢路径块 = 块体
...
def parse_slow_block(text, t, is_sub):
    """慢路径块体逐指令模板断言（**B6/#2026-09-16-23 修复**，2026-09-16 判据网加固批）。
...
def region_equal_mask_calls(text_a, span_a, text_b, span_b):
    ...
    **B1/#2026-09-16-28 修复（2026-09-16 判据网加固批）**：旧实现「遇 0xE8 字节即跳 5」
    ...
    for name, want_exit, want_out, desc in CASES:
        ...
        for opt in (0, 1, 2):
            ...
            if exp == 0:
                ...
                # B6/#2026-09-16-23 修复：块体判据从「2B 前缀锚 + 目标序」升级为**逐指令
