...
def test_p4t4_cir_snapshot_layout_unbumped():
    ...
    try:
        ...
        for f in entries:
            ...
            assert ver == 20, f"{f}: CIR_CACHE_VER {ver} != 20 (2026-09-17 缓存膨胀批「段粒度按函数收窄 + 相对 id + 尾部 trailer」换代后值；前代 = 19 批 5（opt-dex）、18 = TODO #2026-09-16-16 批 2)"
    ...
    **#2026-09-15-5 T2 重钉（死亡证据 = 本批能力变更，非放宽判据）**：原判据对象 = **复合行**
    ...
    try:
        ...
        for f in sorted(os.listdir(cache_dir)):
            ver, nodes, ok = cir_entry_nodes(os.path.join(cache_dir, f))
            assert ver == 20 and ok, f"{f}: version/stride drifted（期望 20 = 2026-09-17 缓存膨胀批换代后值；节点 stride 仍 64B / 头偏移不变（裁-4 + 尾部 trailer）；前代 19 = 批 5（opt-dex）、18 = TODO #2026-09-16-16 批 2）"
