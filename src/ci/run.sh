...
case "$CI_JOB_NAME" in
  ...
  selfhost-tests)
    ...
    # 当前二进制 × 当前源，**76 档**语料（runner `tools/baseline/parity_run.sh`，逐档 clean-cache；
    # 档数 = 实读枚举 36+2+15+19+4（suite/compiler/后端内核/stdlib/examples）= 与 runner 的 `CORPUS_TOTAL=76` 硬断言一致；
    # 2026-09-17 批 6 T6 实测重数 t1=35；**2026-09-18 批8 2(a)（#132）加 `tests/suite/cli_view_test.cr` ⇒ t1=36；
    # 本处由 2026-09-24 补正（#170 只改了 runner 与 REBUILD.md，漏了本处 ⇒ 该句曾短暂为假）**）
    ...
    python3 tests/selfhost/test_cir_str_domain_warm.py # 串域跨进程 id 修复（`CIR_CACHE_VER 20→21`；TODO #2026-09-18-9）：生成期 intern 日志 + 装载期按序重放——A 6 行复现（冷/暖 输出+ELF+`.ccr` 三面同；锚定 `fields=[x,y] len=3`）· B firstintern 判别（解析期已 intern 的 `"x"` 与 gen 期首次 intern 的 `"x,y"` 暖态都必须对）· C var 名路径（`.ccr` 逐字节含合成名 `_cat0/_cat1`）· D 混合命中态（删首/中/末/多条 ⇒ 输出+ELF 同；`.ccr` NOD 项索引差异属**另一类**只上报）· E 突变自证（`--inject-cir-skip-journal` ⇒ 暖态取到 `arena_reset`；含非真空控制 = 无串槽语料上突变不动输出）· F 条目日志段结构（ver=21 · count≥1）= **17 例**
    bash tools/cir-str-domain/run_scope.sh --assert # 串域证据包判据模式（7 档冷/暖矩阵；`interp` 档 = **基线既有红**、非本缺陷面，见 tools/cir-str-domain/known-baseline-red.txt 的证据与触发器）
    ...
    # 修复前根因：lexer 插值分支**双推进**（skip 循环已到 `}` 之后，循环尾通用 `_pos += 1` 又吃一字节）
    # ⇒ ① `}` 后首字符静默丢；② 收尾引号被吞 ⇒ 串吞到行尾 ⇒ 同行 `;`/**`}`** 进串 ⇒ 顶层 `fn` **P21 级联**。
    # 展开（裁定 (b)）：lexer 拆 part token（段 T_STRING / 洞 T_INTERP，**洞内 `{}` 配平**）· parser 串链 ·
    # checker 按洞类型**就地改写**：string 恒等 · int→`int_str` · bool→`bool_str`（**不经 int**）·
    # char/dex/其它 ⇒ **P028 硬错**；畸形插值 ⇒ **P027 硬错**。
    # 判据 = A 解析完整性（同行含 `}`/后续语句 ⇒ 无 P21 + rc）· B 值 10 形态（含洞内配平、实参位、`\${` 字面）·
    # C 错误面 4 例（P027×2 + P028×2：check rc=1 + 码 + build rc=1 + **零产物**）· D bootstrap 面事实
