...
case "$CI_JOB_NAME" in
  ...
  selfhost-tests)
    ...
    python3 tests/selfhost/test_type_engine.py   # 类型项引擎自测通道（selftest-types **346 例**——P4 Task 3 后计数（T2 的 338 + 8：IFACE 段五小节往返/签名项槽/形状名注册/条目扩列/判定面零变化/不可建签名拒绝 7 例 + x2.shape_builtin_names 1 例）；P0/P1/P2a/P2b-T1..T6/P3-T0..T5/P3b-T0..T2/T6/P4-T2/T3 一路漏挂）
    ...
    python3 tests/selfhost/test_ccr_types.py       # R2 P4 Task 1/2/3：段机制（版本 8 + loader 三闸/必备集 + 旧 v7 拒收）8 例 + TYPE 内容面（行表/项 DAG 序列化 + 确定性装填 + corearch 读回对拍 + loader 负分支）10 例 + IFACE 内容面（五小节/条目扩列 16/形状命名化/签名项化/impl 边与方法表/corearch 读回/跨段引用域/确定性）10 例 = 28 例
