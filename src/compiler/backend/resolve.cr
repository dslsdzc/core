...
fn resolve_labels() {
    ...
    loop {
        ...
        loop {
            ...
            inst_idx := ist + ii;
            inst := g_ir_instrs[inst_idx];
            if inst.opcode == IR_BRANCH {
                true_off := g_label_poses[inst.src2];
                false_off := g_label_poses[inst.src3];
                inst.src2 = true_off;
                inst.src3 = false_off;
                inst.type_kind = IR_RESOLVED;
            }
            if inst.opcode == IR_JUMP {
                tgt_off := g_label_poses[inst.src1];
                inst.src1 = tgt_off;
                inst.type_kind = IR_RESOLVED;
            }
            ii = ii + 1;
