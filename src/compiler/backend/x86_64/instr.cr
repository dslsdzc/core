...
fn x86_emit_instr(instr_idx: int, buf: string, pos: int) -> int {
    ...
    if op == IR_LOAD_INDEX_VAR && d >= 0 {
        ...
        e2_w8(buf, pos+cp, 75); e2_w8(buf, pos+cp+1, 139); e2_w8(buf, pos+cp+2, 4); e2_w8(buf, pos+cp+3, 216); cp = cp + 4;
        cp = cp + e2_st(buf, pos+cp, 10, do2);
        return cp;
    }
    ...
    if op == IR_STORE_INDEX_VAR && d >= 0 {
        ...
        e2_w8(buf, pos+cp, 75); e2_w8(buf, pos+cp+1, 137); e2_w8(buf, pos+cp+2, 4); e2_w8(buf, pos+cp+3, 216); cp = cp + 4;
        return cp;
    }
