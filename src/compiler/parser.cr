...
fn parse_new_var_decl() -> int {
    ...
    if check(T_COMMA) {
        loop {
            ...
            advance_tok(); // ,
            nt2 := advance_tok();
            w64(names, nc * 8, tok_lx(nt2) as int);   // 裸槽：显式（批 8 S2）
            nc = nc + 1;
        }
    }
    ...
    loop {
        if i >= nc { break; }
        ni := str_intern(r64(names, i * 8) as string);
        nv := r64(values, i * 8);
...
fn save_func_generics(fi: int, names: string, count: int) {
    ...
    loop {
        if gi >= count { break; }
        ni := str_intern(r64(names, gi * 8) as string);
        ...
        if si >= 0 {
            if sg_count > 0 {
                ...
                loop {
                    ...
                    w64(g_structs, si * ESZ_STRUCTINFO + OFF_SI_GENERIC_NAMES + sgi * 8, str_intern(r64(sg_names, sgi * 8) as string));
                    sgi = sgi + 1;
                }
        ...
        if ei >= 0 {
            if eg_count > 0 {
                ...
                loop {
                    ...
                    w64(g_enums, ei * ESZ_ENUMINFO + OFF_EI_GENERIC_NAMES + egi * 8, str_intern(r64(eg_names, egi * 8) as string));
                    egi = egi + 1;
                }
