...
> 定位：受众 = users/contributors；状态 = active；真源 = corelsp 行为见 [design/corelsp.md](../design/corelsp.md)（实现与文档冲突时以源码为准）；仓库内编辑器素材见 [editor/](../../editor/)（扩展源码与说明）。
> 本文是纯用户向接入指南——各编辑器怎么接 corelsp；corelsp 的架构（检查管线/诊断通道/能力契约）见 [design/corelsp.md](../design/corelsp.md)。
