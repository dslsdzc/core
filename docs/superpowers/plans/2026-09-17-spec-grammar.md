...
**⚠ 未竟项（登记，不静默）**：**腿③ 广度层（`parity_run.sh` 附带的 74 档暖态二跑）未跑完**——
卡在 `corec ccr src/compiler/main.cr` 的**暖态第二跑**：`State: D (disk sleep)` · 该进程
`rchar = 760,556,173`（**760MB 读**）· 工作区 `.core/cache/cir/` 实测 **1.4GB / 1011 条目** ·
单次调用 elapsed **9:52** 且 >8 分钟零进展（对照 `REBUILD.md:78-79` 记载「parity 广度层 **+125s**」**严重不符**）。
**性质判定**：本批**零源码改动**（二进制 = develop 自建）⇒ 该现象**与本批无关**，属**既有性能/缓存病理**
（候选 TODO；**本批不做根因定位**）。**处理**：杀进程 + 清缓存（1.4GB → 0）；该层留 T1 前补跑或按
`REBUILD.md` 的「手工判据」口径另行安排（**不得**把它写成「已跑绿」）。
...
| 10 | 腿③ 广度层（74 档暖态） | ⚠ **未竟**（§10.4 登记；既有病理，与本批无关） |
