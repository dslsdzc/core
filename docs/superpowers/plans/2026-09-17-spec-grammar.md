...
> **状态**：实施计划（**规划轮写进文档**）。开工前置 = §3.1 六门（**已由维护者按默认取裁**）+ §3.2 本批新门（**逐条给推荐，未取裁前不实施**）。
> **定位（受众/状态/出处）**：受众 = 批次实施代理 + 维护者；状态 = **计划（待实施）**；出处 = 本文（切片边界/任务卡/判据）+ `docs/maintainer/design/spec-design.md`（规约设计权威）+ `docs/superpowers/plans/2026-09-16-verification-slice.md`（切片草案，本批 = 其 C1 的正式接入）。
> **诚实边界（**须先读**）**：本计划**只读侦查、零源码改动、零构建**（未跑 `build/corec` / 未跑任何套件）。**行号 = develop `dbadb8d6` 工作树实读值**（下一任务开工时**须重取**——本仓「纸面成果只对当轮 revision 有效」约定）。凡**推断/预测**一律标「**预测**」；凡实读一律给 `file:line`。代价估计为**相对量级**（小/中/大），非工时。
...
**同扫描的否证面（须一并记住）**：`#` 在**非 `.cr` 文件格式**里是**行注释首**——`src/arch/hit/hit.cr:264/:382`（HIT 表）、`src/compiler/module.cr:245`（`.so` 索引）、`src/kernel/kernel_main.cr:105` + `src/kernel/test_term_io.cr:82`（kernel 行格式）、`src/arch/hit/lower_to_core.cr:340`（注释叙述）。**这四处都是「读文件字节后按 35 判列」，不经 Core 词法器** ⇒ 与 `#` token **零交互**（须在 T1 判据里锁定：这四面的行为**不变**）。
...
**影响面实核**：`build_selfhost_native.py` 的 concat 清单（`:35 common_files` / `:103 backend_support_files` / `:311 corec_files` / `:405 corelsp_files`）**只含 `src/**`，不含 `tests/**`** ⇒ 只要 `#` 标注**不进 `src/**`**，bootstrap 在本批**不会被喂到** `#`。既有对照先例 = `tests/selfhost/test_lexer_parity.py`（两前端对 `_`/宽度后缀**同判**是本仓既有约定）。
...
| `tests/harness/ci_hook_allowlist.txt` | `:14-15` 头注「scope 66（selfhost 56 …）/ 挂点 45」 | 实测 **scope 71（59+7+5）/ 挂点 50**（该文件自注「计数随分支漂移，以当场重算为准」⇒ 属**已知约定**，非缺陷；本批顺带刷新） |
...
### 3.1 六门（**维护者已按默认取裁——本计划写进文档照办，不再询问**）
...
| **裁-S7** | **bootstrap（Python）面同批做吗？** | 两前端接受集 / 自举链 | **同批做「最小一致面」**：`bootstrap` lexer 认 `#`+IDENT 产 token、parser 消费标注链并**忽略**（不做检查/VC）——理由：① 两前端**同判**是本仓既有约定（`test_lexer_parity.py`）；② **成本小**（lexer 单字符表 + parser 一处）；③ 否则分歧升级为「拒 vs 收」。**明确不做**：bootstrap 侧不产诊断、不产 VC（它**不是**语言参照实现） | 退路 = **显式登记分歧**（bootstrap 对 `#` 响亮报错 = fail-loud，不是静默）⇒ 可接受但须入 TODO |
...
| **裁-S11** | **新语料入仓位置** | 两条判据计数 / CI 挂点 | **新目录 `tests/spec/`**（`#` 语料）+ **新套件 `tests/selfhost/test_spec_grammar.py`**；**不进** `tests/probes/`（否则 `PROBE_TOTAL 29→30`）**也不进** `tests/suite/`（否则 `CORPUS_TOTAL 74→75`）——理由：两条判据是**既有行为回归网**，其**计数不变**本身就是「旧面零扰动」的最强证据 | 按推荐（若坚持进 `tests/suite/` ⇒ 须同批改 `parity_run.sh:31` 计数 + 分层注释 + `REBUILD.md`，并把 74→75 记入清单） |
...
| **裁-S7** | **同批做「最小一致」（bounded）** | 理由：「一方拒收合法语法」是**最坏那类**两编译器分歧（不是意见不同，是一边直接打断）。**边界固定**：bootstrap 侧**只需「接受并跳过」（零语义）**——能词法化、不报错、**不影响产物**；**不要求**实现 VC/三态；完整 bootstrap VC 面**登记后续**。**判据**：① 一份最小带 `#` 语料在**两侧都 rc=0 且产物逐字节同**；② 既有语料两侧行为不变 |
| **裁-S12** | **取强式** | 同程序 ± 注解 ⇒ ELF + `.ccr` **逐字节同**；它把「`#` 没碰任何下游」从**声明**变成**判据** |
| 裁-S1 / S2 / S3 / S5 / S6 / S8 / S9 / S10 / S11 | **按推荐** | 逐条见上表「推荐」列。附带：S9 的 `result != None` 示例**不在 C1 子集内** ⇒ T6 标注/改写；S11 的「两条判据计数不变」= 旧面零扰动的最强证据（维护者确认） |
...
### **T0 — 前置（冻结基线 + 起点判据 + 六门写进文档）**
- **前置**：无（本卡即前置）。
- **动作**：① `jj workspace add` 独立工作区（本批 = `core-veri-ws` @ `feature/spec-grammar`，**已建**）；② **冻结基线重建**（`bash tools/baseline/rebuild.sh /tmp/spec-baseline`，配方 = `REBUILD.md`；**须重跑自证**：三 sha = 白名单 + 确定性 ×2 + 冒烟 42）；③ **起点判据表**（腿①74 冷态 rc+日志落 `/tmp/spec-t0/parity_pre`；腿②29 + warm 7；`canary_check.sh` 全绿）；④ **六门写进文档**（§3.1）+ **十二门取裁写进文档**（§3.2.1，**已完成**）；⑤ **锚点重取**（§1 全部 `file:line` 在开工 revision 上重核一遍——**行号漂移即更新本节**）；⑥ **RED 复现**：**已实测（2026-09-17），结论见 §10.2——实测结果与本节原「预测」**不符且更严重**（预测 = 报 P0xx 语法错；实测 = **无解析错**，标注表达式被误当函数体 ⇒ `error[TF01]`（**build 面豁免码**）⇒ `build` **rc=0 + 产物**，运行返回 0（应返回 1）= **静默误编译**）。预测文本已由 §10.2 取代，**保留此注否证痕**。
- **判据**：基线三 sha 命中 + 起点两条判据落盘 + 六门/新门入纸 + RED 实测记录。
...
- **判据**：正例 ≥6（单条/多条/多函数/带泛型/`=` 形 body/`{}` 形 body）；负例 ≥8；**零足迹**：T1 的四条全绿。
  **T2 三条硬验收（维护者 2026-09-17 批准时指定，不可省）**：
  1. **两侧都接受 rc=0**（bootstrap 侧 ✅ 已绿）+ **corec 侧 ±注解 ELF/`.ccr` 逐字节同**（裁-S12 强式）——本批判据标准（见 §11 的约定修正）；
...
### 5.3 探针入仓形态 + 挂点三件套
- **语料**：`tests/spec/*.cr`（**不进**两条判据；`tests/probes/README.md` 的「非编译单元」约定**不适用于本目录**——本目录**是**编译单元，由新套件驱动）。
...
- **`.ccr` 五条出处**（**只此一处**）= `tools/baseline/canary_values.tsv` 末 5 行数据行（ELF 1 + `.ccr` 4）；闸门 = `tools/baseline/canary_check.sh`（fail-closed F1-F4；**只锁冷态**，不得「补全」成冷暖双锁）；**手工复跑须 `HOME` 归一化**（照 `REBUILD.md:133-148`；2026-09-17 起归一化**仍保留作双保险**）。
- **腿① = 74 档**（`CORPUS_TOTAL` 硬断言；实测分层 34/2/15/19/4）· **腿② = 29 档**（`PROBE_TOTAL` 硬断言）+ warm 7 档 · **两条判据计数本批不得改**（改了 = 销毁「旧面零扰动」的机械证据）。
- **五 CI job**：`check` / `bootstrap-tests` / `selfhost-tests` / `suite` / `full-bootstrap`（`src/ci/run.sh:61/67/98/199/204`）。
...
2. **两条判据出现非章程 diff** ⇒ 判定面行为变更（新检查若改既有语料 rc ⇒ 先 report-only + 裁决）。
...
6. **纯度时序**：若实施中需要读生成期 `fi_ispure` 而无法在不改发射面的前提下收尾 ⇒ 停（裁-V5 已把该风险出列，**不得**顺手「解决」）。
...
- **诚实边界**：**本计划零构建、零源码改动**；行号 = `dbadb8d6` 实读；**§1.2 的全仓扫描是本计划实跑的唯一实验**（只读、可复现，扫描范围已写入判据设计）；**§4 T0 的 RED 复现为「预测」**，标注待实测。
- **与既有裁决的一致性**：ADR-0001（规约并入 `.cr`）· spec-design §六（`#` 与 `@` 分工）/§十二（unproven 不拦）· 裁-V1..V6 · `TODO #2026-09-11-9` 判据标准（结构性断言 + 语义零变化 + 自举稳定）· 三态纪律 · P4 T5「零足迹」先例（裁-V4）· P6 事故护栏（含「先裁」入卡）。
...
> **约定**：本节全部数字 = **当场实测**（非引用文档）；命令与产物路径逐条给出，可复跑。
...
**约定声明**：本批 leg-① 的 pre-side = **上表自建二进制**（本批起点树、确定性 ×2 已证）。
...
（`REBUILD.md` 应补「广度层不覆盖大文件」约定；**不得**把它写成「已跑绿」）。**腿③ 牙齿层（7 档）与腿② 广度层（29 档）不受影响，均实测绿。**
...
| 3 | 六门 + 十二门写进文档 | ✅ §3.1 / §3.2.1（提交 `05667d03`） |
...
| ② 全仓裸 `#` 扫描 | ✅ **182 档 · 0 命中**（构造性零足迹；与 T0 同一标准同脚本） |
...
- **约定修正（结构性，且不可回避；维护者 2026-09-17 已准并定为**本批判据**）**：「两侧**产物逐字节同**」**不可构造**——两前端的**后端不同**（bootstrap = Python `X86_64StackAsmGen` + `as`/`ld`；corec = 自研 ELF 发射器），无共同产物面 ⇒ 本条按**可构造的最强形**实现：**(a) 两侧都接受（rc=0）** + **(b) 各自零足迹**（bootstrap：asm ±注解逐字节同，**已绿**；corec：±注解 ELF/`.ccr` 逐字节同 = 裁-S12 强式，**T2 后验**）。
...
**回归闸门**：canary **5/5 PASS** · 腿② 29 档 **零差异** · J 判据 **4/4** · 腿① 74 档 rc 分布不变（35×0/39×1）且**两档日志差异经机械归因 = 纯行号平移**（`_import.cr` 4 行 · `ccr_io.cr` 156 行，**全为 `--> 行:列` 与源码回显形态；剥行号后逐字节同**——根因 = 本批给编译器源加了 193 行，而对拍语料含编译器自身源）⇒ **判定面行为零变更**，属预期内差异（入清单）。
...
两条判据在新锚点上**零差异**。（同教训的另一处实例：缓存批 rebase 后 `jj diff` 显示其树少了批 5 report 的 24 行 —— **develop 一前进，比较基就失效**。）
...
| 3 | **源序确定**（两次 dump 逐字节同） | ✅ |
| 4 | **开关不改产物**（有/无 `--dump-vcs` ⇒ ELF 与 `.ccr` sha 同） | ✅（黄态语料；红态无产物，判据标准另定） |
| 5 | **冷/暖 dump 逐字节同** | ✅ |
...
**四点固定的落实**：① stdout、零新产物、开关不改 rc/产物 ✅ · ② 三态分流（红走诊断 / 绿黄进 dump）✅ ·
③ **红也列**（打印点在闸门之前 ⇒ 红态照样列出；「没列出来」与「列出来是红的」可区分）✅ ·
④ **只读侧表**（`vcs_dump` 直接读 `g_spec_*`，不另起状态 ⇒ 无第二处定义）✅。
...
文档面（`syntax.md` / `spec-design.md` 示例 / `REBUILD.md` 计数勘误）· 全量回归与清单。
...
| # | 路径 | 内容 |
|---|---|---|
| 1 | `tests/spec/`（**33 档 .cr + README.md**） | 四组语料：`t2_*` 语法面 15 · `t3_*` 检查面 11 · `t4_*` dump 通道 3 · `delta_*` 零足迹对拍 4（两对，**成对使用不可拆**）。**独立目录**：不进腿①（parity 75 档）与腿②（probes 29 档）——两条判据计数不变 = 旧面零扰动的证据 |
...
`REBUILD.md` 计数勘误「72/73 → 75」）· 统一清单 · TODO 收尾。

---

## 16. T6 收官：五 CI job 全量回归 + 清单 + 文档勘误（2026-09-17）
...
| T1 | 词法 `T_HASH=101` + bootstrap 最小一致 | 常量静态断言 · 全仓裸 `#` 扫描 182 档 0 命中 · 非 `.cr` 面 24/24+8/8 · 两条判据零差异 |
...
| T6 | 五 job + 勘误 + 清单 | **五 job 全 rc=0** · 文档勘误先实测 |

**本批登记条目**：`#2026-09-17-8`（快照二次膨胀）· `#9`（TF01 豁免对）· `#10`（bootstrap 关键字残留）· `#11`（顶层吞 token）· `#12`（`cmd_cir` 死码）。
**TODO 收尾**：`#2026-09-16-5`（验证切片轮立项）状态由「立项/草案」改为「**已实施（批 6）**」（含落地面 + 判据 + 下一刀指向）。

**约定修正共两处**（均经维护者准）：① 「两侧产物逐字节同」不可构造 ⇒ 改「两侧都接受 + **各自**零足迹」；
② `.ccr` 逐字节同不可达（注解词素必然进 STR）⇒ 改「ELF 同 + 除 STR 外逐段同 + **STR Δ 恰为公式值**（多一字节即红）」。

### 16.4 ⚠ 历史收敛登记（**T3/T4 的 src 变更落点**；审阅者必读）

T5/T6 两次 rebase 的 `TODO.md` 冲突消解过程中，**T3/T4 的 `src` 改动被从原提交剥出**、落入工作副本
（链上逐提交查 `vcs_dump` 命中 = **0**）；已在 **T6 提交 `773b1042`** 收敛，**最终树 = 本轮全部判据实测过的状态**
（提交后复验：build ✓ · 冒烟 42 · 套件 48/48 · canary 5/5 · J 5/5）；
`jj diff --from develop --to feature/spec-grammar` = **55 文件 +1732/−16**，**含全部 src**。

- **性质**：**位置问题、非内容丢失**（合并为 squash ⇒ 最终树不变）。
- **机制（可复现）**：`jj edit <中间提交>` + 重写工作副本文件（用于消解 7 个 TODO.md 冲突）会把**该提交的后续改动**
  一并卷进工作副本；反复 `edit`/写/`rebase` 后，某些提交的实际内容与提交信息不再一一对应。
- **教训（方法论）**：**大文件冲突消解不要用「逐提交 edit + 重写」**——优先「**一次收敛到一个提交**」（本轮 T5/T6 已按此做）。

### 16.4.1 ⚠⚠ **通用陷阱（本批最有价值的**过程**产物）：判据跑在**工作副本**上，断言必须落在**提交**上**

**第二个实例（2026-09-17，rebase #101 后）**：按「一次收敛」把合成的 `TODO.md` 写进 `T6 收官` 提交后，**本地 J 判据 5/5**
——但**判据读的是工作副本**；随后的一次 `jj edit`/bookmark 操作把合成内容从**尖端提交**里带走（尖端 `TODO.md` 只剩 develop 的 10 条）。
**抓到它的是 CI**：`bootstrap-tests` → **J2 悬空 = 10**（`TODO.md` 与 `REBUILD.md`/计划多处引用 `#2026-09-17-8..12` 而无对应标题）——**因为 CI 读的是推送出去的提交**。

**⇒ 复核点修正（覆盖上一条的「逐提交」说法）**：

| | 正确做法 |
|---|---|
| 复核**对象** | **最终提交**：`jj file show -r @ <file>` / `-r <bookmark>`（**不是** `cat <file>`） |
| 复核**范围** | **最终提交**（不是逐中间提交——本次丢的正是尖端，逐提交查法**没抓到**） |
| 最终裁定 | **推送后以 CI 判据为准**（CI 读提交，是唯一能证明「内容在提交里」的机械判据） |

**一句话**：**「本地判据绿」≠「内容在提交里」**——本地判据证明的是**工作副本**的性质，而**提交**才是被审阅与被合并的对象。

**同族（三条一体）**：① §13「rebase 之后必须重锚 pre 侧」（比较基位移）；② §16.4「一次收敛 + 复核」（提交内容位移）；
③ **16.4.1「判据对象 = 提交而非工作副本」**（判据对象位移）。三者合起来 = 本批的**过程方法论文集**：
**凡「读的东西」与「被裁定的东西」不是同一个对象，结论就不能照用**。
- **同族**：§13 的「rebase 之后必须重锚 pre 侧」= 同一类「历史/比较基位移」问题的另一半。

---

## 17. 批终态与方法论产物（**本批最值钱的两条，一个抓实现、一个抓流程**）

### 17.1 实现面：**判据抓到实现自身**（Δ 判据的当场兑现）
T3 首轮 Δ 判据红（d0 期望 0 / 实测 10）⇒ 解码 `.ccr` 的 STR 段 ⇒ 根因 = 本批自己写的
`checker.cr::spec_check_func` 里 `str_intern("result")` **无条件调用**（未注解的函数也驻留该串）⇒ 移进「确需绑定」分支后 Δ 归位。
**意义**：若无维护者加的「**零 Δ 用例必须绿**」这一条，一条**纯编译期的实现细节**会永久污染 `.ccr` 产物而无人察觉
——**判据不是验收仪式，是发现器**。

### 17.2 过程面：**判据对象位移**（§16.4.1；比 §16.4 原命题更根本）
**「判据跑在**工作副本**上，断言必须落在**提交**上。」** 两个实例：① 逐提交 `jj edit`+重写 ⇒ T3/T4 内容被从原提交剥出（§16.4）；
② 一次收敛写盘 + 本地判据 5/5 ⇒ 随后的 `jj edit`/bookmark 把合成内容从**尖端**带走，**本地判据仍绿**，**CI（读提交）当场红**（J2 悬空 10）。
**⇒ 复核点 = 最终提交（`jj file show -r @ <file>`，不是 `cat <file>`）+ 推送后以 CI 为准。**

**与 §13 合起来 = 三条一体的过程纪律**：① rebase 后**重锚 pre 侧**（比较基位移）· ② 冲突消解**一次收敛 + 复核**（提交内容位移）·
③ **判据对象 = 提交而非工作副本**（判据对象位移）。**共同判据**：**「读的东西」与「被裁定的东西」必须是同一个对象；不是，结论就不能照用。**

### 17.3 判据盘点（本批入仓面）
`tests/spec/`（33 档）+ `tests/selfhost/test_spec_grammar.py`（48 项，挂 `selfhost-tests`）——四组：
**A 语法 16 · B 检查 11 · C dump 15 · D 零足迹 6**；其中 **Δ 公式 + 零 Δ 用例**（D 组）是 17.1 那条教训的**载体**，
必须留在仓里（已在 `tests/spec/README.md` 与套件头注写明机制与「多一字节即红」）。
