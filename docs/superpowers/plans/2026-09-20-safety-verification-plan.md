...
| **C-2** | 「逃逸三点检查」（存储/传参逃逸） | `pointer-model.md:73-79`（列三点） | **两点零诊断**：`rc_store_escape` 丢返回值（`region_check.cr:102`，注释自承 simplified）；`rc_pts_has_escaped` 唯一调用者即该处 ⇒ 整条死 | **两种裁法**：(a) 是缺陷 → 修 + 两向钉子；(b) 是有意简化 → 改 `pointer-model.md:77` 表述 + 豁免登记。**判别依据**（本仓既有口径）= **有没有既有契约**：`pointer-model.md` 作为 active 设计文档把三点并列，**形式上构成契约** ⇒ 倾向 (a)，但**须维护者裁** |
...
| U-3 | `2026-09-17-concurrency-model-design.md` 除 §0/§1 外的 680 行 | 只读头部 60 行 |
