...
- **Task 1 越界配套注记（2026-09-09 评审裁决，用户批准推迟）**：elf.cr hit_rel 回填守卫重构（语义等价规避）随 Task 1 同提交——激活面 = 既有编译器巨型函数尾代码生成缺陷（v6 时代潜伏，v7 loader 帧轮廓改变陈旧栈值后激活；TODO.md 预存 Bug #4 已注册跟踪含 repro/gdb 证据/子系统假设）。Task 1 关闭条件 = 跟踪项注册（已满足）。
