...
> 数值出处警示（2026-09-06 M2 收尾修正）：下方示例仅为计划期快照——**字节值与
> 角色以 `src/arch/hit/core-x86.toml` 为唯一依据**（表驱动编码器直读该文件）。
> 快照与出处已同步：M1 寄存器惯例 r10（累加/结果，rm 字段）+ r11（reg 字段）为
...
4. **M2 挂账（评审 Minor）**：hit_w32 负值标记值写入实现分裂（store dst/load s2 未用标记值 −1 读回 255 vs 0xFFFFFFFF）、预检对称性断言、add 反减 dest-fresh 前提固化、'events emitted' 计数改名、REX 注释 W+R+B
