...
**不得 unset/置空 `HOME`**——`module.cr:526` 有硬编码 `/home/DslsDZC` 兜底（已独立登记 TODO）。
...
> **对「反例测试」口径的一处更正（须与维护者取裁的表述一致）**：裁 (A) 条件 4 期望「在受控
> `HOME` 下造 `io/index` ⇒ 仍 5/5」。**实测该期望是反的**：受控 `HOME` 就是编译器的**有效**
> `HOME`，在其中放索引 ⇒ 编译器真的会读 ⇒ 产物真的变（V2a/V2b）⇒ **闸门应当红**（那是输入
> 被改，不是环境噪声）。「解耦」的正确读法 = **与开发者的/任何外部的 `$HOME` 解耦**（V1 已证）；
> 闸门自身的受控 `HOME` 每次采集都 `rm -rf` 重建为空目录 ⇒ **外部无法在运行中污染它**。
> 若将来需要「索引在场」的覆盖面，应另立语料/口径，而不是放宽本闸门。
