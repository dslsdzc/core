...
| Task 6 impl 契约 | ✅ 已交付（P3b） | 见 §11 | 用例 319→**333**；签名四维（参数/返回/应用实参/接收者模式）+ 形状项逐成员判定 + mangling 退役（**零 str_intern**）+ 泛型体内方法调用**崩溃修复**（旧 rc=139 → 正确值）；收紧 4 条 / 放宽 0；ELF/`.ccr` 逐字节同 | `.superpowers/sdd/p3b-task6-report.md` |
