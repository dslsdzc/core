...
**判定面定义（本批语义）**：**error = 阻断**（`rc=1` + 零产物）；本仓**现行无 warning 通道**（全仓无 `warning[` 打印面；`TM04`（`EC_TM_REDUNDANT`）在 `main.cr:159` 注中记为「软面登记（同 Rust unreachable-pattern 的警告约定）」但**打印仍是 `error[TM04]`**）⇒ **本批要么把软面码降级为真 warning，要么登记为豁免**（见裁-FC-2）。

---

## 表 1：全诊断面清点（**核心产物**；`check_all` 面 32 个可达码逐码分诊）

> 约定：**① 必须阻断** = 语义上「判定继续 ⇒ 产出静默错产物 / 判定不可信」（照 P6 裁-P6-2(c) 的成文化判据）；**② 必须豁免** = 有**已实测语料命中**且该命中属已知假阳性/过渡期，或**已有政策裁定**使其非 error；**③ 本批不动** = 在闸门作用域之外（另见表 0/表 2）。**「命中」列 = P6 T0 报告 §3.1 的 72 档实跑记录值（`/tmp/p6t0/corpus_rc.txt`），非本实例实测；套件面命中一律标「须 T1 实测」。**
...
| 13 | TM04 | `EC_TM_REDUNDANT`（:464） | `checker.cr:3100` | 冗余臂 | 0 | **②豁免（政策既定）** | `main.cr:159` 原文：「TM04（冗余臂）**不入**本名单 = 软面登记（同 Rust unreachable-pattern 的警告约定）」⇒ 默认全阻断会**推翻该裁定**，须登记豁免或改判为真 warning |
...
| 24 | TG02 | `EC_TG_BOUND`（:489） | `checker.cr:1227`/`:1233`/`:2059` | 泛型约束不满足 | 0（P5 清单：全语料 0 命中；唯一命中 = `test_generic_constr.py` 1 例登记面已重新锁定） | **①阻断** | 约束未满足 ⇒ 实例化假定不成立 |
...
| `.ccr` 四条判据 | `ptr_arith` / `generics_test` 两种形式 | `ccr`+`build` | 四条 = T3 新锁值（96015/96158/142793/142936） | **≥1 档**（`ptr_arith` 含 B04） | P6 T6 §Step 2 + `p6-task4-report.md` §4 |
...
|---|---|---|---|---|
| **裁-FC-1** | `check` 面与 `build` 面**是否用同一闸门**？**(A)** 现状（check 严 / build 宽）+ 本批只反转 build 面；**(B)** 两面同闸同表（check 也走豁免表 ⇒ 72 档 rc 分布换代、parity 基线重锁）；**(C)** 两面同闸但豁免表带**生效范围**字段（`build`/`check`/`both`），初始绝大多数条 = `build` 范围 ⇒ check 基线不动。 | 表 0 全行；72 档 + 探针两条对拍判据的可用性 | **(C)**（同机制、显式登记差异、check 基线零扰动；(B) 会让 ≈25 档 rc 1→0，parity 判据从「零差异」退化为「大面 diff」，丧失回归网价值） | 按 (A) 执行：只动 build/ccr/cir/run 四面，check 不动；差异显式登记 |
...
| **裁-FC-6** | **「零产物」的边界**：失败时 **(a)** 是否删除**本次**写入的 `<out>.ccr`（`main.cr:678` 写在 `corearch` 之前 ⇒ corearch 失败即留半成品；纯静态路径的 ELF 在 `ctx_emit_static` 末尾一次性写（`ld.cr:525-529`），本身不留半成品）？**(b)** `.core/cache/cir/*.cir` 过程缓存与 `.core/cache/cir/` 目录（`main.cr:473`）是否算产物？ | T5 的实现面 | **(a) 删除**（仅限本次 `save_ccr` 成功且 `corearch` rc≠0 的路径；**已存在的旧产物不动**——不删用户文件）；**(b) 登记为约定豁免**（过程缓存非交付产物；且 frontend 闸在 IR gen 之前 ⇒ 闸触发时不产生任何缓存） | 按推荐执行（不外扩删除面） |
...
| T6 | 收官：全量回归 + 判据复验 + 统一清单 + 文档 + 批终态 | 全部 |
...
- **`.ccr` 记录值引用约定（D28 继承）**：一律带**命令形式**（`corec ccr F -o O` vs `corec build F -o O --static`，差恒 143B）+ **缓存态**（冷/热；非可选程序冷≠热为**预存**豁免）；比较任何 `.ccr`/缓存态产物前 `clean-cache`。
...
| 7 | `.ccr` 两种形式四条（冷态） | `ptr_arith` **96015B** `680a6f98…` / **96158B** `76f36e6a…`；`generics_test` **142793B** `41e9d845…` / **142936B** `704316c8…`；**差恒 143B** | P6 T3 新锁值（P6 T6 §Step 2 复验） |
...
- **表 D：锚点实核**——本计划「Tech Stack」「表 0/1/2」的每个 `file:line` 逐条实读复核（**行号漂移以本轮实读为准**，漂移逐条写进文档）。
...
**停条件**：① 重建 sha 不可复现 ⇒ 停下（判据失效）；② 表 E 无原文 ⇒ 停下（本批方向性背书缺失）；③ 表 B 任一项与上表记录值不符且无法归因 ⇒ 停下（起点漂移 ⇒ 后续所有对拍无意义）。
...
- [ ] **Step 3: 逐档归因表**（表 2 的「转红面」列写进文档）：每档 = {码 → 归因（真错 / 假阳性 / 语境缺失 / 政策软面）→ 处置建议}。判据：**无「未知/待查」行**（不能归因的档 = 停条件下上报）。
...
- [ ] **Step 3: 裁决门提交**（裁-FC-1..6 逐条照抄问句；附 T1 的转红面逐 job 表作事实输入）。判据：六问**全部**有明文答复（**口头/转述不计**——照 P6 `p6-task4-report.md` §6 的「协调者已裁」需写进文档的先例，**须维护者本人答复**）。
- [ ] **Step 4: 报告**。

**判据**：表 1 无占位符 + 豁免表每条六字段齐 + 六问答复写进文档（含「不裁则按未取裁分支执行」的显式声明）。
...
**Files:** 按 T1 归因表逐条定；预期面 = `src/compiler/checker.cr`（若修前端假阳性：如 TF01 并发族、TF07 `@raw_int` 位、B004 末次使用）+ `tests/suite/*.cr`（若语料转负例，**须裁-FC-4 批准**）+ `src/ci/run.sh`（SKIP 语义若改）+ `tests/selfhost/test_*.py`（重新锁定）+ 本计划（豁免表终稿）。
...
- [ ] **Step 2: 零产物语义**（裁-FC-6）。判据：新套件断言——失败路径 `<out>` 与 `<out>.ccr` **均不存在**；**已存在的旧产物不被删除**（前置造一个旧文件 + 触发失败 ⇒ 旧文件仍在）；`.core/cache/cir/` 约定写进文档。
...
- [ ] **Step 3: 统一清单**：逐任务列 {实施 / 收紧 / 豁免 / 修复 / 语料处置 / 登记 / 偏差}——与各任务实际 diff 逐条对应；**豁免表终稿（含每条 exit 条件）单列一节**。
...
**判据**：五 CI job + 全枚举 + canary + 自举链 + 同源对拍 + 探针 + **命中面=豁免表的逐条等式** + 清单逐条可溯源 + 文档指针链闭合。
...
| R3 | **豁免表变成新的静默面**（码级豁免在别处静默放行） | 表只减不增 + 每条 `exit` 条件 + **每次开新面必重跑 report-only** + 清单单列一节 + 突变 M2/M3 咬合 |
...
| R6 | **判据失效**（若取「两面同闸」，72 档/探针两条对拍判据的「零差异」判据换代） | 裁-FC-1 推荐 (C)；若取 (B) ⇒ 先出「新期望分布 + 逐档归因」再开门 |
...
- **判定面清点的诚实性**：「42 可达码」= 全仓 grep 三形态（`check_error(EC_*` / `diag_type_incompatible(…, EC_*` / `DIAG_REC_SIZE, EC_*`）去重实得 43 减 1 个假命中（`EC_SIZE` 为 `DIAG_REC_SIZE` 子串）；**未**用「源码注释/文档统计」冒充实测（`docs/error-codes.md` 的 ~146 为文档自述值，已标出处）。分诊的 7 条豁免**全部带命中证据或政策出处**；其余 25 条阻断**全部按「判定继续 ⇒ 静默错产物 / 不可信」判据成文**（裁-P6-2(c) 约定），未用一个「大概/应该」。
...
- **72 档差异约定（先例继承）**：日志差异 **2 档 = 纯行号偏移 +53**（编辑编译器自身源码 ⇒ 语料含编译器 ⇒ 行号位移；先例 = P6 T2 的 +146）；rc 分布两侧 34×0/38×1 零差异。
- **新登记（本批施工中发现，另批）**：**安全面诊断随 `.cir` 暖缓存静默消失**（冷 rc=1 / 暖 rc=0 出 ELF；P6 期二进制同病 ⇒ 预存）⇒ TODO #2026-09-15-5（含派生建议：两条 runner 加「暖态判据」，**本批不实施**）。
...
- **判据**：**默认关两态零差异**（72 档 `diff -rq` 零 + 探针 29 零）· 开态 rc/产物 sha 逐字节同（仅多一行标记）· 重放 101 档 **无表外命中**（语料 25 档 blocked 与 T1 的 34 行 − 7 build 豁免 − 2 P21 对齐；探针 11 档 = T1 未覆盖面，形式差登记）· 五 CI 5/5 · 枚举 60/60 · canary IDENTICAL · `.ccr` 四条同 · 链 `24802386a1…` IDENTICAL · `backend_bootstrap` rc=0。
- **约定注**：报告面只覆盖**前端类型面闸**（`g_diag_count > 0` 块）；**解析阶段闸**（`:134/:135`）与**安全检查面闸**（`:594-603`）的诊断不出现在报告行中（前者如 P21、后者如 TU03）——重放对表时须按此约定解读。
- **环境干扰事件（非本批代理所为，登记）**：T3 测量期间 `build/`（整目录）两次被外部删除、`examples/*.ccr` 等 7 个 tracked 文件被删（`jj status` 见 `D`）、`/tmp/fct3/parity_*/parity.out` 被删 ⇒ 首轮 parity/probe 判据作废（rc=127 满屏）；重建后（corec sha 复原 `308f06a3…`）**全部重跑并取有效值**。**未提交、未还原**那 7 条 `D`（铁律 #3）。

---

## 统一清单（FC 批 T1–T6；**T6 收官**）

> 约定（继承 P6/P5）：逐件给**证据**与**替代/覆盖**；全语料命中数一律与**同任务起点**基线逐条比。
> **KPI**：实施 **3 件**（T2/T3 + T1 扫面）· 收紧 **1 类**（闸门反转 = 默认阻断）· 放宽 **0** ·
> 修复 **1 件**（TC02，前批 `7f53305d`，本批引用）· 豁免 **9 条**（带退出条件）· 登记 **4 件** ·
> 偏差 **6 条** · 环境干扰 **1 起** · **T4/T5 折入 T2（已批准）**。
...
- **T4（命中面处置）**= **由 T2 覆盖**：命中面以**豁免表 + 退出条件**形式落定（9 条逐条带「位点证据/理由/退出条件」），**无根因修复需求**（维护者裁定方向 = 豁免主导；唯二根因件 TC02 已在前批 `7f53305d` 修复）。
- **T5（开门 + 零产物 + CI 面同步）**= **由 T2 覆盖**：**开门已在 T2 生效**（默认阻断 = 唯一语义，无「report-only 默认关」形态需切换）；零产物删除点 + 边界（只删本次写下者）在 T2 落定；CI 面（`run.sh` 挂点）已在 T2 同步。
- **覆盖理由**：T2 的实施顺序把「先 report-only 清单审查 → 再开门」与「豁免表定稿」合并为一次落地（T1 已提供清单审查面），故 T4/T5 无独立交付物。**不静默略过**：本条即其显式记录。

### 3. 收紧 / 放宽（各 1 / 0）

| # | 形态（前 → 后） | 件 | 全语料命中 | 牙齿 |
|---|---|---|---|---|
| 1 | 类型面闸：**正向名单（11 码）→ 默认阻断 + 豁免表（9 条）** | T2 | 起点即含全部命中（T1 已清点）；T2 后**无表外命中** | 突变 M1（删豁免条 ⇒ 复红）· M2（scope 失效 ⇒ 静默放行被抓）· M3（去零产物 ⇒ 半成品）· M4（全放行 ⇒ 负控转绿） |
| — | **放宽 0 件**（本批无放宽动作；TC02 的放宽发生在前批） | — | — | — |

### 4. 豁免表 9 条（含触发位点与退出条件；载体 = `diag.cr`，只减不增）

| # | 码 | scope | 触发位点（实测） | 退出条件 |
|---|---|---|---|---|
| 1 | TF01 | build | 5 档并发族 suite build | 修前端根因后撤 |
| 2 | TF07 | build | `ptr_ref_first.cr:8`（`@raw_int`） | #20/F3 收尾后撤 |
| 3 | TB01 | build | `ptr_ref_first` + 8 档库单元（**级联码**） | 随根因码消失 |
| 4 | TM04 | build | `test_match_exhaust.py:217`（用例锁定「软面 rc=0」） | 政策改判 |
| 5 | TK01 | build | `test_xcut_iface.py:234-241`（用例锁定「软诊断面」） | 政策改判 |
| 6 | B04 | build | `tests/suite/ptr_arith.cr:12:8`（canary 载体） | 裁-FC-3 (c)/(i) 后撤 |
| 7-9 | N01 / N06 / N11 | check（**仅登记**，(C) 终局 ⇒ 惰性） | 25 档库单元单独 check | 约定固定；不得据此实现 (B) 活分支 |
...
| 2 | **TODO #2026-09-15-5** 【高危·预存】安全面诊断随 `.cir` 暖缓存静默消失（冷 rc=1 / 暖 rc=0 出 ELF；P6 期二进制同病）+ **派生建议：runner 加暖态判据（本批不实施）** | 独立批候选 | 已落 TODO |
| 3 | **TODO #2026-09-15-4** FC 批 T3 隐藏通道（形态/判据/对表/约定注/登记） | 本批 | 已落 TODO |
...
| # | 现象 | 处置 / 出处 |
|---|---|---|
| 1 | **72 档日志差异 2 档 = 纯行号偏移 +53**（`_import.cr` / `ccr_io.cr`：29935→29988 / 23506→23559） | 语料含编译器自身源码、本批编辑了它 ⇒ 位移；**rc 与诊断逐条同**；先例 = P6 T2「+146」 |
...
| 4 | **corearch sha 约定澄清**：lead 给的 `f69240ae…` = **自举链** sha（非 corearch） | 已核；本批 corearch = `22e96d03…` |
| 5 | **B6 项（安全面负控）如实保留**：冷态 PASS / **暖态受预存缺陷影响**（诊断消失 ⇒ rc=0 出 ELF） | **未加豁免、未改判据**；归 TODO #2026-09-15-5 |
| 6 | **T3 测量期环境干扰**：`build/` 与 `examples/*` 被外部删 ⇒ 首轮 parity/probe 判据作废（rc=127 满屏）；`/tmp/fct3/parity_*/parity.out` 被删 | 重建（corec sha 两次复原 `308f06a3…` + 冒烟 42 ⇒ 干扰不改构建产物）后**全部重跑取有效值**；证据目录未删（本批自用仅 11MB，`/tmp` 95% 占用非本批所致） |

### 7. 约定注（T3 引入；对表/复放须按此解读）

**`--diag-gate-report` 的报告面只覆盖前端类型面闸**（`g_diag_count > 0` 块）——**解析阶段闸**（`main.cr:134/:135`，如 P21）与**安全检查面闸**（`:594-603`，如 TU03）的诊断**不出现在报告行**（实测：TU03 档 on/off 两态 0 行差异）。⇒ T3 重放对表时，「T1 有而重放无」的 2 行（`at_test_mini4/6`）即由此约定解释。
...
| 面 | 终态 | 出处 |
|---|---|---|
| **闸门语义** | **默认阻断 + 豁免登记表（9 条）**——任何不在表内的类型面诊断 ⇒ `rc=1` + **零产物**；语法面（P/L 族）与安全检查面（B11/TU03…）本就「任一即 rc=1」（**未改动**） | T2 `b01a948c` |
| **豁免表** | `diag.cr::diag_gate_exempt`（码级 9 条：build ×6 + check ×3[仅登记]；逐条带位点证据/理由/退出条件；**只减不增**纪律入头注）· scope 常量落 `globals.cr` | T2 |
| **零产物** | corearch 失败 ⇒ 删**本次**写下的 `<out>.ccr`；既有旧文件不删（裁-FC-6）；ELF 侧本就无半成品（内存缓冲后一次写） | T2 |
| **隐藏通道** | `--diag-gate-report`（默认关、只读、两态零差异；报告行 = `[diag-gate] face=build blocked=N total=M codes=…`） | T3 `3990a0a5` |
| **回归判据终值** | 五 CI **5/5 rc=0** · 枚举 **60/60** · selftest **415/415** · canary **`95084e7b…d475`（28822B）IDENTICAL** · `.ccr` 四条 96015/96158/142793/142936（差恒 143B）· dump 3521 · cir 165915B · 链 `24802386a1…` IDENTICAL + N06=0 + 冒烟 42 · `backend_bootstrap` rc=0 · 72 档 rc 零差异 · 探针零差异 | T6 |
| **测试** | 新套件 `test_diag_gate.py` **17 例**（正控 9 / 负控 6[三面] / 零产物 3）+ `run.sh` 挂点；`test_tc02_branch.py` 15 例（前批） | T2 |
...
**三、剩余登记面**：**TODO #2026-09-15-5**（缓存静默缺陷 + runner 暖态判据建议——**独立批候选**，维护者已采信）· **环境干扰事件**（12 个 tracked `D`，**待维护者处置**，不得擅自还原）· 豁免表 9 条的**退出条件**（逐条在表内；撤条须带根因证据、加条须维护者批）· T3 报告面约定注（P21/TU03 不在报告行）。
