...
## 统一台账（FC 批 T1–T6；**T6 收官**）

> 口径（继承 P6/P5）：逐件给**证据**与**替代/覆盖**；全语料命中数一律与**同任务起点**基线逐条比。
...
| 2 | TF07 | build | `ptr_ref_first.cr:8`（`@raw_int`） | #20/F3 收口后撤 |
| 3 | TB01 | build | `ptr_ref_first` + 8 档库单元（**级联码**） | 随根因码消失 |
| 4 | TM04 | build | `test_match_exhaust.py:217`（用例钉「软面 rc=0」） | 政策改判 |
| 5 | TK01 | build | `test_xcut_iface.py:234-241`（用例钉「软诊断面」） | 政策改判 |
| 6 | B04 | build | `tests/suite/ptr_arith.cr:12:8`（canary 载体） | 裁-FC-3 (c)/(i) 后撤 |
| 7-9 | N01 / N06 / N11 | check（**仅登记**，(C) 终局 ⇒ 惰性） | 25 档库单元单独 check | 口径固定；不得据此实现 (B) 活分支 |
...
**`--diag-gate-report` 的报告面只覆盖前端类型面闸**（`g_diag_count > 0` 块）——**解析阶段闸**（`main.cr:134/:135`，如 P21）与**安全检查面闸**（`:594-603`，如 TU03）的诊断**不出现在报告行**（实测：TU03 档 on/off 两态 0 行差异）。⇒ T3 重放对表时，「T1 有而重放无」的 2 行（`at_test_mini4/6`）即由此口径解释。
