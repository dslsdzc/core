...
**建议（裁-V3 的推荐案）**：**验证消费面 = `.ccr`**（+ 若需规约元数据，则 `.csr` 与 `.ccr` **并列产物**，`target_node` 指向 NOD 索引）；`.cir` 面只作**开发期 dump**。理由：① `.ccr` 有版本闸/段表/确定性/读回四件套（P4–P6 已打牢）；② 让验证工具依赖**私有缓存**（`.cir` 快照）会把「缓存键/编译器身份」这类 TODO #5 家族的坑带进验证链；③ 若坚持 `.cir` 优先，等价于**新增一个对外序列化格式** ⇒ 应作为**独立裁项**（工作量远大于 C1）。
...
- **判据口径按 TODO #26**（结构性断言 + 语义零变化 + 自举稳定，非「与旧版逐字节同」）· 三态纪律 · 新硬错先 **report-only** 全语料 · 清单三面同核 + `test_backend_bootstrap`（`error[`=0）。
...
- **与既有裁决的一致性**：ADR-0001（规约并入 `.cr`）· spec-design §十二（unproven 不拦）· §十三（只有 proven 进优化器）· P4 T5 的「零足迹」先例（裁-V4）· TODO #26 判据口径 · P6 事故护栏（含「先裁」入卡）。
