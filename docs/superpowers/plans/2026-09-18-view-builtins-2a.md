...
| # | 机制 | 代价 / 为何不取（或何时才取） |
|---|---|---|
| **B** | **全 route 1**：把裸字缓冲改成存 **intern 索引**（`w64(buf, off, str_intern(s))` + `r64` 读出后 `istr_get`） | **不需要任何新语法**（可**立即**修完 route 2 点），但**改变存储语义**：缓冲里不再是指针字 ⇒ 需逐点证「该字从不被当作指针消费」（syscall/FFI/长度头）；**intern 表增长**（长跑内存面）；且把「无类型裸字」问题**藏进**索引约定。**适用**：仅当某点的字确实只在本档内往返（可作为 route 2 的**子集**手段） |
