# 2(a) 视图内建批（`@ptr_of` / `@str_of`）—— 计划（**规划轮：只读侦查 + 写进文档**；未实施）

**日期**：2026-09-18 · **工作区**：`core-optdex-ws` · **性质**：纸面（team-lead 裁定「2(a) 立批先出计划」）
**前置**：bool 类 171 处已收尾（PR #128）；S2 残桶 = 68 点 string↔int 待收尾；S3 硬错等本批。
...
- **本批范围**：① `@ptr_of`/`@str_of` 两面实现（bootstrap + 自托管）；② 68 点按类归位（route 1 用 `istr_get`，route 2 用视图内建）；
  ③ 值面 e2e（**cli.cr 类目前语料零覆盖**，见 §2.3）；④ `-17` 的分流写进文档（§6）。
...
⚠ **约定**：S1P 的 `line` 是**拼接源坐标**（corec 清单 45 档顺序拼接），不是档内行号。
...
### §2.2ter **两套表示并存（已写进文档的既有事实）——分类时**必须**逐点判「哪一套」**
...
- 命中点在 **`cli.cr`/`parser.cr`**：这两档在**两种运行路径下都要成立**（解释器 + 原生），
  其裸字缓冲存的必须是**同一套表示**（否则两条判据分歧）⇒ 逐点须答「这条路径下这个字是什么」，
  **不得**只按一个面判（这正是 §7.3 要求值面 e2e 跑**两条判据**的原因）。
...
| **D** | 改 `r64`/`w64` 签名为**泛型/字类型** | 语言层缺「任意字类型」概念；等于设计新类型系统件，**远超收尾批** |
...
**误读纠正（必须一并写进文档）**：`@sizeOf(...)` **在语料（自托管面）可用**——`tests/suite/opt_dex_test.cr`（15 处 `@raw_int`）等档就是活证；
**在编译器源码（bootstrap 面）不可用**。「不可用」永远**要带面**说，否则会把语料面能力误判成语言缺口。

## §7 判据要件（实施批的验收线）

1. **清单逐桶**：`CORE_S1P=1 … check src/compiler/main.cr` ⇒ 68 → **0/或仅有已声明的残点**；**bool 桶仍为 0**（PR #128 的判据⑦不得回退）。
2. **两面一致**：同一探针在两面的 rc + 运行值一致；bootstrap 面**未知内建名必须 fail-closed**（正控 + 负控各一条）。
3. **值面 e2e（新增，必做）**：`tests/suite/` 新增「裸字缓冲往返」档——把串存进裸缓冲 → 读回 → 打印比较；
   **`cli.cr` 类必须有一档**（今日零覆盖，§2.3）；`@ptr_of(@str_of(p)) == p` 恒等判据。
   **必须跑两条判据**：解释器（`corec run`）**与**原生 ELF（`build --static`）——因 §2.2ter 的两套表示，
   单条判据绿**不**证明另一判据绿（这正是本类缺陷的风险面）。
4. **逐点语义依据**（**清单归零 ≠ 语义正确**）：每个点必须写「这个字是**索引**还是**指针**」的依据（赋值链/消费点），
   两类的修法不同；不得按文件整批套用。
...
| P3 | route 1 点先归位（`istr_get`，**逐点给依据**） | 清单桶下降且逐点留痕 |
| P4 | route 2 点归位（视图内建）+ 新 e2e + 突变自证 | 清单 0 + 值面绿 |
| P5 | 写进文档（`-17` 分流、覆盖边界、版本位）+ 登记 | 文档 + CI 挂点 |
...
- **为什么不用 `@raw_int` 的「新建变量 + IR_STORE」**：那换的是 dex↔int（XMM vs GP 两条真不同机器路径）；
  int↔string 两条判据都是同一 GP 字 ⇒ 搬运只会多一条 IR_STORE 并**破坏可擦除性**（实测该版本 ② 判据红）。

### §10.2 迁移结果（68 → 0）

| 档 | 点数 | 修法 |
|---|---|---|
| `src/stdlib/cli.cr` | 29 | 注册 `@ptr_of` · 读取 `@str_of` · `_cli_find_flag` 比较 `@str_of`；**另修 2 处返回位**（`cli_get`/`cli_arg`，清单不收——见 §10.4） |
...
却 `return r64(...)`，**68 点清单从未收录**（本批一并修）。⇒ **S3 升硬错前必须补返回位**，否则同类点仍静默。
...
- **突变自证（两条，均断言命中目标）**：① 回退 `_cli_find_flag` 首视图点 ⇒ 清单 0→**2**（`str_len`/`str_eq` 复现）；
