...
**性质**：`只读侦查 → 写进文档 → 取裁 → （裁后）实施`。**本轮零源码改动、零构建**。
...
**本文件约定**：所有 `file:line` 均为 **`develop 2cfa2646` 现状**（亲自读过；与 TODO/审计中的旧行号有偏移 ⇒ 见 §0.4 坐标校正表）。凡标 **预判** 者 = 读码推断、**待 T1 实测**；凡标 **实核** 者 = 本轮直接读到的代码行。
...
| **N1**（**非回归断言**） | `s.f == d` / `@raw_int(s.f)`（apx 批已转正的 B-3/B-4/B-5/B-6/B-8 形） | — | **今天绿，修后须仍绿**（防「修法把正确的搞坏」） |
...
① 为主，另在 LET（`:2458`）与读点补登记，使 `slot_decl_ti` 与 `irv_type` 两种形式**一致**。代价 = ① 的足迹 + 侧表一致性维护；收益 = 消除「两种形式不一致」的隐性坑。**推荐序：① → ③（若实施中发现 `slot_decl_ti` 约定有人依赖）**。
...
- ⇒ **副产物**：锁定 5 条是**实现约定的判别器**——若有人把候选 ① 做成**无条件**（不判声明面）施加，`generics_test.cr` 的 `b.val` 读槽会翻成 `TI_DEX_S` ⇒ `gt_ccr`/`gt_static_ccr` **必变**。**这条回归本身就能抓住「约定错误」**，须写进 Global Constraints。
...
|---|---|---|
| ELF canary `95084e7b…d475`(28822B) | **IDENTICAL** | 停，查约定 |
| `.ccr` 四条（96015/96158/142765/142908） | **IDENTICAL** | 停，查约定（极可能是「无条件施加」） |
...
| **腿 D（突变控制）** | ① **关掉 apx hack**（`:1285-1286` 注释掉）⇒ 腿 A/B **仍须全绿**（证明通用机制独立成立）；② **把读点改成无条件 `TI_DEX_S`** ⇒ 锁定 5 条**必红**（证明 canary 判别器有效、约定不是摆设） | 两条突变都必须按预期红/绿 |
| **腿 E（适用范围）** | 非 dex 聚合读（`int` 字段/数组）**必须零足迹**：产物与改前逐字节相同 | 逐字节 `cmp` |
...
| **裁-AGG-7** | `.ccr`/`.cir` 若如预期**不变**（锁定语料零 dex），是否仍要 bump 版本号？ | 不 bump = 旧产物仍被命中（**语义一致**，因为变化只发生在 dex 聚合读程序——它们的旧 `.ccr`/`.cir` 与新版**不等价**！） | **需你裁**：建议 **bump `.cir`（`CIR_CACHE_VER` 17→18）+ 对 `.ccr` 按「同版本号但语义变了」的风险给结论**。§10 U5 给出两种约定的代价 | 停，等裁 |
| **裁-AGG-8** | 腿 D②（无条件突变）导致锁定 5 条必红——是否把该突变固化为常驻判据（照判据批 22/22 先例）？ | 固化 = 防「约定被改宽」；代价 = CI 时间 | **固化**（一行开关 + 期望必红） | 不固化 |
...
| **T2** | **先裁写进文档**（八门结论）+ 探针入库（RED 状态，白名单登记） | 门逐条有裁决 |
...
2. **约定门**：定型**只对声明面为 dex** 生效；**非 dex 聚合读逐字节不变**（腿 E 是这一条的判据）。
...
- **U5**：**`.cir`/`.ccr` 是否 bump**（裁-AGG-7）：约定 A = 不 bump（理由：锁定 5 条不变、且变化只落在 dex 聚合读程序）；约定 B = bump `.cir` 17→18（理由：旧 `.cir` 快照里读槽型是 `TI_INT`，与新建的 `TI_DEX_S` **不等价** ⇒ 同一源文件在修复前后会命中不同语义的旧快照，**这正是 #8 缓存事故的同族**）。**建议约定 B**（bump `.cir`）+ **`.ccr` 侧**同步给出结论（`.ccr` 无「快照复用」语义 ⇒ 可只留痕不 bump，但须写成显式声明）。
...
| R7 Core 实参（断言） | 7 | **7** | **绿**（与读码判定一致 ⇒ 归类修正，§11.6） |
| N1 apx 批已转正形（断言） | 7 | **7** | **绿** |

全部 8 档 `check` = 0 error（**无诊断**：这正是「静默」的实测证据）。

### 11.4 IR 级机理正据（比 rc 更强的断言）
...
| **R7**（断言） | `load_field field = s.0` → `call dbl(field)` ——零转换**且正确** | Core 形参物化 scaled ↔ 读值 scaled **同形** ⇒ 直传正确 |
...
### 11.6 归类修正（T1 实测结论 ⇒ 改约定）

- **R7：「RED 候选」→「非回归断言」**（读码 + 实测双证：今日 rc=7）。**修后必须仍绿**。
...
- **U4 裁后约定**：extern 边界 = **规范化成 ABI binary64**（`ir_gen.cr:1900-1907` 只做 `TI_DEX_S → bits`）⇒ R6 期望 = **转换**（不是「不变」、也不是「转 scaled」）。
...
- **新文件**：`tests/selfhost/test_agg_read_type.py`（**未挂 CI**，白名单有条目）。
  - **腿 A（语义值）**：R1 LET 中转 · R2 match 载荷 · R3 局部数组元素 · R4 元组元素 · R5 写回 apx 槽
    （期望 `elf=7`）。
  - **腿 C（非回归断言）**：R7 Core 调用实参 · N1 apx 批已转正形（改前即绿、修后须仍绿）。
...
- **落点**：`tools/baseline/REBUILD.md` 新增 **`### 命令层陷阱（假红）`**（置于「换代纪律」之前，
  与既有「适用范围限制 / 已知限制」同区）——① `noclobber`：重定向到已存在日志 ⇒ **命令根本没跑**、
...
- **判据**：cache miss ⇒ 无害重建（照 TODO #2026-09-10-4 收尾约定）；canary/五条**不受影响**（无 dex）——
...
- T2-3：R6 的断言在 T3 实施后需**按实际 cir 形态复核**（转换序列可能随实现约定变化）。

---

## 13. §T3 记录（2026-09-16 —— 实施 + 判据 + 突变 + 换代重锁）

> 维护者 T3 放行（2026-09-16）：① 事故「我担一半」+ 正确配方入派单约定；② 门控见证设计获肯定（「先确认工具链能不能表达 X」）；
> ③ 实施约定 = 声明面形式判定 + 3 读点定型 + `CIR_CACHE_VER` 17→18 + 腿 A/B 转绿 + 挂点三件套；判据 = 腿 A 5/5（**见 §13.4 冲突**）·
> 腿 B 四条见证 · 腿 C 断言 · **停条件①**（非 dex 任何字节变化 ⇒ 停）· **停条件②**（关 hack 后腿 A 红 ⇒ 停）。
...
### 13.4 ⚠ R4（元组）与 T3 约定的**冲突**（须你裁）

- 你的 T3 约定写「腿 A **5/5** 转绿（R1–R5）」，但 **裁-AGG-3** 已裁「**元组**（无声明面）**登记，不纳入本批**」。
...
- **若要现在就覆盖元组**（最小机制，供你裁）：在元组构造点按「**全元素同形**」把该形式登记进**既有声明侧表**（`irv_set_decl_ti(tv, TI_DEX_S)`，2 行、零 alloc）；数字下标读再查 `slot_decl_ti(obj_var)`。
  **代价/风险**：`slot_decl_ti(tv)` 变 `TI_DEX_S` 会波及唯一取址面 `ir_gen.cr:1530`（`&tuple` 的 pointee 型行身份：`TYP_PTR(TI_INT)` → `TYP_PTR(TI_DEX_S)`）⇒ 需一条实证；且这是**新机制**，与「最小改动」约定相抵。**推荐：维持裁-AGG-3（登记）**。
...
> 命令形式：`CI_JOB_NAME=<job> bash src/ci/run.sh`（**argv 写法 = 假红**，见 `tools/baseline/REBUILD.md` 的命令层陷阱段）。
...
`CIR_CACHE_VER` 17→18；**腿 A 4/5 转绿（R4 按裁-AGG-3 登记 `[GAP]`）· 腿 B 四条见证 · 腿 C 断言 · canary 5/5 IDENTICAL ·
