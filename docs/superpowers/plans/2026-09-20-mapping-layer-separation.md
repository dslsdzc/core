...
### ⑤-bis ⚠ **两条腿（interp / ELF）在「地址/arena 面」已有可观测差异**——**既有**差异，本批可能改变其可见性

**性质**：这是**既有**分歧（不是本批引入），但**本批（二档）可能改变它的可见性** ⇒ 必须算进强度表；**至于该不该对齐——不是本批的事**（本计划只给观测，不裁对错）。

**观测（最小、两腿可跑、rc 即读数）**——`corec run <内联源码>` 的 **rc = `main` 的返回值**（已自证：`return 42` ⇒ rc=42）：

```core
fn main() -> int {
    i : ., mut = 0;  prev : ., mut = 0;  gap : ., mut = 0;
    loop {
        if i >= 3 { break; }
        s := alloc(8);
        p : ., mut = 0;
...
| 探针 | interp 腿（`corec run`） | ELF 腿（`corec build --static` + 执行） |
|---|---|---|
| 上式（三次 `alloc(8)` 的**末次距离**） | **rc = 0** | **rc = 16** |
| 单次 `alloc(8)` 的 `@ptr_of` 是否 0 | **是（rc=7 分支：地址 = 0）** | 否（距离 16 非 0，故地址非 0） |
| 自证 rc 通道 | `main` 返回 42 ⇒ **rc=42** ✓ | —— |

⇒ **观测成立**（两条腿输出不同：**0 vs 16**）。

**⚠ 定界（诚实登记，勿过度解读）**：本探针证明的是「**两条腿在地址/arena 面可区分**」，**不是**「差异的成因是 arena no-op」——
interp 侧 `@ptr_of(s)` **恒 0**（与「interp 的串是 intern 索引、不是地址」一致），**该事实本身已足以造成本差异** ⇒ **「arena no-op」与「无地址概念」两条成因本探针未分离**（**待实测/待裁**）。
（`interp.cr` 注释自陈「interp 无 arena——no-op 近似」是**注释**，与上表读数**分开看待**。）
...
6. **⚠ 二档任何一步若改变了两条腿在 §⑤-bis 那个观测上的关系**（`corec run` 的 rc 与 ELF 腿的 rc 由「0 vs 16」变成别的关系）**且未登记** ⇒ **停**（登记前不得继续）；**即使新关系「看起来更一致」也停**——「顺手对齐」同样是未经裁决的状态变更。
...
7. （**已升入 §⑤-bis，见该节**）解释器腿与 ELF 腿在**地址/arena 面**的可观测差异——**本批实测：`0` vs `16`**；该条属**既有**差异且**零判据覆盖**，故按 team-lead 裁定**移出本节、列为 §⑤ 的并列项**（强度表要把它算进去）。
