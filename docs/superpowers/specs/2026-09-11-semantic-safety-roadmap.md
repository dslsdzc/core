# 语义安全路线图与九份设计 spec 的共享契约

日期：2026-09-11
状态：**路线图定稿（用户裁决：全部起草）+ 收敛批 v3 补 §4 编号对照表（2026-09-11）**——本文件不是设计本身，而是 9 份设计 spec 的**共享契约 + 清单 + 起草/实施节奏 + 编号对照**。
...
**轴语义出处（2026-09-11 用户裁决 D1(a)）**：`trust` = **可判的传播轴**（来源 = 库声明 + 自动推导 + 净化节点）；**来源真实性的 attestation 不可判**（属 §1.3 第 2 类），但**程序内传播可判**——该传播面即 `trust` 轴。`integrity`（= 可篡改性，与 `trust` 对偶但**不同轴**）不因「不可判」改写；S-B3 的 `Secret ↛ Public` 对偶方向（`Untrusted ↛ Trusted`）**消费 `trust` 轴**，不单列第二判定面。三份消费方（S-A / S-B1 / S-B3）各写「谁定义 / 谁判定 / 谁是消费者」对表（承 D1 共同要求）。

**放行 / 声明条目（2026-09-11 用户裁决 D2(a)）**：任何**绕过义务的通道**（`¬义务 → 义务`）**只**经 S-A §3.1 的「放行 / 声明条目」栏（`kind{declassify｜grant｜deny｜mint｜stability} · 主体/身份 · 目标/范围 · 理由 · 派生源 · 义务引用`），且**必须落清单**。**`kind` 闭集五枚**：`declassify`（显式降密/净化）· `grant`/`deny`（**既存**对象/权限的授予/拒绝）· **`mint`（能力创建点 = 根声明 / `alloc_at` 类例外入口授权——声明的目标即新建域）** · `stability`（治理层稳定性注解）——**四族填法** = S-B3（`declassify`）· S-B2（`grant｜deny`）· **S-B4（`mint`）** · S-B5（`stability`）（B 线 N2）。**形态默认 = 图上显式节点**；偏离节点形态者（`grant`/`deny` 边集合、`mint` 的声明型根条目、`stability` 注解）须在 S-A **显式豁免并补审计字段**——不得沉默。
...
**默认强度分层（2026-09-11 用户裁决 D3，总约定）**：本地开发 = 登记 + 继续编译；**strict 档 / CI 门槛 = 严（拒编）**；② **enforce 默认启用**（可 enforce 的一律 enforce，高安全档关闭）；**未声明的边界条目取 `unknown`（保守判定方向）但不拒编**，报告面可见。细则落 S-A §2.2 附。
...
1. **起草阶段（本次）**：9 份 spec 全部起草（本路线图 = 共享契约，各 spec 只填自己的范围，**不得**重定义 §1.1 术语或 §1.3 不承诺）。
2. **实施顺序（建议）**：
...
## 4. 编号对照表（2026-09-11 收敛批 v3 新增；**规范性**）

**背景（B 线 N2）**：十份文件里并存 **五套编号**，其中两套（用户裁决 `D<n>` 与各 spec 本地 `D<n>`）**在同一文件内撞义**——实例：S-D 的本地 `D8`（记录随缓存条目存取）vs `D8`（deadline 仅诊断，用户裁决）；S-C 的本地 `D4`/`D5`/`D7` 同理。本节给唯一对照，并**注销无定义表的编号**。**此后约定**：本地裁决一律写 `L<n>`；用户/收敛裁决写 `D<n>`（首次出现可用 `★D<n>` 强调）；评审条目一律带线别（「A 线 M1」/「B 线 C2」）。

### 4.1 五个编号空间

| 编号空间 | 写法 | 定义位置（唯一依据） | 范围 | 说明 |
|---|---|---|---|---|
| **用户/收敛裁决** | `D<n>`（★ 强调形同义） | 全量 = `.superpowers/sdd/decisions-sheet.md`；A 簇索引 = **S-A §0.4** | D1–D30 | D1–D7 / D9–D15 / D19 / D22 / D23 / D29 / D30 落点在 A 簇 spec；**D8 / D16 / D17 / D18 / D20 / D21 / D24–D28 落点在 B 簇 spec**（S-C / S-D / S-E——S-A §0.4 尾注已声明，本表为补齐） |
| **本地编号** | `L<n>`（**收敛批 v3 统一改名**，原 `D<n>` / `D-n` / `D-1…D-8`） | **裁决表**：S-C §0.2（L1–L7）· S-D §0.3（L1–L15）· S-E §0.3（L1–L8）· S-B3 §0.3（L1–L6）· S-B4 §0.3（L1–L7）；**判据编号**：S-B1 §6.1（L1–L5 = 验收门）。**层名**：S-B4/S-B5 的构造合法性 / 解析身份两层 = `LV-1` / `LV-2`（原 `L-1`/`L-2`，收敛批 v3 改名以消同形混淆） | 各 spec 内 | 只在本 spec 内引用；与 `D<n>` **不得混用**（N2 的根因）。无本地编号表的文件：roadmap / S-A / S-B2 / S-B5（其 §0.3 用「决 1/2/3」） |
| **A 线评审编号** | 「A 线 C/I/M/N」 | `.superpowers/sdd/safety-review-a.md`（正文小节） | C1–C3 · I1–I5 · M1–M8 · N1–N5 | 可直接定位 |
| **B 线评审编号** | 「B 线 C/I/M/N」 | `.superpowers/sdd/safety-review-b.md`（正文小节） | C1–C4 · I1–I5 · M1–M6 · N1–N8 | 可直接定位。**警告**：A/B 两线的 `C1`/`M1` 等编号**互不相同**，引用必须带线别（同一文件内同时出现两线编号时，后加线别的写法为准） |
| **张力 / 待实测编号** | `Tn` | A 线 `T1–T4` = **S-A §0.3 四处张力**；B 线 `T1–T5` = `safety-review-b.md` §4.3「待实测」 | 见左 | 跨文件引用须带线别 |

### 4.2 注销登记（**无定义表 ⇒ 不得再引用**）

| 编号 | 判定 | 依据 |
|---|---|---|
| B 线 `B1` / `B2` / `B5` / `B9` / `B10` / `B11` / `B13` | **注销**（评审单轮内部编号，无定义表、不可追溯） | 全仓检索无定义；正文引用处已按 §4.3 改写为可定位编号 |
| B 线 `T6` / `T7` / `T8` / `T9` / `T10` / `T11` / `T12` | **注销**（同上；**`T10–T12` 两份评审 + 十份 spec 全仓 0 命中**，评审原文亦未定义） | 同上；B 线实际存在的待实测编号只有 `T1–T5` |

### 4.3 已改写的引用（收敛批 v3 闭环）

| 原引用 | 位置 | 改写为 | 依据 |
|---|---|---|---|
| `B1/T8` | S-D §5 S-D3 判据⑤ | **A 线 M1**（门禁约定一律引 R2 §4 该行） | 该处语义 = A 线 M1 的修法 |
| `M2/B9` | S-C §8 O6 | **B 线 M2**（O6 改「规模上界实测」） | 该处语义 = B 线 M2 的修法 |
| `M3/B10` | S-C §6 切片前置 1 | **B 线 M3**（`ir_gen.cr:1590-1593` 行号校正） | 该处语义 = B 线 M3 |
| `M4/B11` | S-E §5.1-(1) 行形说明 | **A 线 M4**（落档表行形） | 该处语义 = A 线 M4 |
| `M6/B13` | S-D §5 S-D3 范围行 | **B 线 M6**（`det` 进不做清单） | 该处语义 = B 线 M6 |
| `T9` | S-B2 §3.3 enforce 行 | **A 线 ⚠️5**（`cmp+jb+ud2` 与 `sub+cmp+jae+ud2` 同义统一） | **按语义对应认定**（非引用）——该行所说的"编码措辞统一"即 A 线 ⚠️5 |

### 4.4 两类编号**不再新增**（防复发）

- 新增本地裁决 / 判据**必须**用 `L<n>` 并登记在本 spec 的表；**不得**再用裸 `D<n>`。
- 评审条目引用**必须**带线别（「A 线 M1」）；**不得**引用 §4.2 的注销编号。
- **既存同形编号的引用纪律**（不改名，只要求带出处）：S-B2 §0.4 的 `C1`–`C3`（M1 修订触发条件）· S-B1 §0.2 的 `1`–`10`（本族裁决序号）· S-B5 §0.3 的「决 1/2/3」——引用时**必须带出处**（如「S-B2 §0.4 C2」），裸引用与 A/B 线评审的 `C<n>` 视为歧义。
