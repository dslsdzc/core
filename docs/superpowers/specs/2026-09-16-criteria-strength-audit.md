...
- **强判据四写法**：① **连续序列断言**（整条指令字节，含 `??` 通配位）；② **带定长 gap 的正则**（`..{0,N}`，N 尽量小并写明理由——A1 即此形）；③ **锚点 + 定长窗口**（长度可变段用**长度公式**，如 A2 的 `text[m+1]==0x83 and text[m+2]==0xEC → text[m+3]`）；④ **反例驱动**：每条字节判据评审时须回答「**什么坏实现能骗过它**」——写不出反例 ≠ 强，写得出而判据仍绿 = 弱判据。
...
| A8 | `test_hit_table.py:1212-1224`（判据 `:1220`）| 表数据派生 legacy 字节流（事件 1-4）：`chk(stream[:2] == want, …)` | **弱-前缀**（**B2**）|
...
| **B3** | `test_hit_table.py:281` | 未用哨兵字段检查 = **4 字面量黑名单** | 未用字段写 `1` 或 `0xFE`(−2) ⇒ 不出现 `255/-1` 字面 ⇒ **绿** | 无（该字段无消费者 ⇒ 危害低）|
