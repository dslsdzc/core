...
| 前六段字节 | — | — | **布局逐字节不变**（取值随 STR 段增长重编号——IFACE 注册名驻留 ⇒ 冷路径晚于 `init_types` 的串 ni 后移；语义不变、ELF canary 不变；判据按 TODO #26 = 结构性断言） | NOD 段 `tk` 槽**字节不变**；新增 `item` 槽（0 → 内容）；其余段逐字节不变（`.ccr` 四条记录值差 = **4B × 节点数**，实测 pa +5192B / gt +7452B） |
