...
1. **锚点/常量类改动：人工清单会漏、手抄会错 ⇒ 必须「机械枚举（grep）+ 编辑后 diff 复核」双保险**。
   - 本批实例 A：T0 手列 G7 载体漏了 `test_ccr_types.py:2022` ⇒ 由 grep 全量枚举抓到；
   - 本批实例 B：首次编辑**误截断** `CIR_CACHE_MAGIC`（`-4485090715960753727` → `-5090715960753727`）⇒ 当场发现 + 复原 + `jj diff` 复核确认 magic 逐字节未动。
   - 这条防的是**以后每一批**。
