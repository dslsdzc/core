...
**⇒ 口径要求**：登记/引用一律写「**未被 `.cr` 遮蔽的内建条目**」，**不得写「22 个 callee 零覆盖」**。
...
- **取 (乙)：明确登记「内建族参数面免检」这个契约。**
  理由（按本仓口径 = **契约比较**，非成本比较）：`bi_add` 只有返回型、**20/22 条的实参语义就是裸字/字节/地址/下标**，ABI 消费的就是裸寄存器值 ⇒ **「参数面免检」是有意设计的边界，不是缺陷**（有契约 = 有意设计）。
...
1. 落 `TODO` **`-5`**：内建回退覆盖洞（口径按 §4 改写为「**未被 `.cr` 遮蔽的内建条目**」；并把 §3 的「类型在返回面」写进机理叙述）。
2. 落 `TODO` **`-6`**：§7 的 `fiber_init` 名字碰撞（**必须写明「潜伏、今天不可达」**）。
3. 若日后要做 (乙) 的「契约登记」，本文件 §1/§2 可直接作证据面（**引用时保留 §6 的口径限定**）。
