...
**底层裁决**：TODO #56「指针写可选槽 × 表示位/规范化互斥：`*p = Some(9)` 双路径分歧静默错值」→ 裁决 **(A) 表示位随存储走**（(D) 语言面拒绝**未取用**）
...
**判据口径**（非缺陷）：探测暖态腿**跨二进制**日志文本差异 = frozen 侧 `lower to ccr…` vs 当前侧 FC 批硬闸早停 ⇒ 已入 `tools/baseline/REBUILD.md`「口径限制」段（与 TODO #65 同条，不另立）。
