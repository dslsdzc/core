...
> **⚠ 定性（team-lead 2026-09-23 要求在本节保持；防后来人读混）**：
> 本轮收缩 = **否决一个未来候选**，**不是移除既有能力**——两件事，不得混读。
> - 「7 名」**从未进入**块二的「最小语法增量」（§2.4 结论 1）⇒ 收缩**不增不减**该增量；
> - `flow` 的**既有面**（§2.4 义① region kind `SG_FLOW` · 义② 声明前缀 `FlowDecl` /
>   `flow fn name()`）**不在收缩范围，一字不动**；
> - 尤其：**`flow fn name(...) -> R { … }` 作声明是既有语法，不得因本条被删**（§2.4 义②）。
>
> 同理适用于未来同类收缩：**「不加新 X」永远不蕴含「删掉已有的 X」**——后者按铁律 #3 走（还原需明确许可）。
...
- 类型行：`src/compiler/ast.cr:291-309` 新增 `TYP_FN : int = 5`（**附确认 #2**：空位 5 经 A8 复核确实空闲）。
  ...
  - **不得**在 `init_types()` 里给 `TYP_FN` 预建规范行（含「表头占位行」形态）——**这是维护者
    2026-09-23 的裁定**（原文「按需建 + 同签名去重，不许在 `init_types()` 预建」），不是建议；
    理由、反面先例（`TY_DEX_S`）与判据见 **§8.1b**。
...
| **`init_types()` 里预建规范行** | **是**：**每一档** `.ccr` 都 +24B ⇒ **四条锁定值全变** | A10（占位先例）→ **已被裁定禁止**，见 **§8.1b** |
...
### 8.1b **约束（维护者裁定 2026-09-23）**：按需建 + 同签名去重，**不许**在 `init_types()` 预建

> **状态**：本条原为写手建议，**2026-09-23 经维护者裁定，升为约束**（不再是可选项）。
> 措辞按裁定原样落档：**「按需建 + 同签名去重，不许在 `init_types()` 预建」**。

**约束内容**：

1. `TYP_FN` 行**只在该签名第一次被用到时**建（`alloc_type`，`checker.cr:10`）；
2. 建前必须**先查**是否已有同签名行（**只查不建**），命中即复用——照 `TYP_NAMED` 的 `named_dedup`
   先例（`checker.cr:20-33`；该先例的注释原文即「alloc_type 仍是裸分配器，语义不变」）；
3. **禁止**在 `init_types()`（`checker.cr:231`）里为 `TYP_FN` 预建任何规范行 ——
   **包括**「表头占位行」这种形态。

**理由（裁定采纳的足迹分析）**：

- `init_types()` 是**每一档**编译的首行入口（`check_*` 家族）⇒ 在其中建 1 行 ⇒ **每一档** `.ccr` 的
  TYPE 段 `+24B`（段体结构见 8.1）；而 `.ccr` 四条锁定值覆盖的是 `ptr_arith` / `generics_test`
  两档语料（A38）⇒ **四条锁值会全部变化**，且变化与「本轮是否真的用到函数值」**无关**。
- 「按需建」的代价只是**首次用到该签名时**多一次查表 + 一次追加；不用则**零足迹**。

**反面先例（明确不照抄）**：`init_types()` **确实**有预建占位行的先例——
`alloc_type(TYP_BASE, TY_DEX_S, 0)`（`checker.cr:260`，设计注 `ast.cr:284-288`：该占位是为了让
`TI_DEX_S = 8` 哨兵永不与真实用户类型碰撞）。**本约束的要点正是**：那条占位的收益是「哨兵不碰撞」，
代价是全局行序位移，**当时接受了**；本轮 `TYP_FN` **没有**同类必需性（它不需要占用固定行号）
⇒ **不接受同一代价**。「有先例」不等于「可照抄」——先例要连**它的代价**一起读。

**判据**：

- 负钉（**必须**）：一档**完全不用函数值**的语料，其 `.ccr` 与本轮落地前**逐字节相同**
  （TYPE 段 `row_count` 不变）——这条是本约束的**直接可观测形式**；
- 正钉：同一档语料加上一处函数值用法 ⇒ TYPE 段恰好 `+24B × n`（`n` = 唯一签名数），
  且**同签名重复出现不追加**（去重生效）；
- 与 8.2 的两级契约合读：`.ccr` 若因正当理由变，仍按「归因 + D1 + 重锁 + 留痕」办。
...
- **预期**：若实现遵守 **§8.1b 的约束**（按需建 + 去重、不预建）且**两档锁定语料不含函数值用法**，则
