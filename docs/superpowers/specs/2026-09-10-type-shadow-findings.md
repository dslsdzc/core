...
**§15 之后不再有新影子读数**（通道已删，`--type-shadow` 不再是合法旗标、`g_shadow_*` 全局已不存在）⇒ **本文件冻结**：不再增「X 后复跑」节；本节 = 收官复核与替代网口径的唯一收口点。

**终态复核（T7 实跑）**：五 CI job 全 rc=0 · 全枚举 **58/58**（51 selfhost + 7 bootstrap）· `selftest-types` **404/404** · ELF canary `95084e7b…d475`（28822B）IDENTICAL · `corec2==corec3` `cmp` IDENTICAL（`5559beef…`）+ N06=0 + 冒烟 42 · **冻结基线同源对拍**（pre-P5 二进制 `5d2b15ad…` × 当前源 vs `f737e26b…` × 当前源；72 档 `check`）**rc + 日志零差异** · **行为探针全集 30 档零差异** · 突变控制（M1 派生面 / M2 判定面）逐条转红 + 精确回滚。统一台账 = 计划 `plans/2026-09-13-r2-p5-cleanup.md`「统一台账」节；P6 交接 = 同计划 **附录 E**。

**替代网口径（强制继承）**：判定面回归网 = ① 冻结基线同源对拍 ② 行为探针 ③ 突变控制 ④ 三态纪律（ICE04/D23：未知/不可译 ⇒ 拒绝，绝不静默当 0/1）。**任何后续批次不得引用本文件 §1–§15 的计数/摘要/站点直方图**（§15 已声明不可复现）。**载体面判据**（段/读回/TK/表示位）= 结构性断言 + corearch 读回对拍 + 定向探针（本通道对其零覆盖——历史声明继续有效）。
