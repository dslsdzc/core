...
> 定位:受众 = 开发者;状态 = active。答案以当前实现为准;标注"设计态"的条目尚未落地。
...
> 修订(2026-09-24):该问题**不再作为阻塞项**——`corec2` 链可跑通且逐字节稳定(`corec2 == corec3` IDENTICAL;由 `src/ci/run.sh` 的 `full-bootstrap` job 覆盖)。原文保留供追溯。

## 状态相关

**哪些功能还没实现?**
语言与编译器在活跃自举期。已知设计态/挂账:验证管线(规约证明)、平台桥(IO/时钟语义接口落地)、惰性求值、分布式、部署配置完整消费、.ccr 内容随 v9 段表推进(版本以 `src/compiler/ccr_io.cr` 的 `CCR_VERSION` 为准)。最新状态看仓库 TODO.md 与各文档头部"状态"。
