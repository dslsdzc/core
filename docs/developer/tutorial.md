...
**图模型**：规约约束挂载在图上(设计态)——性质与代码同源,验证器直接消费同一张图。
...
| **并发服务**（服务器/网络后端） | ③④ | 动态图 | 多核/OS 线程 | 已实现(基础并发:go/chan) |
...
**关联**：`docs/maintainer/design/execution-model.md` §五(部署配置与时间)、`docs/maintainer/design/memory-model.md`
