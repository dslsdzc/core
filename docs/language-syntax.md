...
默认使用文件名（不含 `.core` 扩展名）作为标识符，可在文件头部手动覆盖：

```core
// file: lib/helper.core
fileid "my_helper"
...
每个目录下可放置 `_import.core` 文件，其中的导入会自动应用于该目录及所有子目录的 `.core` 文件（除非子目录有自己的 `_import.core` 覆盖）。
...
· `_import.core` 本身不生成代码，只起导入声明作用。子目录的 `_import.core` 会继承父目录的导入（合并，冲突时报错）。
