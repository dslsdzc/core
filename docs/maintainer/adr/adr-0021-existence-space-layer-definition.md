...
- **`.ccr` 承载未定,须分层表述(2026-09-20 独立复核后收窄——原写「本次修订不需要加段」,该结论不成立,已撤)**:
  - **证实的**:`ENT.flags` 是 u32,`bit4–31` 空余 28 位,**值现在恒 0**(零实例)——见 §关联 materialization-space.md §7.1/§7.4。
  - **位域路线只对四个小枚举字段成立**(`recipe` / `persistence` / `replicability` / `authority`;值域有界)。
  - **`location` 不可打包**:它是**内存域索引**(资源域代数 = 实例侧声明、**无界**),位打包 = 给内存域数量设**硬上限**——正是本仓一直在退役的 `MAX_*` 模式。⇒ 须**另走承载**。
