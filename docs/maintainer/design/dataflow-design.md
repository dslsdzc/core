...
- **存储侧对应（2026-08-27 v4）**：本节不可重算标注（FIXPT / 神谕 / BSS / 模糊融合）= **图内无配方条目**——统一规则见 `docs/design/cache-semantics.md` 条款 4b（必须有 home、驱逐必写回、身份 = 图节点；不按范式枚举）；映射实例侧（寄存器分配 = 缓存语义映射实例）见 `docs/design/regalloc-cache-mapping.md` §三
