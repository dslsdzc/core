...
>
> **核验基线 = `develop@origin` @ `0eb1efd3`**（本会话 `jj git fetch` 后实读：**基线未动**，与 v1 同一修订；
> 故一切 `file:line` **以该修订为准，非手边检出**）。本次另对**最承重的 16 组锚点做了抽样复核**，逐条见 §八.0。
>
...
>   **本文与它的分工见 §十一.3**（本文管**语义面**，它管**承载面**）。⚠ 其标题仍写 v1 名「Open Relational Lattice」（见 §〇.0 命名说明）。
...
| 增长方式 | 靠**规则**生成无限闭包（「有限规则可以生成无限结构」） | 靠**开放的关系域**不封顶（「开放 relation universe 本身已经足够不封顶」） |
...
| S-1d | CCR-4 **Finite Presentation, Unbounded Denotation** | ⚠ **降级为「开放 universe」，「无界指称」失去承载机制** | 「有限呈现」仍在（`.ccr` 是有限文件）；但 v1 的**无界指称**靠 #2（自由项代数）+ #6（规则生成闭包）承载，**两者都已砍** ⇒ v2 下单个 `.ccr` 所指称的更像**有限**结构。v2 保留的是另一件事：「**开放 relation universe 本身已经足够不封顶**」= **可扩展性**，不是**单文件指称无界** | ⚠ **两个不同的「无界」被 v1 混在一条公理里**，v2 只留后者 ⇒ **是否仍主张前者，见 §十 待裁 新-9** |
...
| **S-2** | **`.ccr ≠ 𝓛`** / 「有限 presentation，指称可无限」（v1 §一，v1 自称「全部设计的支点」） | **改写** | 见 S-1d：支点的**后半**（指称无界）失去机制；**前半**（`.ccr` ≠ 它所指的关系总和）**仍然成立且更重要**——因为关系由 analyzer/checker 产生，「盘上写的」与「成立的关系」在 v2 下**天然不等**（`reachabilitySource` 句柄即是显例） | 「`.ccr` ≠ 关系总和」→ §六；原记号 `𝓛(G, CCR)` **退出 Core 记号**（§〇.5） |
...
| **S-11** | **验收标准 §31 + 三个反例测试**（v1 §二十一：新 domain 入场零修改；无限递归 / 非格 / 矛盾三例「都必须能表示」） | **须重算**（v2 **未给**验收标准） | v2 砍掉 Rule/Constraint 后，三个反例的**对象**发生了根本变化——见 §七.2（其中 #1 从「必须能表示」变成「**不再需要表示**」） | §七（全部 `[提案]`） |
...
- ⚠ **`checker = ...` 一行的地位（本文登记，见 §十 待裁 新-10）**：这一行**是工具钩子还是语义声明**，v2 未裁决。
  它恰好是本版**最容易把 Core 重新拉回逻辑语言**的一处（若 `checker` 变成可求值声明，就等于恢复了 Constraint）。
  **本文按「工具钩子」读**，并标为**本文展开、非原话**。
...
## 七、验收与反例（**v2 未给验收标准 ⇒ 本节全部由本文重算，全部 `[提案]`**）

> ⚠ **先声明性质**：v1 有明确的验收节（§31）与三个反例测试；**v2 通篇没有验收节**。
> 故本节**不是**对 v2 的转写，而是**本文按 v2 的四组件重算出来的**。
> 凡与 v1 §二十一 不同的地方，**逐处标出差异与依据**。**全部 `[提案]`，当前无实现可跑。**
...
### 7.2 v1 的三个反例在 v2 下的重算（**本节是全档最需要「不许静默覆盖」的地方**）

> v1 §二十一 21.2 的三个反例（无限递归关系 · 非格关系 · 相互矛盾的分析事实）来自**v1 的失败判据**：
> 「三者都必须能表示。**如果其中任何一个要求 Core 加特殊 case，说明规则还是太强。**」
> **v2 砍掉 Rule/Constraint 后，三例的对象全部改变**——逐例给处置。**本节全部是「本文展开」。**

| v1 反例 | v2 下的处置 | 依据（原文 + 推导） |
|---|---|---|
| **① 无限递归关系** | ⚠ **从「必须能表示」变成「不再需要表示」** | v2 原文给了替代路径：`ReachabilityAnalyzer ↓ CCR: reachable(A,B); reachable(A,C); …`，或「如果不想展开巨大 closure，则：`CCR: reachabilitySource = analyzer/result handle`」。**规则没了 ⇒ CCR 里不再有「递归关系」这个对象**；无界闭包是 analyzer 的事，CCR 只收结果或句柄 |
| **② 非格关系** | **从「必须能表示」变成「平凡满足」** | v2 原文：「甚至以后如果发现某一 relation **根本不是 lattice**，完全没事。」Core 不要求格 ⇒ **没有可失败的对象**。v1 §二十一 21.2 里那条「加强版钉子」（对无 `join`/`meet` 声明的 relation 查 join/meet ⇒ 「未声明」）**在 v2 下仍有效且更简单**（§三 3.4 判据 1/2） |
| **③ 相互矛盾的分析事实** | **从「必须能表示」变成「就是两条记录」** | v2 原文：「CCR：**两个关系都存在。** 结束。」⇒ 保留下来的判据只有**爆炸探测**（§四.4 判据 2），四值面（v1 的 `unknown/positive/negative/conflict` 可分辨）**随之作废** |

> ⚠ **本条处置的强度声明**：上面三行的**方向**（能表示 → 不需要表示 / 平凡满足）由 v2 的机制删除**逻辑蕴含**；
> 但「**是否仍有必要保留这三个测试**」**是裁决问题**，不是本文能定的 ⇒ 登记为 §十 待裁 新-11。
> **本文不删 v1 的三个反例测试**，只标其**对象已变**（v1 测试文本已随 v1 修订存留，见 §〇.2）。

### 7.3 新增反例 4：一个拼写差异**不得**造出两种 relation（**本文展开——直接源于 v2 的 RelationDomain 动机**）
...
- **为什么这条值得单列（本文展开）**：它是 v2 **唯一**由「工程原因」驱动的 Core 组件（#5）的**存在理由**；
  若这条测试红了，RelationDomain 就没有存在的必要，v2 的「四件事」会退化成「三件事」。

---

## 八、现状锚点实核（`develop@origin` @ `0eb1efd3`）
...
> **v1 有 C-1…C-9；本版新增 C-10，并把 C-5 标为「消解」**（逐条给理由）。
...
|---|---|---|---|---|
| 1 | 「改进版后缀」的编码形态 | **保留** | v2 未复议 v1 的版本裁决（C-1）⇒ 该问题原样存在 | 决定 `CCR_MAGIC`/`CCR_VERSION` 的**类型** → 触发 `canary_values.tsv` 重锁 |
...
| **新-9** | **「无界指称」在 v2 下是否仍成立/是否仍需要** | 本文派生（§二 S-1d） | v1 的 CCR-4 把**两件不同的事**并成一条：① 可扩展性（新增 domain 不封顶）② 单文件指称无界。承载 ② 的机制（自由项代数 #2 + 规则闭包 #6）**已砍**；v2 只正面重述了 ① | 决定是否仍要设「指称可无界」的判据；影响与 v1 验收的接续性 |
| **新-10** | **`checker = ...` 是工具钩子还是语义声明** | 本文派生（§五 5.2） | v2 的 Theory 示例含 `checker = ...` 一行；若它是**可求值声明**，就等于把 Constraint 从 #7 那条路放回来（**本版最大的回退风险点**） | 决定 Theory 降级是否彻底；决定 §七 验收是否要加一条「checker 不得被 Core 求值」 |
| **新-11** | **v1 的三个反例测试是否仍保留为验收项** | 本文派生（§七.2） | v2 的机制删除**逻辑蕴含**了三例对象改变（①不需要表示 / ②平凡满足 / ③就是两条记录），但「**是否仍有必要保留这三个测试**」是裁决问题 | 决定 §七 的终极验收形状 |
...
> ⚠ **两表合计读作「至少 21 项」**（v1 **保留 6**〔#1/#3/#4/#5/#8/#10〕· **收窄保留 1**〔#2〕· **作废 3**〔#6/#7/#9，**其中 #9 有后继项**〕= **10**；**新增 11**〔新-1…新-11〕⇒ `10 + 11 = 21`；逐行已机械重数 ✓）。
...
### 11.3 与其他文档/产物的联动登记（**v2 新增两行**）

| 文档 | 联动点 | 性质 |
|---|---|---|
| **`docs/superpowers/specs/2026-09-22-ccr-v8-blast-radius.md`**（**另一位写手；`develop@origin` 上尚无**，按名引用） | **分工**：它管**承载面**（谁产谁读谁搬运 · 段级消费者矩阵 · 后端函数级清单 · 判据面作废清单 · 假定段表的间接依赖），本文管**语义面**（四组件 + 删除登记 + 冲突/待裁/未核实） | **互补，不重复**。⚠ **两处必须对齐的接缝**：① **版本/身份标识**（本文 C-1 = 它的 §5 对象之一）② **段表去留**（本文 §十 待裁 8/10 = 它的 §5 主题）③ **判据面作废范围**（本文 C-8 = 它的 §4）。⚠ **它的标题仍用 v1 名**（§〇.0 已登记）；⚠ 本文**只读了它的标题/节标题与若干行**，未逐条对账 ⇒ §十四 U-16 |
| `docs/maintainer/design/materialization-space.md` | §7.4 已立「承载方案与 `CCR_VERSION` 一并单独立项」，且实核 `CCR_VERSION 现 = 9`（`:786`）；`:846` 记「tag 9+ 两个具名主张者」 | **必须对齐**：V8 的版本策略（C-1）与承载面决策（§十 8/10）不得与该立项各自为政 |
| `docs/maintainer/design/execution-mapping-design.md` | 其「Mapping 一等层」 vs V8「Mapping 非本体」 | **直接对立**（C-4），待裁 4 |
| `docs/academic/cache-semantics.md`（条款 1–7） | `existence-structure.md` 的条款权威；v1 §十六 16.1 曾把 Cache 降为普通 theory，**该节本版已删**（§二 S-8） | ⚠ **本版变更**：v1 说「条款 1–7 的适用范围收窄为 `theory cache`（**不废除**）」；**v2 没有 `theory cache` 这个 Core 对象了** ⇒ 该收窄表述**失去对象**。**建议改为**：条款 1–7 的适用范围 = 「某个 cache 相关的 relation domain + 其 checker」。**登记，不改**；⚠ 这条**属本文展开**，须维护者确认 |
| `src/compiler/ccr_io.cr` 头注释 | C-6 的 9 处陈标（含 2 处直指版本闸）+ C-10 的 metadata 撞车 | V8 落地批的**必做前置**（否则读侧真源被陈标遮蔽）；**两条应同批处置** |
...
| 〔已设计未实现〕 | **4** | 无成表清单（散落 4 处，见下） | B 档不适用——本档主张不集中在单一清单 |
| 〔提案〕 | **19** | 其余全部（**默认档**） | A 只数**显式写出**的标记；**未含**默认推断档 |
...
**〔已设计未实现〕的分布**（无成表清单，逐处给，**合计 4 处**）：§〇.4 术语护栏表 **1 处**（`存在格`——同行的「能力格」**无独立标记**）· §九 C-4 **1 处**（`execution-mapping-design.md`）· §十一.1 表 **1 处**（`existence-structure.md`）· §〇.3 定义表 **1 处**（**方法论自述，非主张**）。

> ⚠ **时点**：以上读数为**本文件当前修订**的读数。本文若后续增删章节，**须重跑上列命令并同批改本表**
> （否则本表立刻变成陈旧锚点——本仓已有此类事故，见 §九 C-6/C-7/C-9）。

**实测读数（本会话 2026-09-22，`f` = 本文件）**：

```
grep -o '\[已实现\]'        $f | wc -l    # → 9
grep -o '\[已设计未实现\]'  $f | wc -l    # → 4
grep -o '\[提案\]'          $f | wc -l    # → 19
```
...
| 9 | 待裁重算（v1 十项处置 + 新增 ≥7） | §十 | ✅ v1 10 项（**保留 6 / 收窄保留 1 / 作废 3**【其中 1 项有后继】）+ **新增 11 项**（任务点名 7 + 本文派生 4）⇒ 合计 **21 项**，逐行机械重数 ✓ |
...
| 14 | 未核实项照旧列清单（v2 下作废的划掉、仍有效的保留） | §十四 | ✅ v1 **十一项**逐个处置（**保留 7**〔U-1/2/3/8/9/10/11〕· 转化保留 1〔U-6〕· 部分作废 1〔U-4〕· 降级移出本档 1〔U-5〕· **作废 1**〔U-7〕= 11）+ **新增 6 项**（U-12…U-17）⇒ 合计 **17 项**，逐行机械重数 ✓ |
| 15 | 不重写历史 / 文件名不动 / 标题变更入正文 | §〇.0 + §〇.2 | ✅ v1 存留父修订 + 读取方式给内容特征 |
| 16 | 不动其他文件 | 全文 | ✅ 只改了本档一个文件（`jj diff` 两段式核过，见交付说明） |
...
| 档 | A（实测） | 构成 |
|---|---|---|
| 〔已实现〕 | **9** | **4 处方法论自述**（前言「全文除显式标注…」句 · §〇.3 三态表的表头行 · §〇.3 后「本设计的特殊性」说明 · §八 节首「本节全部…」说明）+ **5 处真主张**（§〇.4 术语护栏的 `metadata` 行 · §三 3.1 现状对照 · §三 3.3 现状对照 · §十一.1 表 `2026-09-09-lattice-ir-v7-format.md` 行 · §十一.1 表 `adr-0002` 行） |
| 〔已设计未实现〕 | **4** | 1 处方法论自述（§〇.3 表头行）+ 3 处真主张（§〇.4 术语护栏的 `格/Lattice` 行 · §九 C-4 · §十一.1 表 `existence-structure.md` 行） |
| 〔提案〕 | **19** | 3 处口径自述（§〇.3 表头行 · 「默认必须是…」句 · 「本设计的特殊性」说明）+ 16 处正文/自查/自我约束标记 |
...
| v1 # | 未核实项 | v2 处置 |
|---|---|---|
| U-1 | 「版本整数 = 8」是否意为从现行 9 回退 | **保留**（v2 未复议该裁决；原话歧义仍在） |
| U-2 | 「改进版加后缀」的后缀具体形态 | **保留**（= §十 待裁 1） |
...
| # | 未核实项 | 为什么没核 | 谁需要它 |
|---|---|---|---|
| **U-12** | **v2 是否撤销 v1 的两条裁决**（版本整数 = 8 + 后缀 / corearch 改为查询消费者） | **v2 通篇未提版本号与后端**。本文按「**未撤销**」处理（§九 C-1/C-2），但**这是本文的展开，非原话** | 裁决人；V8 落地批 |
