...
| ~~**U-18**~~ | ~~`.coi` 设计档与本文的相容性~~ | ✅ **已核（2026-09-23）**——已做**逐项对账**，**见 §十一.4**（该节含**读数范围**与「**不声称全档穷尽**」声明，是这张表能被信的前提）。结果 = **12 项：🔴 第一类命中 3 · 🟠 接缝 4 · ✅ 对齐 5**。⚠ 三条命中**待维护者裁**（命中 1 = `.ccr` 是否只装 target-independent · 命中 2 = 类型/接口面归属 · 命中 3 = 该档引的是本档**已撤回**的建议）⇒ **本档不裁决、不预改**，等裁定同批落。另：§十一.4.2 已**接收**两条反向约束（C-11a / C-11b） | 维护者（裁定）；两档作者 |
