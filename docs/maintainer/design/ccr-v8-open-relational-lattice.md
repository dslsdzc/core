...
- **三分流（裁-5 第 2 / 3 条，原文逐字）**：
  ...
  > ⚠ **第 3 条的档名待对齐（本文登记，不裁决）**：裁定写「**Target Model**」；
  > 而 `.coi` 档把**同类内容**记作 §5.3 的「硬件能力描述 → HIT 表 / hw-map / `CapabilityDomain`」（`:581`）
  > 与「**canonical target model** → 同上」（`:582`）。
  > ⇒ **裁定的「Target Model」疑似即该档的「canonical target model」**（两处描述同向），
  > 但**本文标为疑似**，**由维护者确认后再统一档名**。
...
> ⚠ **仍待确认的只剩两条，且都是「落点内新发现的接口问题」**（**不是**原三条命中）：
> ① `.coi` 的「Target Model」档名对齐（裁-5 第 3 条 vs 该档 §5.3）。
> ② `locatedAt(x, r12)` 与「禁止 target-specific」的关系（(甲)/(乙) 两读）——见 §五 5.1 的 🔴 登记。
...
| ~~**U-18**~~ | ~~`.coi` 设计档与本文的相容性~~ | ✅ **已核（2026-09-23）**——已做**逐项对账**，**见 §十一.4**（该节含**读数范围**与「**不声称全档穷尽**」声明，是这张表能被信的前提）。结果 = **12 项：🔴 第一类命中 3 · 🟠 接缝 4 · ✅ 对齐 5**。**三条命中均已收口**：**命中 1 / 2 由裁-5 裁定**（落点 = §五 5.1 · §三 3.3）· **命中 3 由本批第二半的 `.coi` C-10 引用同步处置**。另：§十一.4.2 已**接收**两条反向约束（C-11a / C-11b）。⚠ **余下两条待确认项见 §11.4 计数块末**（裁-5「Target Model」档名对齐 · `locatedAt(x,r12)` 的 (甲)/(乙) 读法） | 维护者（那两条待确认项）；两档作者 |
