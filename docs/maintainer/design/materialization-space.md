...
维护者随后把范围**收窄**为:**只改「本体」那个词 + 层内定义**;`格` 保留作简称
(既有链名「语义 → 图 → **格** → 编码」不动,这是**术语具体化**不是层结构变更);
**凡只是用了「格」字、或方向本来就跟新定义一致的,降级为「不动」**。

据此复核我的首轮改动,发现 **5 处过改**(Class-D「X = 缓存语义映射实例」4 处 + `region-model.md:4`),
**已回退**:

| 回退处 | 理由 |
|---|---|
| `memory-model.md` §四 关联 bullet | Class-D:该类陈述已是「实例」框架,方向一致 ⇒ 不动 |
| `regalloc-cache-mapping.md:1` 标题 + `:4` 定位行 | 同上 |
| `dataflow-design.md:235` 「寄存器分配 = …」 | 同上 |
| `glossary.md` §五 标题 | 同上 |
| `region-model.md:4` 定位行 | 维护者判定 `:3-5` **方向已对、可作改写范本** ⇒ 不动 |

**一致性回归**:回退后全仓 Class-D 陈述口径统一为「缓存语义映射实例」
(与原本未动的 `project-book.md:111`、`TODO.md:2161/2313` 一致)。
`regalloc-cache-mapping.md` 保留的改动 = `:29` 三层图「cache-semantics 本体」→「存在格本体」(**本体词**)
+ §二「公理 = 缓存七条」→「条款 1–7」(**本体词**)+ **§三.1 新增显式约束**(4b 义务面,维护者点名要求)
+ §四 规则③ 按 recipe 分档(同上);`region-model.md:102/:156` 保留(**本体词**改动)。
...
| **K** | **`TODO.md:2137`**「v4 定稿:…层规则零签名 = **缓存七条**…」 | ⏳ **待裁**。与 `adr-0005:13` **同类的历史 v4 公式**(非权威宣告);TODO.md 属活档且**非 ADR**,不受「不改已 accepted ADR」保护,但收窄口径下亦不必改。**未动**,报维护者定 |
