# 存在格 / Materialization Space:层定义修订提案

> 定位:受众 = 维护者;状态 = **提案(proposal)**——**非定稿、非现行有效**。
> 设计意图真源 = 维护者原话逐字稿(`/tmp/briefs/materialization-space-raw.md`;**本文件不得改写它**,本文件只是它的工程化展开)。
> 本文主张修订 `docs/academic/cache-semantics.md`、`docs/maintainer/design/memory-model.md`、`docs/project-book.md` 等处的现行措辞。
> **合入前旧文档仍是现行有效**(效力边界见 §零)。本文件不替维护者裁任何事——§九 列出全部待裁事项)。
...
## 零、双份权威窗口管理(本文档的效力边界,不得删改)

本文档主张修订 `docs/academic/cache-semantics.md`、`docs/maintainer/design/memory-model.md`、
`docs/maintainer/design/existence-structure.md`、`docs/project-book.md`、`docs/z-vision.md`、
`docs/glossary.md`、`docs/maintainer/adr/adr-0005-memory-model-layers-v4.md`、
`docs/maintainer/adr/adr-0006-cache-semantics.md` 等处的现行措辞。

**但在维护者明示合入之前,上述旧文档仍是现行有效。**

- 二者不一致处,**一律以旧文档为准**;
- 本文档的效力**仅限「登记修订请求」**——它不是第二个权威,不构成对旧文档的取代;
- 本文件**未改动任何既有文件的正文**(只新增本文件一个);
- 合入方式(就地改 / 加修订注 / 本文档转正)由维护者裁(§九 A)。

> ⚠ 本节存在的理由:本仓已多次出现「同一层有两个都自称权威的文档」的事故面。
> 一份「主张修订权威文档」的文件若不自限效力,它本身就是在仓库里造第二个权威。
> 在本文件被合入(或转正)之前,任何读者遇到分歧**必须以旧文档为准**。
...
现行教义把**缓存语义**当作格层的**本体定义**,不是当作一类实例。三处定义性表述:
...
- **本账的可靠性边界**:§6.1 与 §6.2 中标注「✓实核」的行由我**逐字复核**过 line 与摘句;
  其余行按同一 grep 口径登记,**未逐行复核**——若要据本账动手,请先复核目标行(§八)。

### 6.1 甲类:必改(把缓存当**本体定义**说的)

这些位置删除「本体/定义」措辞后语义**不变**,因此改动是低风险的措辞收口。
...
> 本文档只登记,不裁定。以下各项均需维护者明示。

| # | 事项 | 本文档的倾向(仅供参考) |
|---|---|---|
| **A** | **合入方式**:就地改旧文档 / 加修订注 / 本文档转正?若转正,新层定义的**权威文档是谁**——本文档,还是修订后的 `cache-semantics.md`? | 倾向「新写一份 `existence-space.md` 作层定义 + `cache-semantics.md` 降为该层的一类映射条款」;但**权威归属是治理决定**,不属本文档 |
| **B** | 修订账三条分组(必改/加注/不动)是否照准 | 仅供裁 |
| **C** | **`location` 字段是否入格层**:现行实现**故意**把 `home` 恒 -1(放置决策 = mapper);新定义若要求格层陈述 `location`,则与现行划界冲突 | 倾向**不入**:`location` 属 mapper 注记,格层只承认「存在多份 materialization」这一事实。若照原文列为字段,须重新划界 |
| **D** | **`replicability` 与两份活跃 spec 的对应**:`{COPY,MOVE,SHARE}`(2026-09-04)与 `{MOVE,LOAN,SPLIT,FILL,COPY}`(2026-09-17)未收敛;`replicability` 字段是否取代它们? | 倾向**不取代**:字段是**格层陈述**,标签是**跨边界决策**;须出对齐表(该批已自行挂账) |
| **E** | **`version` 是存储字段还是派生字段**:现实现为落盘期派生(内存表无此字段) | 倾向**派生**:由图坐标唯一决定 ⇒ 依 §5.2 修正判据属格层,但不必占存储 |
| **F** | **格式施工面**:七个字段不须新段(§7.4),但真实施工要动 `CCR_VERSION`(现 9)+ 重锁 `.ccr`/ELF 判据网 | **建议单独立项**,不并入文档修订批 |
| **G** | **`recipe` 的施工**:§7.1 已证「位就位、语义未实现」;是否本批一并实现生产者与消费者? | 倾向**分开**:文档修订与语义实现是两个批次,混批会让「文档变了但行为没变」不可判别 |
| **H** | **术语冲突**:`物化` 一词已被 `so_materialize`(模块系统符号物化)占用 | 须择一改名或加限定语 |
| **I** | **本文档的受众/状态行**:现写「受众 = 维护者;状态 = 提案」。若转正,状态行须改 | 仅供裁 |
...
| 既有定稿 | 新定义下的地位 | 依据 |
|---|---|---|
| 寄存器分配 = 缓存语义映射实例 | **更自然**:从「映射实例」升为「一类映射实例」 | `docs/maintainer/design/regalloc-cache-mapping.md` |
