...
> ⚠ **本节原写「七个字段不需要新段」,该结论不成立,已撤**(见下)。撤稿理由:
> 我把「七字段概念上能对上现有命名槽」错读成了「有空闲 4B 槽可放新字段」,并据此下了
> 「不需要加段」这个**我未在 REG/MAP 侧核过**的结论。
...
| `replicability` | 3 值 | ✅ |
| `authority` | 3 值 | ✅ |
| **`location`** | **内存域索引(无界)** | ❌ **不可打包** |

> **打包的前提是值域有界**。`location` 是**内存域索引**——资源域代数 =**实例侧声明、无界**
> (`ent_kernel.cr` 头注:位置登记表 `{entry→loc}` 为**不透明**位置)。位打包 = 给内存域数量
> 设**硬上限**,正是本仓一直在退役的 `MAX_*` 模式(如 `MAX_FN_PARAMS` 那类)。
> ⇒ `location` **须另走承载**,不得进 flag。

**2) ENT 没有空闲 4B 槽(原「其余各有 4B 槽」不成立——实核)**

| 表 | 尺寸 | 槽位 | 空闲 |
|---|---|---|---|
| ENT 内存 | `ESZ_ENTRY = 24` = 6×4B | `var_id` `def` `live_start` `live_end` `home` `flags` —— **六槽全命名** | **0** |
| ENT 盘面 | `ESZ_ENTRY_DISK = 28` = 7×4B | 上列 + `version` —— **七槽全命名** | **0** |

⇒ 新增**独立 4B 字段**无处可放;`identity`/`version`/`location` 只是**概念上分别对应**
`var_id` / `version` / `home` 这三个**已占**槽,不是「有空位」。

**3) `MAP` 归 REG 段,而 REG 记录全满 ⇒ 要 bump `CCR_VERSION`**

`MAP` 是 **region 侧**承载,因此落在 **REG 段**;而 REG 两态皆满:
`ESZ_SG_DISK = 24`(6×i32:kind/parent/enter_nod/exit_nod/first_ent/last_ent)、
...
- **「不需要加段 / 不需要 bump `CCR_VERSION`」不得作为结论写入**;
- 本次修订**只动文档、不动 `.ccr`**——承载方案与 `CCR_VERSION` **一并单独立项**(§九 F);
- 真实施工时的**已知必经点**:`location` 另寻承载 · ENT 无空槽 · REG 全满(若涉 MAP) ·
  `CCR_VERSION` 现 `= 9`(`src/compiler/ccr_io.cr:135`,load 严格等值、旧版整类拒收——D10) ·
  同批须重锁 `.ccr`/ELF 判据网。
...
**一条共有的教训(2026-09-20,双方记录)**:本文件与 ADR-0021 早先那句
...
| **F** | **格式施工面**:七个字段的 `.ccr` 承载方案未定(§7.4 **已重写**——`location` 不可打包 / ENT 无空槽 / REG 全满);真实施工要动 `CCR_VERSION`(现 9)+ 重锁 `.ccr`/ELF 判据网 | ⏳ **待裁**。**建议单独立项**。⚠ **原写「本修订不需要加段」已撤**(该结论经独立复核不成立,见 §7.4 撤稿说明)。另注(lead 转来,**未经我复核**):`.ccr` 段位 tag 9+ **已有两个具名主张者**(驱逐标注段 / 证书段),且 **`MAP` 归 REG 段而 REG 全满**——加段/改布局前须先与之对齐,不得各加各的 |
