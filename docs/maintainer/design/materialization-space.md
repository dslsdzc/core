...
| **I** | **`adr-0005:13`「层规则 = 缓存语义七条,永不新建」的框定词** | ⏳ **待裁**。该行是 accepted ADR 正文,**按 `adr/README.md` 规则不得就地改**;它属「层定义条款」而非权威宣告。三条出路供选:①再起一份 ADR 记录该行的框定收窄;②在 adr-0005 加状态注(**须维护者明示**,规则默认禁止);③不动(条款数确实不变——本修订未增删任何条款)。**本批未改** |
