...
> 设计意图出处 = **两份**维护者 2026-09-20 口述输入（**本文档均不改写它们**）：
> - `/tmp/briefs/execution-mapping-raw.md` —— 主输入（30 节 + 能力格 + M0–M11）；本文 = 其**形式化展开 + 仓库锚定 + 不变量收紧 + 冲突登记**。
> - `/tmp/briefs/materialization-space-raw.md` —— **对已定稿教义的修订**（「格 = 缓存」→「格 = Materialization / Existence Space，缓存是其中一类映射实例」）。本文 §三/§六/§八 已按其重述。
>   ⚠ **时态更正（2026-09-20 复核后）**：原写「**既有「定稿」文档一律不改**，只登记」——**已不准确**。
>   现况：**那 7 处正由另一名写手就地修订**（维护者裁「同批就地改」）；**本文（本文件）仍全程未改它们**（§十一 C-11 已同步收窄）。
>   凡读 C-11 的表，须按「**修订前的原快照**」读，**不是现状**。
...
>   ✅ **该文件已于 2026-09-20 落盘**（此前本文按预定名引用）。**两侧已做过逐节对拍（11 节：8 重叠 / 3 不重叠）**，重叠处的**主从已逐条建立**：
>   | 命题 | **主副本（权威侧）** | 本文侧处置 |
>   |---|---|---|
>   | 格层四问 | **§2.1**（含两套措辞的权威标注） | §〇.2 / §6.7 / §八⑨ 引用，**不复述** |
>   | 七字段取值域 | **§2.3 字段级模型(七字段)** | §3.4.2 逐字引，只留实例取值列 |
>   | 驱逐不变量的**式** | **`cache-semantics.md` 条款 2′**（权威侧 §4.2 亦标同主副本） | §6.4 / §八⑧ 引用 |
>   | 式的**判据形状 / fail-closed / 显式约束** | **§4.3 / §4.4 / §4.5** | §八⑧ 的**判据**与之互补 |
>   | **逐字段实现距离审计** | **§7.0–§7.4** | §3.4.5 删去重复表，只留执行映射侧两条结论 |
>   | cache policy 划界 | **§五（§5.1 明写 + §5.2 可检验判据）** | §〇.2 / §6.7 引用，只留「也不属执行域层」那句 |
>   | 与既有定稿的**兼容性声明** | **§10** | 本文引用（本文侧不另立） |
>   | 三节**不重叠**：§4.3 断言形状 · §7.4 `.ccr` 分层结论 · §10 兼容性 | 权威侧 | 本文 **§3.4.5 / §9.2 / C-3** 与之**互证**（§7.4 独立支持本文"REG 全满"复核） |
>   ⚠ **两侧若仍不一致，一律以 `materialization-space.md` 为准。**
...
本文档与既有文档存在**四处同名不同义**，开工前必须定死，否则后续所有讨论都会串线：
...
| **缓存 / cache** | ⚠ **本批降级**：「缓存」= **存在格在经典机器上的一类映射规则**（"经典可再生值的一种映射规律"），**不是层本体** | ~~既有**定稿**措辞~~ ⇒ **修订前的措辞（过去式，2026-09-20 复核后改时态）**：`project-book.md:216`「存储语义**本体**为**缓存语义**」、`z-vision.md:15`「存储半边已定稿（语义本体 = 缓存语义）」——**该两处已被就地修订**（不再以现在时引用）；`cache-semantics.md:4`「本文件是 Core 存储语义**本体**的**唯一权威**」亦在修订面内 | **本文（本文件）未改它们**；改动由另一名写手就地执行（§十一 C-11 原快照 + 已裁）。本文**不再写**"缓存 = 存储语义本体"；凡须指既有条款处写「**缓存语义条款**（映射实例规律）」并给出处 |

### 〇.2 层归属：格层不承担 policy（本批新增的硬边界）

> ⚠ **主从声明（2026-09-20 二次复核后补）**：本条**与权威侧同命题**——
> **主副本 = `materialization-space.md` §五（划界:cache policy 不属于格层）**，其 §5.1 明写归属、**§5.2 给可检验的划界判据**
> （「格层描述里出现『何时/多久/顺序』这类词 = 格层管了该归 mapper 的事」）。
> **本文不复述权威的判据**（避免第三份拷贝）；**本节只保留本文特有的那一句**：
> 「policy **也不属本文的执行域层**」——因为本文（执行映射层）**自己也声明不承担 policy**，与格层同侧，故须双向写明。
> **取数/取判据一律以 §五 为准。**

新输入明确划出的一条界，**本文全程遵守**（四问编号与权威 **§2.1「格层回答的四问(原文逐条)」** 一致——**该节是四问的主副本**）：
...
⇒ **可检验判据（本文采用权威 §5.2 的，并落成本文侧的一条判据）**：本文 §八⑨ 判据 2「策略词零命中」**须与权威同一判法**——格层面不得出现「何时/多久/顺序」类词；**该条判据怎么判以权威 §5.2 为准**。
...
Source
  ↓
Semantic HDFG                    ← 语义的定义处；不知道 CPU/GPU
  ↓
...
#### 3.4.2 七字段（**本表 = 执行映射侧视图**；**取值域权威 = `materialization-space.md` §2.3**）

> ⚠ **主从声明（2026-09-20 复核后收窄 —— 本条是"同一命题两处说法"的第一类命中，已按主从统一）**
> 七字段的**取值域（枚举成员）**由**层定义侧**给出，**权威 = `docs/maintainer/design/materialization-space.md` §2.3**
> （表题「**字段级模型(七字段)**」（**该表题本轮已由权威侧改名**，原为「字段级定稿」——改名是为消「定稿」与「本表整体 = [提案]」两说），四列 = 语义 · 取值域 · 谁写 · 谁读，另附两条蕴含关系）。
> **本表不另立取值域**：`取值域` 列**逐字引**那份；本文**独有**的内容 = 最右两列（**两个映射实例的取值**，取自本文输入）
> ＋「本文三态」列。**冲突时以 `materialization-space.md` 为准。**
> ⚠ **修正留痕**：本表**原写**了一份自立的取值域（`recipe` = `0/1`、`authority` = 可转移/可重建、`persistence` = 否/`external`、
> `replicability` = `yes`/`no/limited`），**与权威 4/7 行不等**——那正是本条要消灭的"两处各自给一份、互不指向"。
> **已按主从统一**（见 §十二 未核实项 15 的状态变更）。

| # | 字段 | 取值域（**逐字引** `materialization-space.md` §2.3） | 含义 | 缓存实例取值 | MMIO 实例取值 | 本文三态 |
|---|---|---|---|---|---|---|
| 1 | `recipe` | `{recomputable, unrecomputable}`（二元） | 存在能否由**图内配方**重新导出等价状态 | `recomputable` | `unrecomputable` | `[已设计未实现]` |
| 2 | `identity` | Entry 标识 = (变量标识, 版本序)；匿名条目 = 产生节点身份 | 这是哪一条 Entry（同一性） | 产生节点 | 产生节点（= 设备读数节点） | `[已实现]`（`def_nod`） |
| 3 | `version` | 1-based 组内序（现行实现的做法） | 同一 identity 版本序列中的位置 | 版本序数 | 版本序数 | `[已实现]`（**仅盘面**） |
| 4 | `authority` | `{single(ref), shared-readonly, unresolved}` | 同 Entry/version 有多份时**哪一份算数** | `single(ref)` | `single(ref)`（**本文读法**：ref = 设备侧物化，对应原输入的 `authority = device`） | `[提案]` |
| 5 | `location` | mapper 的位置域（**格层不定义位置代数**；现行约定：位置域 = 实例侧声明） | 当前在哪里存在 | DDR / VRAM / 寄存器 | 设备寄存器窗口 | `[提案]` |
| 6 | `persistence` | `{free, transfer-required, externally-owned}` | 消失前其状态**是否必须被转移**到合法载体 | `free` | `externally-owned`（原输入写 `persistent = external`） | `[已设计未实现]` |
| 7 | `replicability` | `{free, readonly-share, forbidden}` | 是否允许存在**多份** | `free` | `forbidden`（原输入写 `no/limited`；`limited` 一档对应 `readonly-share`） | `[提案]` |

⚠ **最右两列的定性**：它们是**映射实例的示例取值**（取自本文输入），**不是取值域定义**——取值域以 §2.3 为唯一来源。
⚠ **上表第 4/6/7 行的实例映射是本文的读法**（把原文的 `device` / `external` / `no|limited` 落到权威枚举上），**如与 §2.3 的解释冲突，以那份为准**。

**权威 §2.3 另给的两条蕴含关系（不是等价）——本文必须服从**（逐字引）：
...
⚠ **对本文 §八⑧（`Evictable`）的直接后果（本文补的分析）**：`Evictable` 的**第二支** `PreserveRequiredState` 与
`persistence` 直接挂钩 ⇒ 上述两条蕴含给出**两条可机械检查的交叉约束**，应作为 §八⑧ 判据 4 的一部分：
- 凡 `recipe = unrecomputable` ⇒ **`persistence` 不得为 `free`**；
- 凡 `replicability = forbidden` ⇒ **`authority` 必须是 `single`**。
⇒ 二者都应在实现侧作为**断言**（违反 = 硬错，不是"退化"）。

原文给的两个实例（照录，供对照——**注意：原文用的是自述词 `yes/no/external`，权威现已给规范枚举，以权威为准**）：
...
> ⚠ **主从声明（2026-09-20 二次复核后收窄 —— 本条是"同一份审计工作的两份拷贝"的处置结论）**
> **逐字段实现距离的「主副本」= `materialization-space.md` §七（实核:新定义与实现的距离）**，其四节分工：
> **§7.0 结论摘要**（七字段一行一览）· **§7.1 `recipe` 是不是真 bit** · **§7.2 其余六字段逐格读数** · **§7.3 版本面** · **§7.4 flag 位预算**。
> **本文原也有自己的一份逐字段表（本会话独立做的），结论与权威一致但是同一份工作的第二份拷贝 ⇒ 必然漂移。**
> **本处已删去重复的逐字段表**，只保留**本文（执行映射侧）特有的两条结论** ＋ **一处对本文 §9.2 有直接后果的引用**。
> **取数一律以 §七 为准**（它比我原表更细：多了 `src/lattice/ent_kernel.cr` 的生产者/消费者面、`replicability` 的设计面出处、`location` 的旁路表）。
> ⚠ **两处说法有一处不一致**（以权威为准）：`persistence` 权威判「**无独立字段**（义务被『必须有 home』隐式承担）」，
> 本文原表写作「槽位在、值恒默认」——**以权威的说法为准**。`recipe` 权威判「**槽位就位，语义未实现**（恒 0，零生产者、零语义消费者）」，与本文「预留位 + 恒零值」同义，**以权威措辞为准**。

**本文（执行映射侧）保留的两条结论**（分析归本文，**依据数据取 §七**）：

1. **`recipe` 与 `persistence` 是同一形态**：**槽位已预留、值恒为默认**——`flags = 0` / `home = -1`；且**原因同一**，
   `ccr_io.cr:83-84` 明写「**分配决策不写回格式**」⇒ 二者**不是"未设计"，而是"设计了槽位但写侧不做决策回填"**。
   ⇒ 对实施批的含义：**变活位不需要改布局**（位已预留），需要的是**一个写点 + 一个消费点**。
   ⚠ **但本条须与权威 §7.4 合读才完整**（见下）——「不需改布局」**不等于**「记录里还有地方放别的」。
2. **七字段里绝大多数落在 `[提案]`/`[已设计未实现]`，只有 `identity` / `version` 是实打实的 `[已实现]`（且各自只是"部分"）**
   ⇒ 新框架**不是对既有实现的重新描述**，而是一次**真实的抽象升级**——不要把"ENT 里已经有 version 和 home 了"读成"新框架已实现"。

**⚠ 对本文 §9.2 / C-3 有直接后果的引用（权威 §7.4，本文采纳其更严结论）**：
权威 §7.4「flag 位预算」的结论是——**位确实预留了，但那个字段所在的记录没地方再放别的**。
⇒ 本文 §9.2 结论 1 的「变活位不需改布局」**成立**，**但不得**据它推出「ENT 记录还有富余可承载新字段」：
`ent_kernel`/`ccr_io` 侧的 ENT 记录**槽位全满**（本文独立复核亦得同结论，见 §十一 C-3 表）。
**两者不冲突，权威那条更严；本文按其结论收窄。**
⇒ 另：权威 §7.4 确认「**全 `.ccr` 只有 ENT 一处有 flags 字段**」（NOD/REG/EDG/TYPE/IFACE 均无），
这正好**独立支持本文 C-3 的「REG 盘面 6×i32 全满、MAP 若按 region 承载须改布局」**——**两处结论互证**。
...
⚠ **`valid` 位被删除**（本批要点）：旧稿的 `X2: DDR valid / VRAM stale` 是"哪份新鲜"的**副本有效性**模型；新框架用 **`authority`** 取代它（"哪份有权威性"）——因为对**无配方条目**（`recipe = unrecomputable`），"VRAM 那份比 DDR 那份旧"这句话**没有意义**（两份可能都不是可再生的；且 `replicability = forbidden` 时根本不该有两份）。
...
`[提案]`。
⚠ **但推导的原料面大量 `[已实现]`**：`effects` 可从 state 链 + 真纯度取（`compute_all_purity`，**`checker.cr:4555`**；opcode 的唯一定义处 `purity_op_effect`，**`checker.cr:4508`**）；`control` 可从 region kind 取（`SG_LOOP/SG_FOR/SG_IF/SG_FLOW/SG_UNSAFE`，`dyn_arr.cr:154-159`）；`addressing` 可从指针分析取（`ptr_analysis.cr`）；`storage` 可从 region 的 arena 归属取（`g_df_node_region`）。
...
⇒ ⚠ **并且**：新框架下还需检查 **`replicability`**——若该 Entry `replicability = forbidden`（一次性 token / 线性资源），则"在 VRAM 再物化一份"本身**非法**，不是"贵"。旧稿的 `valid` 位模型**无法表达这一档**（它默认副本总是允许的）。
...
> 📌 **式的主副本 = `docs/academic/cache-semantics.md` 条款 2′**（**改式先改主副本**；本处为引用）。
> 同标注见 `materialization-space.md` §4.2（权威侧已标）。**本文按同一主副本引用，不另立。**
> ⚠ 与权威侧的分工：**§4.2 = 新式与旧条款的逐条对照（R1–Rn 反例表）+ 主副本指针**；
> **§4.3 = 该式的判据形状（怎么测）**；**§4.4 = 该式的 fail-closed 规则（必配）**；**§4.5 = `recipe = unrecomputable` 的显式约束**。
> **本文 §6.4 只保留执行映射侧要用的那句式 + §八⑧ 的判据**（其判据与权威 §4.3 的「断言形状」互补，**不重复**）。
...
|---|---|---|
| 条款 1 值 = 配方 | 对应 `recipe = recomputable` 一档的**身份来源**；`recipe = unrecomputable` 档**不适用** | `cache-semantics.md:24` |
| 条款 2 驱逐不变量 | `Evictable` 的**第一支** `Recoverable` 的一个实例 | `:25` |
| 条款 3 再生等价 | 同上（`Recoverable` 的机制保证） | `:26` |
| 条款 4 边界公理 | 对应 `recipe = unrecomputable` + `authority = device/external` | `:27` |
| 条款 4b 图内不可重算 | 对应 `recipe = unrecomputable` + `persistence = 必须` + `authority` 不可转移 | `:28,50-59` |
...
> ⚠ **与 §〇.2 同命题，主副本同上**：**`materialization-space.md` §五（含 §5.1 明写归属 + §5.2 可检验划界判据）**。
> 本节 = 执行映射侧的**归属表**（本文特有），**判据不在此复述**。
...
| 事项 | 归属 |
|---|---|
| 哪些 materialization 合法 / 哪些可共存 / 哪些代表同一 Entry+version / 哪些转换保持语义 | **存在格**（本体，归 `materialization-space.md` **§2.1** 四问） |
| 什么时候驱逐 · 放寄存器还是 RAM · GPU 是否保留 residency · LRU / write-back / write-through / prefetch | **mapper**（本文 §五；判据见权威 **§5.2**） |
...
关键性质（S-C L3）：**任何变换的缺失都可接受——直接形态恒合法。**
...
> 每条给：**形式陈述** / **可执行判据的形状** / **三态** / **违反后果** / **反向断言**。
> 判据写法遵既有纪律：**判据组须有正反两条断言**（反向断言当场红 = 最好的诊断）；**单向判据对反向缺陷恒绿**。
...
- **可执行判据的形状**（将来怎么测）
  1. **正向判据（同源对拍）**：同一 `flow` 区域取两版源码（有/无 placement 注解）→ `corec cir` 产物 `.cir` **逐字节 `cmp`**；`corec ccr` 产物同理。
     ⚠ **必须限定条件（本文实核的陷阱）**：新增的**逻辑域名是编译期 interned 串** ⇒ 若域名进 STR 段，`.ccr` 会多一条串 ⇒ **"逐字节同"必假**。判据须写成 **「摘除 MAP 段后，其余段逐字节同」**（或域名不进 STR 段、改走部署配置侧索引）。**这是一条会让判据假红的已知机制，必须写进判据文本。**
  2. **反向判据（正控 = 注解确实生效）**：MAP 段**非空** + dump 面**确实显示** domain constraint（否则"什么都不做"也恒绿 = 覆盖为零）。
     ⇒ **判据对 = {其余段逐字节同} × {MAP 段非空}**；缺任一条 = 判据网失效。
  3. **结构判据**：`erase(A)` 后 `.ccr` **段数**回到基线、**版本不变**。
  4. **端到端判据**：两版源码 `build` 产物在**同一目标描述**下，值/效应序/rc/输出一致（对齐 S-C §3.2 判据 1「观测等价：同程序 + 不同目标描述 → 值/效应序/rc/输出一致」的镜像方向）。
...
- **反向断言**：构造一个**故意**把 domain 名写进值流/类型的最小改动，断言判据 1 **当场红**。做不到 ⇒ 判据是真空的。
...
- **可执行判据的形状**
  1. **配置判据**：部署配置构造 (i)/(ii) 两例 → 断言 mapper 的**候选域索引**与 **residency 键空间**是**两个独立 id 空间**（不共用 id 分配器、不互相索引）。
  2. **检索判据**：给 `(d, m)` 对，`attachedMemoryDomains(d)` 与 `attachedExecutionDomains(m)` 是**两个分别回答的函数**（一域多内存、一内存多域都能答）。断言二者**不同构**（存在 (i) 例 ⇒ 前者非单射）。
  3. **负控（关键）**：把两表**合成单表**（强制一一对应）⇒ 上述配置判据必须**当场红**。若合成后仍绿 ⇒ 判据没有真正约束合并。
  4. **前端判据**：`@executionDomain(X)` 的 X **只能**解析到 ExecutionDomain 命名空间；解析到 MemoryDomain 名必须是**硬错**（且**不是** TF/TK 码——§5.9）。
...
- **可执行判据的形状**
  1. **判据 (a)**：构造 `Req ⊄ Cap(D)` 但 `score(D)` 极小（如成本恒 0）的配置 → 断言 `D ∉ candidates` 且 `placement ≠ D`。**防"成本翻盘合法性"。**
  2. **判据 (b)**：构造 `Req ⊑ Cap(D)` 但 profile 全缺失（或 `UNKNOWN`）→ 断言 `D` 仍可被选中、程序仍编译、行为正确。**防"缺成本 = 不可执行"。**
     ⚠ 此判据须与 S-C §2.3 契约 2/5 对账：缺失 = **保守退化，不猜**；且**不得**用"经验常数"兜底成 `MODELED`。
  3. **正反两条断言**：判据 1 与判据 2 **互为反向断言**——只写判据 1 会把实现逼成"成本永远不生效"；只写判据 2 会放过"成本翻盘"。**必须同时在场。**
  4. **Guarantees 单独一条判据**：构造 `Guarantees` 不满足（如 strict IEEE FP vs relaxed FP 域）但 `score` 极优 → 断言不入选。**这是"合法性条件不是性能条件"的专项断言。**

- **三态**：**仓库已有对应分界的明文** `[已设计未实现]`——S-C §2.2「合法性 soundness = 可判定 / 收益 profitability = 启发式」表 + **L3 裁决**（"合法性可判定、收益启发式（分离原则）"）+ 关键性质"任何变换的缺失都可接受——直接形态恒合法"。
...
- **形式陈述**
  ...
  ```
- **可执行判据的形状**
  1. **正向判据（原子性）**：构造 `X1@DDR` 与 `X2@VRAM` **同时存在**，断言 GPU 消费 `X2` 时**必须**产生 `materialize(X2 → VRAM)`——**不得**复用 `X2@DDR`（域错）也**不得**误用 `X1@VRAM`（版本错）。两向都给期望值（域错与版本错各一条档）。
  2. **接口判据（结构性）**：静态扫消费面，断言**不存在** `latest()` / `newest()` / **`valid` 位** / 跨域 `max(version)` 之类的查询点。⚠ 判据须**限定为"跨 MemoryDomain 的比较"**——**同域内**的版本推进比较（存在格的版本性）**不违反**本条。**不加限定 = 判据会对合法实现恒红（过判）。**
  3. **分档判据（本批按字段重述）**：对 `recipe = unrecomputable` 条目断言：`persistence` 必填、跨域"再生"路径**不可用**（必须写回而非重跑）；**反向**对 `recipe = recomputable` 断言可驱逐可再生。**正反两条断言**：只写前一条会把所有条目都当不可再生（过判）。
  4. **`authority` 判据（本批新增）**：断言**至多一个** authoritative materialization（或权威显式落在 `device`/`external`）；反向：构造两份都标 authoritative ⇒ 必须**硬错**。⚠ 这是唯一能抓住"副本同值"假设回归的判据。
  5. **`replicability` 判据（本批新增）**：对 `replicability = forbidden` 的 Entry，断言"再物化一份"路径**不存在/被拒**（而不仅是"贵"）；反向：对 `replicability = free` 断言复制合法。**这是旧框架完全无法表达的一档**（旧 `valid` 位模型默认副本总是允许）。
  6. **版本可折叠（防过判）**：`X1` 与 `X2` 在**版本代数**上可证等价（如 `x = x + 0` 类）⇒ **允许**复用。⚠ 但**不得**靠值比较实现——判据要能区分"按代数折叠"与"按值相等折叠"（后者须红）。
  7. **逐字节判据**：物化动作的注入**不得**改变语义图（与不变量 ① 共用正向判据）。
...
- **判据形状**：负控档 = 目标域**不存在**（部署配置里无该键）→ 断言 `rc=1` + 诊断含 `EMAP` 码 + **零产物**。
  ...
  正反两条断言：`@preferredDomain(X)` 同配置 ⇒ **rc=0** + 回落（防"一律报"）。
...
- **判据形状**：同源码两种 `mapping` 模式 → 断言 (i) static 产物中无 mapper 符号/无运行期域查询；(ii) **两者都不违反不变量 ①**（语义图同）；(iii) static 下 `@executionDomain` 不可满足仍是硬错（与 ⑤ 同判）。
  正反两条断言：(i) 的正控 = dynamic 产物**确实含** mapper（否则"删了 mapper"和"从未生成"不可区分）。
- **三态**：`[提案]`。
...
- **判据形状**：对**全语料**（`tests/suite/*.cr` + `src/compiler/*.cr` 自身）断言 AUTO region 的 `candidates(R) ≠ ∅`。**覆盖为零的判据只会腐烂不报错**，故须挂到既有语料 runner 而非独立小档。
  反向断言：构造一个 `candidates(R) = ∅` 的畸形配置 ⇒ 断言它是**硬错**而非静默。
...
> 📌 **式的主副本 = `docs/academic/cache-semantics.md` 条款 2′**（同 §6.4；**改式先改主副本**）。
> 本条的**判据 = 执行映射侧视角**；权威侧的**断言形状**见 `materialization-space.md` **§4.3**（怎么测）、**§4.4**（fail-closed 必配）、**§4.5**（`recipe = unrecomputable` 的显式约束）——**三者互补，不互为替代**。
...
- **可执行判据的形状**
  1. **正反两条断言**：(a) 构造 `recipe = unrecomputable` 且 state 未转移的条目 ⇒ 断言驱逐**被拒**（硬错/零产物），**不是**"执行了但慢"；(b) 构造 `recipe = recomputable` ⇒ 断言驱逐**合法**。只写 (a) 会把所有条目当不可驱逐。
  2. **第二支专项断言**：构造 `recipe = unrecomputable` 但 state **已**转移到合法 materialization ⇒ 断言驱逐**合法**。⚠ 这是"第二支"的唯一断言；**缺它则第二支形同虚设**（实现可以永远只走第一支而判据全绿）。
  3. **合法性 ≠ 性能 的分离断言**：构造 `Evictable = 真` 但成本极高 ⇒ 断言决策仍是 **mapper** 的选择（可能选择不驱逐），而**不是**格层把它标为非法。**反向**：`Evictable = 假` ⇒ 断言**任何**成本都不能让它被驱逐（与不变量 ③ 判据 (a) 同形）。
  4. **与旧条款的一致性判据**：对 `recipe = recomputable` 条目，断言新式与条款 2/3 的判定**逐条一致**（防"新式松到把缓存实例判错"）；对 `recipe = unrecomputable`，断言与条款 4b 的"必写回"一致。

- **三态**：`[提案]`。既有条款 2/3/4b 是它在**缓存实例**上的取值 `[已设计未实现]`（文档面 `cache-semantics.md:25-28`）。
- **违反后果**：把"非法驱逐"实现成"合法但慢" ⇒ **不可再生状态被丢弃** = 静默语义破坏（本仓最重视的失效形态）。
- **反向断言**：见判据 2（第二支）与判据 3（合法性/性能分离）。
...
- **形式陈述**
  ...
  ```
- **可执行判据的形状**
  1. **可缺席性判据**：清空全部 policy 配置（无 `objective`、无阈值、无 PGO 数据）⇒ 断言**格层判定结果逐条不变**（与带 policy 时对拍）。**这是本条最关键的一条判据**——若格层判定随 policy 变，则 policy 泄漏进了格层。
  2. **策略词零命中判据**：静态扫格层面（`materialization-space.md` 的规则 + §3.4 字段 + §八①④⑧），断言无 `LRU` / `prefetch` / `hot` / `recent` / `write-back` 等策略词。⚠ 判据须**限定在格层面**——mapper 侧（§五）**必然**有这些词，不加限定会恒红。
  3. **正反两条断言**：反向构造一个**必须**靠 policy 才能决定的问题（如"这两份等价 materialization 该保留哪份"）⇒ 断言格层**拒绝回答**（返回"二者皆合法"），把选择留给 mapper。**这说明格层判定里确实没有 policy。**
...
| **M1** | **Syntax** | `@preferredDomain(name)` · `@executionDomain(name)`；支持 `fn`/`flow`/`loop`/`region`；**parser/checker 只做名字与结构处理** | 正：四类位置各一档，注解被解析进侧表；负：未知域名/非法位置 ⇒ rc=1 + 码 + 零产物（`check` 面）；**结构判据**：注解**不产生任何 IR**（`--dump-*` 前后逐字节同） | M0 |
| **M2** | **IR metadata** | annotation → region mapping metadata；**HDFG byte/semantic identity 不因 placement 改变**；dump 支持显示 domain constraint | **= 不变量 ① 的四条判据**（含"摘除 MAP 段后逐字节同"的限定条件） | M1 |
...
| **M4** | **Static placement** | AUTO/PREFER/REQUIRE；**先 CPU-domain ↔ CPU-domain；不涉及 GPU** | **= 不变量 ③ 的正反两条断言 + ⑤**；同后端两域（原文 §25 例）全链路可跑 | M2,M3 |
...
- **我的核实结论（**已按"以书面为准"回退为进度问题**）**：
  ...
  3. **"canLower(R) -> true" —— 今天为假，但这是进度性的（M2 挂账未做）**：表模式覆盖**直线子集**（`const/store/load/add/sub` + 4 核事件），其余指令形状今天走硬编码 `e2_*` 路径。
     ⇒ **该硬编码路径是「临时缺口」，不是「永久设计」**——`TODO.md:2215` 明列「全量切换：无 `--table` 旧路径退役」，**M2 完成后即消除**。
...
- **「通用下降路径」的归属（**第一轮的"设计空缺"判定已撤回**）**：
  - **HIT 仍是该位置的既定承担者**——只是**要等 M2 完成才成立**。这正是待裁项 (c) 的内容（第一轮删错了，本批恢复）。
...
- **待裁点**：
  - (a) 原文的 `backend = "hit"` 是否**应改述**为「`backend = "classic"`（其编码由 HIT 表 + 尚未退役的旧路径共同实现）」？还是保留 `"hit"` 但**在文档里显式声明**"此处 hit = 表机制所在的默认域"（即接受一次术语重载）？
  - (b) **通用下降路径的身份**：是 HIT 表模式（**须先完成 M2**），还是今天的硬编码路径（**已可用但不在表内**）？这直接决定 §八 不变量 ⑦ 的"通用域"是什么。
  - **(c) 【本批恢复】若采 (b) 的前者，则 M4 前置应增加「HIT M2 完成」——原文里程碑无此项。**
...
- **⚠ 本节作为方法论实例（本批新增，勿淡化为一次普通修订）**——**「口述 vs 书面记载冲突 ⇒ 以书面为准」**的一个**完整实例**：
  ...
  **要点（下游可复用）**：
  ...
  - **如实记录是关键的一条**：若第 ② 步把口述写成事实，第 ③ 步的实核就失去触发理由（**"已证"的前提会让反证显得像噪音**）。
  - ⇒ **本文档全程适用**：凡 team-lead 转达的维护者裁决，一律写「**未见原话，来源 = 经 lead 转达**」（维护者已确认此做法）。
...
- **剩余待裁点（**只剩一条**，不裁决）**：**条款 4b 自身的措辞是否要改**——它现在写在 `cache-semantics.md`（被定位为"缓存映射实例的规律"）里，而它讲的其实是**存在格的一般规则**（any `recipe = unrecomputable`）。⇒ 是"条款 4b 应**上移**到 `materialization-space.md`"，还是"保留在原处、由存在格文档引用它"？**本批不裁决，也不改那份文件**（C-11）。
...
- **⚠ 实核与下调（2026-09-20）**：另一名写手核出 **`.ccr` 不需要加段、不需要 bump `CCR_VERSION`**——**七字段复用 `ENT.flags` 空闲位 + 现有 4B 槽**。**本文独立复核（部分证实、部分须留待）**：
  ...
  **⚠ 由本表得出的一个附加约束（本文补，其他写手未提）**——**位打包路线可行的前提是"字段值域有界"**：
  ...
  ⇒ **下调范围（本文结论）**：**"段位资源争用"在材料化七字段面消失**（那部分确不需要段）；**MAP 自身**是否入 `.ccr`、以何承载，**仍待定**——但**优先解不是"加段"**：
...
- **我的核实结论**：原文"部署策略不进源码"与 L5/§1 **同向**。但**边界待定**：`objective` 属于**部署配置**（原文）→ 而 L5 说"放置策略不进**表**"——**部署配置 ≠ 表**吗？在仓库的术语里 `Core.toml` 是**部署配置**、HIT 表文件是**表**，二者是不同物 `[已实现]`（前者 `project.cr` 读，后者 `--table` 传入，`hit.cr:4`）。⇒ **不冲突**，但文档须显式写"部署配置与目标描述表是两处，L5 只约束后者"。
- **待裁点**：`objective` 是否应落**部署配置**（原文）还是**目标描述表**（S-C §3.1 的"目标描述"三轴 + 表）？若落表，则与 L5"策略不进表"直接冲突，须先裁 L5 的适用范围。

### C-11 ⚠ 「格 = 缓存」教义的既有落点（**原快照；2026-09-20 起已由另一名写手就地修订**）

> ⚠ **读法（本批复核后补）**：本节的**表 = 修订前的原快照**——用于**留痕**（记录"改之前是什么样"），
> **不是现状**。**2026-09-20 起这 7 处正被就地修订**（维护者裁定「同批就地改」，见本节末"✅ 已裁"）。
> **本文全程未改那 7 个文件**；下游若要用**现状**，请读那些文件本身，**不要读本节表**。
...
- **待裁点（~~不裁决，且本批一律不改这些文件~~ ⇒ 见本节末"✅ 已裁"：已裁决 = 同批就地改，改由另一名写手执行；**本文仍一律未改**）**：
...
- **✅ 已裁（2026-09-20，维护者经 team-lead 转达）**：**`格` 保留作简称**。理由（与原话一致）：既有链名本来就是「图 → **格** → 编码」且注明「中间存在空间」，维护者原话也是「**格层** = Materialization / Existence Space」⇒ **这是术语具体化，不是层结构变更**，四段链不动。
  ⇒ **本文的落地做法**：正式名用「**存在格**」（首次出现处附英名 `Materialization Space`），简称用「**格**」，**与既有文档的「格」字不冲突、无需全仓替换**。C-12 判断（层名几乎重合）经维护者确认成立。
...
15. ~~**`docs/maintainer/design/materialization-space.md`（existdoc 的产物）落盘后的接口面**——…**未核**该文档实际用的字段名/层名是否与本文 §3.4.2 的七字段一致。~~
    **✅ 本批结案（2026-09-20 复核）——状态由「未核」升为「实测不等 ⇒ 已按主从统一」**：
    该文档**已落盘**（`docs/maintainer/design/materialization-space.md`，65719B；核验时在 `core-existdoc-ws` 工作区，**尚未进 develop@origin**）。
    **实测**：其 §2.3「**字段级模型**(七字段)」（**本轮改名**；本文旧引作「字段级定稿」，已随之更新）给出**取值域**两列（语义 · 取值域 · 谁写 · 谁读 + 两条蕴含关系），
    与本文 §3.4.2 原表的取值域 **4/7 行不等**（`recipe` / `authority` / `persistence` / `replicability`），
    **且两处原互不指向**——命中本文自己命名的「**同一命题两处说法**」第一类。
    **处置（已完成）**：**建立主从** —— 层定义那份为权威；本文 §3.4.2 **改为引用**（取值域列逐字引 §2.3，只保留本文真正多出来的两列实例取值），
    并**全文术语对齐**（`recipe = recomputable|unrecomputable` 等，替换掉原自述词 `yes/no/external`），
    §八④/§八⑧ 的判据措辞随之对齐。**详细统一说明见 §3.4.2 顶部的「主从声明」。**
    ⚠ **仍未核实**：该文档的**其余章节**（我只读了 §2.3 及其前后各十行）是否还有与本文重叠的命题尚未统一。
...
> 复现方式：`grep -o '\[已实现\]' <file> | wc -l`（方括号形态，**包含 §13.2 自查段的引用**）。

**两种统计方式都给**（见既有纪律「枚举型计数随细看只会变多」⇒ 写「至少 N」+ 清单，不写穷举断言）：
...
| 〔提案〕 | **47** | 其余全部（**默认档**） | A 只数**显式写出**的标记；**未含**默认推断档（散落各处、无标记即为提案） |

⚠ **A 档与 B 档的关系（两批合并说明，避免被读成漏登）**：
- **上一批** A **+2/+2/+3**：给 §3.4.2 七字段表加了「本文三态」列（7 行各带标记）——该表是 §3.4.5 的**摘要视图**，逐条依据已在索引表第 35–44 条 ⇒ **不重复登记**为新索引行。
- **本批** A **−2/−2/−2**：**删去了 §3.4.5 的重复逐字段表**（与权威 §7 是同一份审计工作的两份拷贝，已按主从统一）——该表原有的标记随之消失；**索引表第 35–44 条不变**（那是本文独有的实核，仍有效）。
⇒ **两批都是"表格增删"，不是主张增删** ⇒ **B 档两次均未变**（45 / 14）。**A 只随呈现形态浮动，B 才反映主张条数。**

**读数的三条纪律（实测确立，缺任一条则数不可复现）**：
...
**A 与 B 的差额来源**（可复现，不是错）：同一主张常在一处表格 + 一处行内各标一次；索引表本身也是标记出处。**差额不是计数错误**，是两种统计方式的定义差。
...
| 45 | ⚠ **HIT M2 的四处记载（自本批起 = 以此为准）**：仍记「挂账 / 待执行」，**无淘汰记载**；正是它们把口述裁决翻了过来（维护者改裁「以书面为准」） | `TODO.md:2211`、`TODO.md:2215`；`docs/superpowers/plans/2026-09-07-hit-m2.md:3`；`docs/superpowers/specs/2026-09-07-hit-m2-design.md` |
...
- **不变量表**：①–④ 齐（§八），另补 ⑤⑥⑦ + **〔本批新增〕⑧（`Evictable` 定义）⑨（格层不承担 policy）**。每条含**形式陈述 + 判据形状 + 三态 + 违反后果 + 反向断言**。① 的判据**已显式处理 STR 段陷阱**（否则判据会假红）；③④⑧⑨ 给的是**正反两条断言**而非单向。
  ⚠ ④ 本批**加强**：七字段完备 + **删除 `valid` 位** + 新增 `authority`/`replicability` 两条专项判据（这两条是旧框架**无法表达**的档）。
  ⑧ 的关键断言 = **第二支 `PreserveRequiredState` 的单独判据**（缺它则第二支形同虚设）。
  ⑨ 的关键断言 = **可缺席性**（清空 policy 后格层判定逐条不变）。
...
- **〔最新批〕HIT M2 淘汰的落实（C-1 重写 + C-3 下调）**：
  - **C-1 的裁决反转（本节最重的一条，须按"两轮"读）**：
    - **第一轮（口述）**：维护者经 team-lead 转达「**HIT M2 早已淘汰**」（**未见原话，来源 = 经 lead 转达**）⇒ 本节曾据此 ① 删去"M4 前补 HIT M2 完成"待裁项；② 论证**去进度化**；③ 判「通用下降路径 = **未被命名的设计空缺**」。
    - **第二轮（书面校正 ⇒ 整轮回退）**：本文按纪律**去实核书面留痕**，找到**四处反向记载**（`TODO.md:2211/2215`、`plans/2026-09-07-hit-m2.md:3`、`specs/2026-09-07-hit-m2-design.md`）⇒ 维护者改裁「**以书面为准**」⇒ ① 待裁项 (c) **恢复**（留痕式）；② 论证**回退到进度化**（`lower_to_core.cr:21` 是**临时缺口**，非永久设计）；③"设计空缺"判定**撤回**（HIT 仍是既定承担者，待 M2 完成）；④ **四处记载自本批起 = 以此为准**。
    - 连带登记**全部撤回**：HIT spec §1 与 S-C §0.3 的"前提失效"**不成立**（那些是**待 M2 达成的目标**）。
    - **§八⑦ 恢复**为"依赖 C-1 待裁点 (b)/(c)"（**不再标注"不可判定"**）。
    - **方法论实例已写进文档**（C-1 末节）：**「口述 vs 书面记载冲突 ⇒ 以书面为准」**的完整四步实例；要点 = ① 不把口述写成已证事实 ② **实核留痕是可操作的翻案步骤** ③ **回退留痕、不静默改回**。
...
- **未核实清单**：§十二 十五条（第 15 项 = 与 `materialization-space.md` 的命名对齐义务 —— **已结案**，见下）。
- **〔本批〕与权威侧的逐节对拍结果（11 节：8 重叠 / 3 不重叠）—— 主从表见文档头**：
  - **① 双份实核**：本文 §3.4.5 与权威 **§7.2** 各自独立做过「七字段实现距离实核」⇒ **同一份工作的两份拷贝，必然漂移**。
    **处置**：**主副本 = 权威 §7.0–§7.4**；本文**删去重复的逐字段表**，只留**执行映射侧的两条结论**
    ＋ **一处对本文 §9.2 有直接后果的引用**（权威 §7.4「位预留了，但记录里没地方再放别的」——**其更严，本文采其结论**）。
    另：本文原表对 `persistence` 判「槽位在、值恒默认」，权威判「**无独立字段**」⇒ **以权威说法为准**。
  - **② 政策边界无互指**：本文 §〇.2 / §6.7 与权威 **§5** 同命题各写一遍 ⇒ **主副本 = §5**（§5.1 明写 + **§5.2 可检验判据**）；
    本文只保留**特有的一句**「policy **也不属本文的执行域层**」，判据**不复述**。
  - **③ 2′ 公式主副本**：权威 §4.2 已标「主副本 = `cache-semantics.md` **条款 2′**」，本文**未标** ⇒ **已补**（§6.4 与 §八⑧ 各一处，写明"改式先改主副本"）。
  - **④ 四问只指文档未指节号** ⇒ **已补 `§2.1`**（§〇.2 与 §6.7 归属表各一处）。
  - **⑤ 陈旧引用**：本文原引权威表题为「字段级**定稿**」，其本轮已改名「字段级**模型**」（消「定稿」与"本表整体 = [提案]"两说）⇒ **已更新**（两处，并留改名留痕）。
  - **⑥ 三节不重叠的处置（本文判断）**：
    - **§7.4（`.ccr` 分层结论）** ⇒ **引**：它**独立支持**本文 C-3 的「REG 盘面 6×i32 全满、MAP 若按 region 承载须改布局」，**两处互证**；
      且它更严（「位预留 ≠ 记录还有地方」）⇒ 本文 §9.2 结论 1 已按其收窄。
    - **§4.3（式的判据形状/断言形状）** ⇒ **引**：与本文 §八⑧ 的**判据互补**（断言形状 = 该式自己的测法；判据 = 不变量族内的正反两条断言），**已互相注明不替代**。
    - **§10（兼容性声明）** ⇒ **引**（文档头主从表），本文侧**不另立**。
- **〔末批〕复核处理（2026-09-20，两条，均为"同一命题两处说法"）**：
  - **① 七字段取值域主从建立（重）**：复核发现权威 `materialization-space.md` §2.3 与本文 §3.4.2 **各给一份取值域、互不指向**，
    **4/7 行不等**（`recipe`/`authority`/`persistence`/`replicability`）——**正是本文自己命名的第一类命中**。
    本文在 §十二 项 15 **预告过**这条（"未核该文档实际用的字段名是否一致"）⇒ 该文档落盘后**由「未核」升为「实测不等」**。
    **处置**：§3.4.2 改标题为**执行映射侧视图** + 顶部加**主从声明**（取值域**逐字引** §2.3，只保留本文独有的两列实例取值）
    ＋ **全文术语对齐**（`recipe = recomputable|unrecomputable` 等，替换原自述词 `yes/no/external`；§八④/§八⑧ 判据措辞随之对齐）
    ＋ 补记权威的两条**蕴含关系**并导出**两条可机械检查的交叉约束**。**修正留痕**写在 §3.4.2 顶部。
  - **② C-11 标题/时态与自身"已裁"注相反（轻但最易先读到）**：标题原写「**一律不改**」、待裁行写「不裁决…本批一律不改」，
    而 8 行后已写「**正由另一名写手就地修订**」；且文件头 `:6` 与 §〇.1 表仍以**现在时**引 `project-book.md:216`/`z-vision.md:15`
    为"既有定稿措辞"——**那两处已被改**。
    **处置**：标题加「**原快照；2026-09-20 起已由另一名写手就地修订**」＋ 表前加**读法说明**（表 = 修订前快照，下游要现状请看那些文件本身）；
    待裁行加删除线并指向"已裁"；文件头 `:6` 与 §〇.1 表的时态改为**过去式**并注明已改。**本文（本文件）仍全程未改那 7 个文件。**
...
- **层归属（本批新增）**：§〇.2 + §6.7 + §八⑨ 三处**说法一致**地划界「格层只答四问 / policy 全归 mapper」。
- **本文档未修改任何既有文件**（新增单文件；`/tmp/briefs/*` 两份原始输入亦未动）。
