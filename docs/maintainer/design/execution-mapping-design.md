...
- **三态**：`[提案]`。⚠ **本批更新（C-1 裁决后）——本条目前不可判定**：
  维护者已裁「**HIT M2 早已淘汰**」⇒ 通用下降路径**不是 HIT**，且原文假设义下的"HIT 通用域"**永不成立**（C-1 结论 2/3）。
  而本文 C-1 的重新指派结论是：**今天没有一个被明确命名的通用执行目标**（实际的完整路径是 `instr.cr` 硬编码发射器，但它是**发射实现**、不在域模型里）。
  ⇒ **⑦ 的"通用域"身份为空缺 ⇒ 本条须待"通用下降路径"被指派名字与位置后才能写成可判定形式**（C-1 待裁点 (b)）。
  ⚠ **不得**把 ⑦ 默认读成"HIT 兜底"——那正是本批删掉的错误前提。
...
⚠ **术语撞名警告**：本表的 **`M2`（IR metadata）与 HIT 的 `M2`（全表驱动）是两个不同的里程碑**，编号空间独立。本批引入"HIT M2 已淘汰"（C-1）后，**凡见裸 `M2` 须先判是哪一族**——HIT 那族一律写全称「**HIT M2**」。
...
- **⚠ 维护者裁决（2026-09-20，经 team-lead 转达；本文未见原话）**：

  > **「HIT M2 早已淘汰。」**

  ⇒ **M2 不是"未完成、待收口"，而是这个里程碑已经不做了。** `lower_to_core.cr:21` 的「未映射指令 → 硬编码旧路径」**不是临时缺口，是永久设计**。

  **书面留痕核查（本会话实核）**：**未见任何"M2 淘汰"的书面留痕**；相反，**三处书面记载与之直接相反**：
  ...
  ⇒ **结论：维护者口述 vs 四处书面记载冲突**（非"找不到留痕"，而是**找到了反向留痕**）。**本文只登记，不改这些文件**（C-11 同款口径）。
  **待裁**：上述四处如何收口（标注"已淘汰"？删除？保留但加否决注？）——**归维护者，本文不裁决**。

- **我的核实结论**：
  1. **"HIT 不需要为了异构计算修改语义" —— 成立**（`[已实现]` 级）：HIT 位于发射层、在 placement 下游；HIT spec §7「优化不进表（判定/分配 = 格形态层）」已是既有条款。本设计**不需要改 HIT**。
  2. **"HIT = baseline universal execution target" —— 不成立**。理由是**范畴错位**（**结构性的，与进度无关**）：原文把 HIT 当作**一个域**（`backend = "hit"` 的 ExecutionDomain），仓库里 HIT 是**机制**（"替换表 = 换目标平台"）。**一个机制不能同时是它自己的一张表。**
  3. **"canLower(R) -> true" —— 为假**：表模式覆盖**直线子集**（`const/store/load/add/sub` + 4 核事件），其余指令形状**永久**走硬编码 `e2_*` 路径（M2 已淘汰 ⇒ **这不是"还没做"，是"不做"**）。
     ⇒ **今天不存在"HIT 作为通用执行目标"这回事**，且**将来也不会有**（该目标唯一的实现路径 = M2）。
  4. **未指派的后果（结构性的，须登记）**：HIT spec §1 的核心命题「**硬件接口表 = 运行的唯一平台依赖**」「替换表 = 换目标平台/范式：同一 Core 程序配不同表直接运行」**随 M2 淘汰而不成立**——表模式恒只覆盖子集，**全 op 走表永不达成**。
     ⇒ 这也动摇了 S-C §0.3 的「**HIT M2 复启**…M2 的实现归属不变（HIT 线）」`[已设计未实现]`：**该前提已被维护者否定**，S-C 该处须同批重定（**本文不改 S-C**，登记）。
  5. **但原文方向仍有前件（避免误判为"原文错"）**：S-C §0.0「单 CPU 上的 SIMD/cache/NUMA/… 与异构 placement 是**同一个问题**…机制同一，**无第二条性能路线**」仍成立；S-C §3.1 把"目标描述"写成「表：语义表 + 投影条目 + 运行时条目 + 设备/投影表（hw-map）+ 成本事实」仍成立。⇒ **"目标描述走表"的立场不变；变的是"表不再追求覆盖全 op"**。

- **「通用下降路径」的重新指派（本节核心产出）**：**今天没有一个被明确命名的通用执行目标。**
  - **不是 HIT**（结论 2/3：机制 ≠ 域，且覆盖只到子集、永久如此）。
  - **实际承担全部 IR 的是 `instr.cr` 的硬编码发射器**（`e2_*` 家族 + `elf.cr` 发射循环）——它在**功能上**是唯一完整路径，但它是**发射实现**，在仓库里**从未被命名或设计为**"通用执行目标/通用域"，也**不在任何 placement 语汇里**。
  - ⇒ **本文的判定：这是设计空缺，不是已有物。** 新设计**必须为通用下降路径指派一个名字与位置**；把它默认成 HIT 是**错的**（原文假设），把它默认成"硬编码发射器"则是**把实现细节升格为设计概念**（同样不妥，且它不在域模型里）。
  - ⇒ 影响 **§八 不变量 ⑦**（"未指定 ⇒ 恒存在合法映射"）：该条的"通用域"**今天是未定义的**，故 ⑦ 目前**不可判定**——须先补通用域的身份（见 ⑦ 更新注）。
- **待裁点**：
  - (a) 原文的 `backend = "hit"` 是否**应改述**为「`backend = "classic"`（其编码由硬编码发射器承担；HIT 表是并行的**实验性**子集路径）」？还是保留 `"hit"` 但**在文档里显式声明**术语重载？
  - (b) **通用下降路径的身份**（本文判定为"空缺"）——由谁承担、叫什么名字、放在域模型哪一格？（**这是 M4 的真前置**）
  - ~~(c) M4 前补「HIT M2 收口」~~ —— **已删除**：M2 已淘汰，**不得**给一个已废弃的里程碑加前置（本节按维护者裁决删去该待裁项）。
...
2. **HIT 表模式的逐 op 覆盖清单**：本会话核到「直线子集 = const/store/load/add/sub」（`lower_to_core.cr` 头注）+「未映射指令 → 旧路径」，**未逐条核** IR 的 38 op 中哪些走表/哪些走旧路径。
   ⚠ **本批口径更正**：这**不再是"完成百分比"问题**（M2 已淘汰 ⇒ 覆盖率**不会**再变，见 C-1）。该清单的用途改为**划定表模式的永久边界**（哪些 op 永在表内），供 §5.10 variant 与 §5.11 backend 接口使用。
...
| 45 | ⚠ **HIT M2 的四处反向留痕**（仍记「挂账 / 待执行」，**无淘汰记载**） | `TODO.md:2211`、`TODO.md:2215`；`docs/superpowers/plans/2026-09-07-hit-m2.md:3`；`docs/superpowers/specs/2026-09-07-hit-m2-design.md` |
...
- **〔最新批〕HIT M2 淘汰的落实（C-1 重写 + C-3 下调）**：
  - C-1：维护者裁「**HIT M2 早已淘汰**」（**经 team-lead 转达，本文未见原话**）⇒ ① 删去"M4 前补 HIT M2 收口"待裁项；② 论证**去进度化**（改结构论证：机制 ≠ 域）；③ **通用下降路径重新指派 = 「未被命名的设计空缺」**（实际完整路径 = `instr.cr` 硬编码发射器，但它是发射实现、不在域模型里）；④ **书面留痕核查 = 找到四处反向记载**（`TODO.md:2211/2215`、`plans/2026-09-07-hit-m2.md:3`、`specs/2026-09-07-hit-m2-design.md`）—— **不是"找不到留痕"，是"找到了反向留痕"**。
  - 连带登记：HIT spec §1「唯一平台依赖 / 替换表 = 换目标平台」随 M2 淘汰**不成立**；S-C §0.3「HIT M2 复启」前提**已被否定**（**本文不改 S-C**）。
  - **§八⑦ 状态改为"不可判定"**（通用域身份为空缺）。
  - C-3 **下调**（部分）：七字段**确不需要段**（`ENT.flags` 空余 **28 位**，实测）；⚠ 但 ① "现有 4B 槽"**本文未找到**（ENT 内存 6 槽 / 盘面 7 槽**全命名**）；② `location` 需**域编号** ⇒ 位打包**给内存域数量设硬上限**（= 仓库一直在退役的 `MAX_*` 模式）；③ REG 记录**全满** ⇒ MAP 若按 region 承载仍须改布局。§9.3 倾向**由 A 改为 C**（placement 不进 `.ccr`，理由 = 不变量 ① 本身）。
