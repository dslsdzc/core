...
>   | 式的**判据形状 / fail-closed / 显式约束** | **§4.3 / §4.4 / §4.5** | §八⑧ 的**腿位**与之互补 |
...
>   | 三节**不重叠**：§4.3 钉形 · §7.4 `.ccr` 分层结论 · §10 兼容性 | 权威侧 | 本文 **§3.4.5 / §9.2 / C-3** 与之**互证**（§7.4 独立支持本文"REG 全满"复核） |
...
⇒ **可检验判据（本文采用权威 §5.2 的，并落成本文侧的一条腿）**：本文 §八⑨ 腿 2「策略词零命中」**须同口径**——格层面不得出现「何时/多久/顺序」类词；**该腿的判定口径以权威 §5.2 为准**。
...
> ⚠ **主从声明（2026-09-20 复核后收窄 —— 本条是"同一命题两处说法"的第一类命中，已收口）**
...
| 3 | `version` | 1-based 组内序（现行实现口径） | 同一 identity 版本序列中的位置 | 版本序数 | 版本序数 | `[已实现]`（**仅盘面**） |
...
| 5 | `location` | mapper 的位置域（**格层不定义位置代数**；现行口径：位置域 = 实例侧声明） | 当前在哪里存在 | DDR / VRAM / 寄存器 | 设备寄存器窗口 | `[提案]` |
...
`persistence` 直接挂钩 ⇒ 上述两条蕴含给出**两条可机械检查的交叉约束**，应作为 §八⑧ 判据腿 4 的一部分：
...
> ⚠ **主从声明（2026-09-20 二次复核后收窄 —— 本条是"同一份审计工作的两份拷贝"的收口）**
...
> ⚠ **口径差异一处**（以权威为准）：`persistence` 权威判「**无独立字段**（义务被『必须有 home』隐式承担）」，
> 本文原表写作「槽位在、值恒默认」——**取权威口径**。`recipe` 权威判「**槽位就位，语义未实现**（恒 0，零生产者、零语义消费者）」，与本文「预留位 + 恒零值」同义，**以权威措辞为准**。
...
- **可执行判据的形状**
  ...
  3. **分档腿（本批按字段重述）**：对 `recipe = unrecomputable` 条目断言：`persistence` 必填、跨域"再生"路径**不可用**（必须写回而非重跑）；**反向**对 `recipe = recomputable` 断言可驱逐可再生。**两向钉子**：只写前一条会把所有条目都当不可再生（过判）。
  ...
  5. **`replicability` 腿（本批新增）**：对 `replicability = forbidden` 的 Entry，断言"再物化一份"路径**不存在/被拒**（而不仅是"贵"）；反向：对 `replicability = free` 断言复制合法。**这是旧框架完全无法表达的一档**（旧 `valid` 位模型默认副本总是允许）。
...
> 本条的**判据腿 = 执行映射侧视角**；权威侧的**钉形**见 `materialization-space.md` **§4.3**（怎么测）、**§4.4**（fail-closed 必配）、**§4.5**（`recipe = unrecomputable` 的显式约束）——**三者互补，不互为替代**。
...
- **可执行判据的形状**
  1. **两向钉子**：(a) 构造 `recipe = unrecomputable` 且 state 未转移的条目 ⇒ 断言驱逐**被拒**（硬错/零产物），**不是**"执行了但慢"；(b) 构造 `recipe = recomputable` ⇒ 断言驱逐**合法**。只写 (a) 会把所有条目当不可驱逐。
  2. **第二支专项钉**：构造 `recipe = unrecomputable` 但 state **已**转移到合法 materialization ⇒ 断言驱逐**合法**。⚠ 这条是"第二支"的唯一钉子；**缺它则第二支形同虚设**（实现可以永远只走第一支而判据全绿）。
  3. **合法性 ≠ 性能 的分离钉**：构造 `Evictable = 真` 但成本极高 ⇒ 断言决策仍是 **mapper** 的选择（可能选择不驱逐），而**不是**格层把它标为非法。**反向**：`Evictable = 假` ⇒ 断言**任何**成本都不能让它被驱逐（与不变量 ③ 腿 (a) 同形）。
  4. **与旧条款的一致性腿**：对 `recipe = recomputable` 条目，断言新式与条款 2/3 的判定**逐条一致**（防"新式松到把缓存实例判错"）；对 `recipe = unrecomputable`，断言与条款 4b 的"必写回"一致。
...
15. ~~**`docs/maintainer/design/materialization-space.md`（existdoc 的产物）落盘后的接口面**——…**未核**该文档实际用的字段名/层名是否与本文 §3.4.2 的七字段一致。~~
    **✅ 本批结案（2026-09-20 复核）——状态由「未核」升为「实测不等 ⇒ 已按主从收口」**：
    ...
    **处置（已完成）**：**建立主从** —— 层定义那份为权威；本文 §3.4.2 **改为引用**（取值域列逐字引 §2.3，只保留本文真正多出来的两列实例取值），
    ...
    §八④/§八⑧ 的判据措辞随之对齐。**详细收口说明见 §3.4.2 顶部的「主从声明」。**
    ⚠ **仍未核实**：该文档的**其余章节**（我只读了 §2.3 及其前后各十行）是否还有与本文重叠的命题未收口。
...
| 〔提案〕 | **47** | 其余全部（**默认档**） | A 口径只数**显式写出**的标记；**未含**默认推断档（散落各处、无标记即为提案） |

⚠ **A 档与 B 档的联动口径（两批合并说明，避免被读成漏登）**：
- **上一批** A **+2/+2/+3**：给 §3.4.2 七字段表加了「本文三态」列（7 行各带标记）——该表是 §3.4.5 的**摘要视图**，逐条依据已在索引表第 35–44 条 ⇒ **不重复登记**为新索引行。
- **本批** A **−2/−2/−2**：**删去了 §3.4.5 的重复逐字段表**（与权威 §7 是同一份审计工作的两份拷贝，已按主从收口）——该表原有的标记随之消失；**索引表第 35–44 条不变**（那是本文独有的实核，仍有效）。
⇒ **两批都是"表格增删"，不是主张增删** ⇒ **B 档两次均未变**（45 / 14）。**A 只随呈现形态浮动，B 才是主张数的口径。**
...
- **〔本批〕与权威侧的逐节对拍收口（11 节：8 重叠 / 3 不重叠）—— 主从表见文档头**：
  - **① 双份实核**：本文 §3.4.5 与权威 **§7.2** 各自独立做过「七字段实现距离实核」⇒ **同一份工作的两份拷贝，必然漂移**。
    **处置**：**主副本 = 权威 §7.0–§7.4**；本文**删去重复的逐字段表**，只留**执行映射侧的两条结论**
    ＋ **一处对本文 §9.2 有直接后果的引用**（权威 §7.4「位预留了，但记录里没地方再放别的」——**其更严，本文采其口径**）。
    另：本文原表对 `persistence` 判「槽位在、值恒默认」，权威判「**无独立字段**」⇒ **取权威口径**。
  ...
  - **⑥ 三节不重叠的处置（本文判断）**：
    ...
    - **§4.3（式的判据形状/钉形）** ⇒ **引**：与本文 §八⑧ 的**腿位互补**（钉形 = 该式自己的测法；腿位 = 不变量族内的两向钉子），**已互相注明不替代**。
    - **§10（兼容性声明）** ⇒ **引**（文档头主从表），本文侧**不另立**。
- **〔末批〕复核收口（2026-09-20，两条，均为"同一命题两处说法"）**：
