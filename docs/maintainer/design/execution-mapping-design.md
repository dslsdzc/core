...
> （表题「**字段级定稿(七字段)**」，四列 = 语义 · 取值域 · 谁写 · 谁读，另附两条蕴含关系）。
...
15. ~~**`docs/maintainer/design/materialization-space.md`（existdoc 的产物）落盘后的接口面**——…**未核**该文档实际用的字段名/层名是否与本文 §3.4.2 的七字段一致。~~
    ...
    **实测**：其 §2.3「字段级定稿(七字段)」给出**取值域**两列（语义 · 取值域 · 谁写 · 谁读 + 两条蕴含关系），
...
| 档 | A（出现次数） | B（去重条数） | 说明 |
|---|---|---|---|
| 〔已实现〕 | **41** | **45** | B 档每条带 `file:line`，见下清单 |
| 〔已设计未实现〕 | **29** | **14** | B 档每条带出处文件 |
| 〔提案〕 | **48** | 其余全部（**默认档**） | A 口径只数**显式写出**的标记；**未含**默认推断档（散落各处、无标记即为提案） |

⚠ **B 档本批未增、A 档本批 +2/+2/+3 的原因（口径说明，避免被读成漏登）**：本批给 **§3.4.2 七字段表**加了
「本文三态」列（7 行各带标记）——**该表是 §3.4.5 逐字段距离实核的摘要视图，其逐条依据都已在索引表第 35–44 条里**，
故**不重复登记**为新索引行。⇒ **A↑ 而 B 平** 是本批的预期形态，不是漏登。
...
- **未核实清单**：§十二 十五条（第 15 项 = 与 `materialization-space.md` 的命名对齐义务 —— **本批已结案**，见下）。
