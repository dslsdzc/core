...
1. 全新自主语言：自主语法与语义设计，独立于现有语言生态。
2. 内置语法检查：编译前完成全部语法与静态语义校验，规约文件引用错误在编译期即被拦截。
3. 双轨语义 IR：实现层 IR 承载程序语义，规约层 IR 独立存储行为约束，两者通过符号引用关联。
4. 多平台轻量代码生成：后端设计简洁，新平台接入成本低。
5. 模块缓存与增量编译：语义 IR 可跨单元、跨平台复用，增量构建速度快。
6. 形式化验证基础设施：验证工具直接消费双轨 IR，无需重建语义模型。
7. 无模式统一执行：代码结构决定图结构，图结构决定执行方式。
8. 完整教学路径：同一门语言覆盖从入门到系统编程的全阶段。
