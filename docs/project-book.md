...
### 3.2 规约层：行为约束的图上标注（**2026-09-16 文档审计修订**）

> ⚠ **本节整段已改写**：原按「规约 = 独立的 `.corespec` 源文件 + 独立的 `.corespecir` 规约层 IR」叙述，
> 与 **ADR-0001**（2026-09-06，提交 `582a1e1`）**冲突**——`.corespec` 独立格式已**退役**、`.crasm` 同构退役；
> 原义（两类文件 / 两份 IR 并行）**不再是设计**，保留于本注内供追溯。

形式规约不是注释，也不是独立格式的源文件——**规约 = `.cr` 内联标注**（`#check` / `#ensure` /
`spec fn` 等，2026-09 语法定稿，见 `docs/maintainer/design/spec-design.md` §四/§六），
编译时**并入同一张图**（HDFG）成为**约束标注**（`.csr` 的 TagNode；设计见
`docs/ir-schema/corespecir-schema.md`）。规约表达式限于纯逻辑运算：无副作用、不可调用非纯函数。

**一个 Core 项目只有一类源文件**：`.cr`（实现 + 可选内联规约）。

两类 IR 的关系随之变为**同一份载体内的分层**：

- 实现层：`.cir`（HDFG，程序做什么）→ `.ccr`（格形态，v9；落盘载体）。
- 规约层：**图上的约束标注**（TagNode 元数据；`.csr` 为其序列化设计——**当前零实现**，
  见 `docs/ir-schema/corespecir-schema.md` 头部修订注）；其中每条约束指向图中的对应实体
  （节点索引/符号引用），不是另立一份并行 IR。

**原文保留（历史叙述，勿据以实现）**：
> ~~一个 Core 项目由两类文件组成：实现文件（`.cr`）…规约文件（`.corespec`）…两类文件分别通过语法检查器处理后，
> 生成两份独立的 IR——实现层 IR（`.ccr`）…规约层 IR（`.corespecir`）…~~
