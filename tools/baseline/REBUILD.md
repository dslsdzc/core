...
**暖态判据（腿 ③，#2026-09-15-5 T3）**：`tools/baseline/warm_leg.sh` —— 逐档「**同路径二跑**」× `ccr` 面
...
- **适用范围限制（跨二进制暖态判据，2026-09-16 (A) 批登记）**：暖态判据现比对 = **rc + 诊断码集 + 产物 sha**，
  **不覆盖日志文本**——**跨二进制**比日志会把 frozen 侧 `lower to ccr…` 与当前侧（FC 批硬闸 `main.cr:616-621`
  ...
  ⇒ 暖态判据判据标准 = **冷态逐档 + 暖态自洽（FAIL=0）**；**不按跨二进制日志文本判红**。与上条（#2026-09-15-8：不覆盖
