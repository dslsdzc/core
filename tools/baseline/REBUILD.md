...
拿它乘**当前源**，与**当前二进制**乘**当前源**逐档比（**76 档** `check` 的 rc + 日志；档数=**实读枚举**：
`tests/suite/*.cr` **36** + `src/compiler/{main,_import}.cr` 2 + 后端/内核单元 15 + `src/stdlib/*.cr` 19 +
`examples/*.cr` 4 = 76，与 `parity_run.sh` 的 `CORPUS_TOTAL=76` 硬断言一致；2026-09-17 批 6 T6 实测 t1=**35**，
**2026-09-18 批8 2(a)（#132）加 `tests/suite/cli_view_test.cr` ⇒ t1=36；本行与 runner 同批同步（2026-09-24）**），
差异 = 该批对判定面的**行为变更**（预期内 = 收紧/放宽，须逐条入清单；预期外 = 回归）。
...
- 时长实测（2026-09-15，本机）：牙齿层 **0.9s** · probes 广度层 **+3.5s** · parity 广度层
  **+125s**（**75 档** ×2 次 `ccr`，大文件为主；**⚠ 该值对含 `src/compiler/main.cr` 的大文件档不成立**——
  ...
  广度层留手工判据。
- **判据本体不进 CI**（维护者裁定 2026-09-15）：与套件**同语料**、覆盖重叠，收益仅「校验 runner 自身」。
- **适用范围限制（W3 批实测登记，2026-09-15）**：本判据现比对 = **rc + 诊断码集 + 产物 sha**（`ccr`/ELF/DOT 三面），
  **不覆盖 DOT label / 日志文本**——实测存在「暖态名字面」分歧（`ir_gen` 期合成名字串在暖态缺席 ⇒
  DOT 变量名标签掉前缀；**非静默面、不传导产物**，见 TODO #2026-09-15-8）。**若将来要把名字面纳入判据**，须扩比对约定
  （例如 DOT 逐行 diff 或 name 面专用通道）——**本批不实施**。
- **适用范围限制（跨二进制暖态腿，2026-09-16 (A) 批登记）**：暖态腿现比对 = **rc + 诊断码集 + 产物 sha**，
  **不覆盖日志文本**——**跨二进制**比日志会把 frozen 侧 `lower to ccr…` 与当前侧（FC 批硬闸 `main.cr:616-621`
  ...
  ⇒ 暖态腿判据标准 = **冷态逐档 + 暖态自洽（FAIL=0）**；**不按跨二进制日志文本判红**。与上条（#2026-09-15-8：不覆盖
  DOT label / 日志文本）**同一条适用范围限制**，本条只补「跨二进制」这一维（**不另立条目**）。
- **何时该跑（手工判据纪律）**——下列任一情形**必跑** `warm_run.sh`（0.9s）**加**至少一次
  ...
  日常 CI 已由最小面覆盖语料本身，**不**替代上述判据。

### 命令层陷阱（**假红**——2026-09-16 #2026-09-16-16 批 2 T1 实测登记）

跑本目录的 runner / 手工探针时，有两条**会产出假红**（让人觉得「命令跑了、结果是坏的」）的坑。
两者**叠加**时表现为「整批探针全失败」——#2026-09-16-16 T1 首轮 8/8「失败」全是这样来的：

- **`noclobber`：重定向到**已存在**的日志文件 ⇒ 该重定向失败、**命令根本没跑**，`tail`/`grep`
  读到的仍是**上一轮**的旧日志。**How to apply**：换新路径或用 `>|`；**每次跑判据都核对日志时间戳**
  （「旧日志被当成新结果」是第二次踩到：首次在 #2026-09-16-31 批 CI job 轮询）。
- **`corec build` 必须在仓库根跑**：在 `/tmp` 等仓库外目录跑 ⇒
  `error: cannot locate src/runtime/rt.cr`（构建要读仓库内的 runtime 源），rc=1；
  与「cwd 决定 import 解析」（在 `/tmp` 跑探针会得到假 139）**同族**。

**Why 单列**：这类陷阱产出的是**假红**，而假红与假绿一样有害——它会让人去「修」一个不存在的缺陷，
或把**真**红当成环境噪声放过。**How to apply（通用）**：判据命令的**运行环境刻意化**
（cwd = 仓库根、受控 `HOME`、逐档 `clean-cache`）+ **日志路径唯一化** + **每次核对产物存在性**。

## 换代纪律（**只在此处更新白名单**）

1. 白名单换代 = 「基线世代」变更（例如尾批收官后，下一批以本批收官为 pre-侧）。
   **不得**因为重建 sha 不符就改白名单对齐——那正是本配方要抓的「构建不可复现」。
2. 换代须**同批**记录：旧三 sha（含其源修订）+ 新源修订 + 理由 + 出处提交；旧值不删（清单留痕）。
3. 换代后重跑一次**对拍自证**（新基线 × 当前源 vs 当前二进制 × 当前源）并把结果记入该批报告。

## 已知限制（不静默）

- **CI 内不跑**：`.github/workflows/core-ci.yml` 用 `actions/checkout` + `fetch-depth: 2`（浅检出，
  无 `jj`）⇒ pinned revision 在 CI 环境**不可得**。本判据为**手工判据**（每任务/每批收尾跑），
  故入仓的是**可复现性**而非 CI 挂钩（R2 P6 Task 1 §Step 4 评估结论）。
- **75 档语料的既有 rc=1 不是回归**（2026-09-17 批 6 T6 **实测**：`36×0 / 39×1`）：
  **〔2026-09-24 加注（原文保留）〕语料已增至 76 档**（#132 加 `tests/suite/cli_view_test.cr`）⇒ 本行的
  `36×0 / 39×1` 是 **75 档时**的实测值；**76 档的分布与划界**须在下次采基线时重采。
  **本批实测（2026-09-24）**：用 `core-ctfeas-ws` 的二进制（**2026-09-23 07:49 构建 ⇒ 不是 develop 当前二进制**）
...
- **`.ccr`/`.cir` 缓存态**：比较任何产物前 `clean-cache`（runner 已逐档做）；`.ccr` 记录值引用须带
  命令形式 + 缓存态（TODO #2026-09-11-9 / P4 附录 D-1）。
- **`.ccr` 记录值还依赖 `$HOME` 内容（2026-09-16 判据载体化批 CI 红档实锤）**：编译器解析 `import`
  ...
  兜底，置空反会去读原开发者家目录）。根因已独立登记（TODO #2026-09-16-20/#2026-09-16-21，与「缓存键缺编译器身份」同族）。
  注：本条的**两档语料**中 `ptr_arith` 零 import ⇒ 不受影响；受影响的只有 `generics_test`（两种形式）。
  **2026-09-17 更新（第 4 批 `home-repro` 已修，TODO #2026-09-16-20/#2026-09-16-21）**：索引**不再影响产物**——
  `import` 期只落**侧表**（不 `str_intern`/不追加 SYM），仅**被引用**的名字在首次查找时物化，
  且 `.cr` 声明优先（宿主文件不得改写源码声明的含义）。⇒ 载体/手工复跑**不再需要**靠 `HOME`
  归一化来规避本条（归一化**仍保留**，作双保险）；`HOME` unset 亦不再读任何家目录（硬编码
  兜底已删）。**判据 = `tests/selfhost/test_so_index_repro.py`（8 例，已挂 `selfhost-tests`）**：
  两态 `.ccr` 两种形式逐字节同 · 良性索引零足迹 · 关键面（索引独有名字）不退化 · 索引行数
  不再是行为分界（N=2/200/2000 三档产物同且 rc 正常）· `HOME` unset ≡ 空 · ELF 跨两态同。
