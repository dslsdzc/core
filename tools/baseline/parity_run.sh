#!/usr/bin/env bash
# R2 P6 Task 1（E-13）：语料同源对拍 runner —— 73 档 × `check`，逐档 clean-cache。
#
...
#   · `shopt -s nullglob` + 计数断言 73（2026-09-16 全局 seam 批 +1：`tests/suite/global_seam_test.cr`；t1 32→33）：未匹配 glob 不再产生**字面 glob 伪条目**
...
CORPUS_TOTAL=73   # = t1 33 + t2 2 + t3 15 + t4 19 + t5 4（R2 P6 T0 表 C 实核；2026-09-16 全局 seam 批 t1 32→33）
