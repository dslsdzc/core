...
### 84. ⚠ **【判据强度】HIT 表事件 1-4 的字节判据只比前 2 字节**（2026-09-16 判据强度审计 B1——**弱判据，本批不修**）
...
### 85. ⚠ **【判据强度】慢路径块体只有 2B 锚**（2026-09-16 判据强度审计 B2——**弱判据，本批不修**）
...
- **状态**：**登记，未修**；修法同 #84（块体长度公式 + 定长窗口断言）。

### 86. **【判据强度】未用哨兵字段检查 = 4 字面量黑名单**（2026-09-16 判据强度审计 B3——**弱判据，本批不修**）
- **现象（【实核】`tests/selfhost/test_hit_table.py:284`）**：`bad = [s for s in ("dst=255","s2=255","dst=-1","s2=-1") if s in out]`——只排 4 个字面量形态。
...
- **状态**：**登记，未修**（与 TODO #65「暖态名字面」同族——该面已归因；本条 = 其**判据面**缺口）。
...
### 89. ⚠ **【结构性·高危】现存最强的字节判据全部未挂 CI**（2026-09-16 判据强度审计——**挂点缺失，本批不修**）
- **事实（【实核】`tests/harness/ci_hook_allowlist.txt`）**：`test_backend_bootstrap.py` **未挂（`：19` 自标「最高危」）**——**唯一**拦「清单双注册漂移（project-mode `error[N06]` 静默）」之门，且已致 **P3a 33×N06 真实漏检**（TODO #36 段）；`test_hit_table.py`（`:25`）与 `test_mw_task1..6.py`（`:30-35`）同族**未挂** ⇒ **判据强度最高的那批恰恰不在机器判据网内**。
...
- **建议批序**：① 低成本静态 guard（ent_kernel 面）先挂；② `test_backend_bootstrap`（**N06 唯一门**）实测时长后挂；③ mw_task 族 = 口径换代批（先裁基线入仓 vs 结构断言）。
- **状态**：**登记，未修**。
