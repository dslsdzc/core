...
### 56. ⚠ 指针写可选槽 × 表示位/规范化互斥：`*p = Some(9)` **双路径分歧的静默错值**（容量批 T2 登记——**今天就存在 · 预存 · 本批不修**）
...
- **待维护者**：① TODO #60（预存缓存静默缺陷 + runner 暖态腿建议）→ 独立批候选；② **环境干扰事件**：`build/` 被外部删两次 + `examples/*.{ccr,cir,o}` 等 **12 个 tracked 文件**被删（工作副本 `D`）——**未提交、未还原**（铁律 #3），提请处置。
...
- **疑似根因（待实证）**：`.cir` 快照只落指令/节点，而 provenance 侧表（`g_pts`/`g_offsets`/`g_pa_alloc_nodes`）在 `ir_gen`/`ptr_analysis` 期建立 ⇒ 命中路径未重建/未重算 ⇒ 安全检查看不到事实。
- **为何至今未暴露**：全部 runner（`parity_run.sh:35` 等）逐档 `clean-cache` ⇒ 判据面看不到。
- **处置**：**另起批**（本批章程 = 闸门反转；修它要动缓存/分析面）。退出条件 = 冷/暖两态诊断一致 + 全语料对拍（含暖态腿）。
