...
### 96. ⚠ **【静默·预存】`as` 值转换 = 静默重解释（非转换）**（**编号说明**：原取 #94，与判据网加固批 PR #85 的 #94/#95 撞号 ⇒ 依维护者 2026-09-16 指示改取 **#96**；#95 同理改 #97）（2026-09-16 维护者实测 + 本代理自验——**非 #93 批范围，仅登记**）
...
- **状态**：**登记，未修**（**非 #93 批（实参推断缺失）范围**；用户对「是否现在就派小批」未回话）。

### 97. **实参位泛型调用用「退化实例键」实例化**（2026-09-16 #93 批 T1 实测——**本批附带修复**，非「另有批」）
...
- **根因**：ir_gen 的泛型重定向有两条路径（`ir_gen.cr:1931-1984`）——**优先**走 checker 登记的调用点绑定段（`g_gen_binds` ⇒ `inst_key_of_ti` **精确类型项键**，`:1939-1960`）；**兜底**（`bstart < 0`）按**实参 IR 变量类型拼名串**（非原生 → 兜底名，`:1962+`）。而**实参位**的调用点因「已解析直调的实参不被推断」（TODO #93）**从未获得 `g_gen_binds`** ⇒ 永远走兜底 ⇒ **非原生实参退化为 `unit`**。
- **后果（**未追**，明写）**：若实例体**依赖 `T`**（返回/参数类型），实参位的调用**可能拿到错误行为**。本批探针的 `idf` 返回常量 ⇒ **不可观测** ⇒ **未追**（不当断言）。
- **状态**：**本批（#93）附带修复** —— (a) 补实参推断后，实参位调用点获得 `g_gen_binds` ⇒ 自动改走精确键路径 ⇒ 实例名 `idf[unit]` → `idf[P]`。
- **判据（本批 T4 腿 D，**正据**）**：同一泛型调用在**实参位**必须得**精确键**（`idf[P]`），不得是退化键（`idf[unit]`）。实现 = 跑 `cir` 断言实例名在场/不在场。
- **关联**：`TODO #93`（实参推断缺失——本条是它的**下游表现**）· `docs/superpowers/plans/2026-09-16-arg-inference-fix.md` §3ter (3ter)(d) + §5 腿 D + 裁-ARG-8（H3/H4）· `canary_values.tsv` 头注（**本代 `.ccr` 变化根因 = 实参位泛型实例键由退化键转精确键**）。


### 99. ⚠⚠ **【编译器自身·静默内存破坏·高优先】`check_all` 的 SO_FN 保全缓冲容量硬编码 128 且写入无界**（2026-09-17 第 4 批（#82/#83）侦查实测——**比 #82/#83 更底层**，建议并入该批）
...
- **与 #82 的关系（**并入判断**）**：若 #82 取「侧表 + 惰性物化」（本批计划 §2 方案 B），则**该保全块整体变死码**（惰性物化发生在 `g_sym_count` 复位**之后**，复位前表内无 SO_FN ⇒ 存取皆为 0）⇒ **整块删除 ⇒ F1 自然消失**。⇒ **建议并入本批**（作为方案 B 的副产品），否则须单独立项做动态扩容。
...
- **关联**：`TODO #82/#83`（同一段代码：索引行数 → SO_FN 条目数 → 本缓冲）· 前批「静默错值」家族。

### 100. **`.so` 扩展索引与 stdlib 声明面命名漂移**（`print_i` vs `print_int`）——幽灵条目来源（2026-09-17 同上挖出）
- **编号说明**：原取 **#99**，同上撞号避让 ⇒ 依维护者指示**改取 #100**。
- **现象**：`src/stdlib/io.cr` 声明的是 `print_i`（`:19`）/`println_i`（`:23`），而本机 `~/.core/lib/io/index` 写的是 `print_int`/`println_int` ⇒ **索引描述的是另一套命名**。实测后果：Canary 语料 `generics_test.cr` **从不引用**这两个名字，但它们仍被 `str_intern`（`module.cr:385`）驻留进 STR ⇒ **+28B**（= #82 的直接来源；前批 PR #80 红档即此）。
- **性质**：**生成器与声明面不一致**（索引由扩展方生成、无一致性校验），与 #82/#99 同族（同一段代码的三个面：产物泄漏 / 内存破坏 / 命名漂移）。
...
- **关联**：`TODO #82`（泄漏面）· `#99`（内存面）· 本批计划 §2 方案 C + §6 附带发现 F2。

### 101. ⚠ **【静默·预存】调用「索引独有名字」+ `--static` ⇒ 构建 rc=0 但运行 139**（2026-09-17 第 4 批实测挖出——**改前改后一致，非本批引入**）
...
- **关联**：`TODO #82`（同族：SO_FN 的产物面语义）· F16②（`corearch.cr:571-580`，本条的守卫缺口）· 第 4 批计划 §6「预存面」。
