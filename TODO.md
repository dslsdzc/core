...
- **暖态腿（T3）**：入仓语料 `tests/probes/warm/`（7 档，**定路径 = 关键设计**，M3 实证）+ `tools/baseline/warm_leg.sh`（两层）/`warm_run.sh`；CI 只挂最小面（套件 10 例 / 0.5s）；**腿本体手工判据**，「何时该跑」四条见 `tools/baseline/REBUILD.md`。暖态生效档数实测 = 牙齿层 7/7 · parity 广度层 40/72 · probes 广度层 15/29。
...
- **判据网限制（同处登记）**：暖态腿（`tools/baseline/warm_leg.sh`）现约定 = **rc + 诊断码集 + 产物 sha**，**不覆盖 label / 日志文本** ⇒ 本类名字面分歧**不会被腿抓住**；若将来要覆盖名字面须扩约定（**本批不实施**）；约定段见 `tools/baseline/REBUILD.md`。
...
- **兜底**：**无**（该腿是「untagged 快路径字节不变」的**唯一**证据）。静默 SKIP 那半**已于 05:58 改 fail-closed**（`--allow-skip` 显式才降级，缺基线默认 FAIL）；**仍未挂 CI**（`tests/harness/ci_hook_allowlist.txt:31`：零 diff 腿的基线须在**改动前**编译器上产 ⇒ CI 参照物结构性不可得，属标准换代）。
...
- **〔更正注（2026-09-18 · `plan-verikernel` 核查补；**原文逐字保留**）〕** 本条内**四处**仍写着旧约定，按计划 §1-1 同一约定逐处标注（**原文一律未删**）：
  ...
  **现行约定** = 计划 §1-1 T0 回填块 ②（Ⅰ interp 值 + `.cir` 调用点 + `--dump-params` 槽型；Ⅱ ELF 仅作「≠7 + N≥20 次不复现 7」弱腿）；探针 `g(1,d)` **应被拒**（`T` 由首实参绑为 `int`）。
...
- **判据要件（修复时）**：① **两腿同判**（ELF 与 interp 不得分歧）；② **禁第三态**（apx 源 interp ∈ {255（能力边界）, ELF 锚定值}，出现第三值即红）；③ `scaled` 实参对照 + **非泛型直调对照**保持 7/7（非回归断言）；④ **apx 槽自证**（`.cir` 出现 binary64 位常量）防「探针不触发」；⑤ 若同批改语义面 ⇒ `CIR_CACHE_VER` 换代 + 标记值按「显式归因 + 同批重锁 + 旧值留痕」。
- **〔更正注（2026-09-18 · `plan-verikernel` 核查补；原文逐字保留）〕** 上条判据要件 **②** 的括注「`{255（能力边界）, **ELF 锚定值**}`」中，
  **「ELF 锚定值」半支作废**——ELF 腿**非确定**（分布样本：97 次运行命中 `142` 仅 1 次；复跑 60 次 0 次；关 ASLR 稳定 `rc=208`）⇒
  非确定值域**无「锚定值」可言**。**interp 半支保留**（确定值域）。现行 = 计划 §1-1 T0 回填块 **Ⅳ / Ⅳ-bis**：非确定值域改判「**必须可复现**」；
  `255` 只属 interp 腿。另：要件 **④**（apx 槽自证）已随机理推翻作废（见上「⚠ 更正注」）。
...
- **〔更正注（2026-09-18 · `plan-verikernel` 核查补；原文逐字保留）〕** 本条已按新约定对齐两处：**(1)** 现象段的 ELF 读数
  ...
  **(甲) 修复后应为 `rc=1` 被拒**（要「得 7」需另裁推断方向，单独立条）；现行判据形态 = 计划 §1-1 T0 回填块 ②（Ⅰ interp 值 + `.cir` 调用点 + `--dump-params` 槽型；Ⅱ ELF 仅作「≠7 + N≥10 次不复现 7」弱腿）。
...
- **约定更正（本批最重要的发现之一）**：在飞分支（b8）上那条「**6 档异常**」登记，是**朴素枚举循环**（`for f in tests/suite/*.cr; do corec build/run`）的读数，**不是 `run_suite` 腿的读数**——真实 runner **本就跳过**它们（`src/ci/run.sh::run_suite`：① mini 族负例 SKIP ② `[ ! -s "$f" ]` 0 字节 SKIP）⇒ 「6 档异常」**在套件腿上从未出现**。该登记号在未合入分支上，本条目只记录更正、不替其改写。
...
- **判据要件（实施时）**：`"c=" + ch` 两腿（native/interp）同判 + 锚定值；`str_len(ch) == 1` 对照；
...
- **判据要件（实施时）**：`"v=" + if b {…} else {…}` 两腿同判 + 锚定值 · `x := if b { 1 } else { 2 }`（let 初值位）
