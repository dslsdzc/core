...
### 98. ⚠⚠ **【编译器自身·静默内存破坏·高优先】`check_all` 的 SO_FN 保全缓冲容量硬编码 128 且写入无界**（2026-09-17 第 4 批（#82/#83）侦查实测——**比 #82/#83 更底层**，建议并入该批）
...
### 99. **`.so` 扩展索引与 stdlib 声明面命名漂移**（`print_i` vs `print_int`）——幽灵条目来源（2026-09-17 同上挖出）
...
- **性质**：**生成器与声明面不一致**（索引由扩展方生成、无一致性校验），与 #82/#98 同族（同一段代码的三个面：产物泄漏 / 内存破坏 / 命名漂移）。
...
- **关联**：`TODO #82`（泄漏面）· `#98`（内存面）· 本批计划 §2 方案 C + §6 附带发现 F2。

### 100. ⚠ **【静默·预存】调用「索引独有名字」+ `--static` ⇒ 构建 rc=0 但运行 139**（2026-09-17 第 4 批实测挖出——**改前改后一致，非本批引入**）
