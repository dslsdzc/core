...
> （「调用侧环按泛型声明、callee 槽按实例 ⇒ 口径不一致」）**仍是已被 T0 实测推翻的旧机理**；
...
| — | `#2026-09-19-1` | 2026-09-19 | 批 8 (乙) 影响面枚举实测 | 【语义·定性待裁】`&x` 产出 `*int`、`&T` 声明位是 `TYP_REF`——**调用点该接受还是该拒**？（`TA02` 诊断自证 `&x` = `*int`；S1P 台账 PTR→REF 恒报 / REF→REF 恒 0 ⇒ 成因 = kind，**行重复已排除**；冷/暖两态零诊断 ⇒ **非** `#2026-09-15-5`）；含五层影响面（契约本体/自动解引用/指针算术/引擎身份类+变型/两道安全门） |
| — | `#2026-09-19-2` | 2026-09-19 | 批 8 (乙) 复核实测 | 【能力缺口·漏分支】`res_call_type` 无 `EXPR_PTRTYPE` 分支 ⇒ **`*T` 形参在调用点解析成 `TI_UNIT`**（台账 `p_ti=4`）；**含仪器限定：`*T` 形参的 `p_ti` 是垃圾值 ⇒ 该类读数不可作 type 推理** |
| — | `#2026-09-19-3` | 2026-09-19 | 批 8 (乙) 实测立 | 【读数纪律·两条】S1P 扫描口径（只覆盖直调实参 ⇒ **行数是下界**）· **「0 行」有歧义 ⇒ 读 0 前必须先跑活性自证** · **台账 never 跳过**（不得因「该档无行」跳过；须记「探过 + 自证通过」）· **第二盲区 = 整型字面量实参不入账**（`f(y)` 报 / `f(7)` 不报，三形参皆然）· 活性自证注入点**必须自身会响** · 并发面实测 **7 档 / 29 点**（旧「24」漏扫 `conc_test.cr`，作废） |
| — | `#2026-09-19-4` | 2026-09-19 | 批 8 (甲) 刀 3 + `plan-optdex` | 【A 类产出侧·两前端分歧】`EXPR_GO`：checker 说 `int`、`ir_gen` 自己的注释说 channel ⇒ 句柄产出侧未对齐（24 点/2 形全静默，**下界**）；含 `await` 耦合与**为什么改定型不可行**（变量中介 ⇒ AST 救不了 ⇒ 需 `Future<T>` = 语言设计）；第三个真源 = `bi_add("sched_go"/"g_get_curg", TI_INT)`；两向钉子（**反向必须变量形**，字面量被 skip 规则 ③ 吞） |
| — | `#2026-09-19-5` | 2026-09-19 | 批 8 (甲) 刀 3 + `plan-optdex` | 【内建表 callee 实参面】**登记「免检」契约**：`bi_add` 只有返回型 ⇒ 内建回退无 `FuncInfo` ⇒ 环早退；口径三条（**未被 `.cr` 遮蔽的条目** / 22 是这张表的口径 **非内建口径** / 类型信息在**返回面**）；否 (甲) 主因 = 合成 `SYM_FN` 会动 `.ccr` SYM 段 ⇒ canary 需重锁 |
...
- **〔2026-09-19 · `b8-e1` 追加（只增不删）〕检索提示**：本条在**编号迁移对照表**中的那一行，其**标题摘要为旧机理**（「实参形式转换与槽位形式两侧口径不一致」）——**以本条目正文的〔更正注〕为准**。两处各说各的更稳：**正文是检索第一落点**，表侧摘要只作历史留痕。
...
- **〔更正注（2026-09-18 · `plan-verikernel` 核查补；**原文逐字保留**）〕** 本条内**四处**仍写着旧口径，按计划 §1-1 同一口径逐处标注（**原文一律未删**）：
  ...
  **现行口径** = 计划 §1-1 T0 回填块 ②（Ⅰ interp 值 + `.cir` 调用点 + `--dump-params` 槽型；Ⅱ ELF 仅作「≠7 + N≥20 次不复现 7」弱腿）；探针 `g(1,d)` **应被拒**（`T` 由首实参绑为 `int`）。
...
- **〔更正注（2026-09-18 · `plan-verikernel` 核查补；原文逐字保留）〕** 本条已按新口径对齐两处：**(1)** 现象段的 ELF 读数
...
> - **又一处「判据钉住了缺陷」（今日第四例）**：`tests/selfhost/test_cir_warm_path.py` 的 STR 契约原来写「暖 = 冷**前缀**
>   （**预存口径**）」——**那条判据过去一直把本缺陷当口径**（暖态串表更短正是缺陷症状）。本批重定为「冷 == 暖逐条逐字节同」
...
- **修复**：插值分支**自行推进到位并以 `continue` 收口**（绝不落到循环尾 `_pos += 1`）；扫过的原文**逐字进串值**（= 本前端不展开插值，与 bootstrap 词法一致）。
- **判据（入 `selfhost-tests`，22 断言）**：`tests/selfhost/test_string_interp.py` = **A** 解析完整性（插值**同一行**含 `}`/后续语句 ⇒ 无 P21 + rc 判据）· **B** 8 形态原文取值（对照/变量/表达式/多段/相邻/嵌套调用/嵌套花括号/转义混排）· **C** 与 **bootstrap lexer lexeme 跨前端对拍** · **D** 分支 `continue` 收口守卫。
...
- **⚠ 未覆盖面（按裁定登记，理由 = 实测而非「暂不支持」）**：① **`char` 洞 ⇒ P028**——char 运行期值是**字符串**、
  ...
  ③ **泛型洞 `T` ⇒ P028**——洞型在实例化前不可得（与 (乙) 泛型擦除同族），判据已钉（`C.generic_hole.reject`）。
...
- **现象（实测）**：`src/compiler/globals.cr` 里 `g_s1p_on : int, mut = -1;` ⇒ 构建出的 `build/corec` 里该初值**不存在**：装配为 `_g_g_s1p_on: .quad 0`，运行时读到 **0**（探针打印 `on0=0`）。⇒ 「未初始化 = 负哨兵」判据**恒假**，懒初始化/开关类代码**静默失效**。
...
- **本批处置**：**不修 bootstrap**（属另一条链）；受影响的写法已在本批改为**零值哨兵**（`g_s1p_seen`，`#122`）。**实测 `globals.cr` 现无其它非 Literal 全局初值**（全表只此一处）⇒ 无既存损伤，纯属**未来雷区 + 判据失效源**。
...
- **后果（本批当场踩到）**：S2 原按「有现成显式形式就用现成的」选了 `as` 做 `string↔int` 显式化 ⇒
  **`check` job（走 bootstrap 构建）当场 rc=1** ⇒ 全部回退（台账回基线 239 点，逐点一致）。
...
- **实测读数**（逐字探针 `fn g(a: &int) -> int { return 0; } fn main() -> int { x : ., mut = 1; p := &x; return g(p); }`）：
  ...
  - **调用位点（`CORE_S1P=1` 台账）**：PTR 实参 → REF 形参**恒报 1 行**（`p_ti≠a_ti`）· **REF→REF（含经调用）恒 0 行** · 控制组 `int→int` 恒 0 行 ⇒ 成因 = **kind 差异**；**行重复已被排除**（REF→REF 的两处也是不同行号，却 0 行）。
...
- **五层影响面（若将来裁 (A)「`&x` 改判 REF」）**：
  ...
  ⇒ **不是「改 `&x` 的写法」，是改语言语义 + 引擎变型 + 两道安全门覆盖** ⇒ **必须单独立项 + 两向钉子**。
...
- **〔2026-09-19 · `b8-e1` 追加（只增不删）〕`TYP_PTR` 全仓 **53 处**引用按**符号锚**分类**（行号为**示例锚**、可漂移；清点法 = 全仓 `grep -rn TYP_PTR` 逐处读）：
  ...
  - **⚠ 判据里的期望值（最危险）**：`type_selftest.cr::type_selftest_run` 的 `:2012` · `:2778` · **`:2881`（**同一断言里**同时钉 `TYP_PTR` 与 `TYP_REF` ⇒ (A) 落地**必翻面）**，以及夹具 `:2316`/`:2317`/`:2464`/`:2824`/`:3192`/`:3193` 与 `ts_t5_run`（`:277`）。
...
- **〔同上追加〕排除面（显式登记）**：批 8 (甲) 的实参硬错**不得**对 `&x` 形态触发；排除出处 = 本条 + 批报告 + `#2026-09-19-4`。（「排除」= 落纸 + 带钉子；**没写进刀 4 触发条件的排除 = 静默，不算过**。）
...
- **〔2026-09-19 留痕：已裁（只增不删）〕状态变更——本条目标题的「定性待裁」已作废。**
  ...
  - **性质 = 改契约**：`&x` 产出 `*int` 是**文档化语义**——**编译器自己的用户可见诊断**就这么写
    （`error[TA02]: Variable declared as &int, got *int`）⇒ 按本仓口径**必须单独立项**（本条目即其载体）。
  ...
  - **落地面与判据**：`docs/superpowers/plans/2026-09-19-blade4-argface-harderror.md` **§8**
    （五层符号锚 + **三处将转红载体** + **§8.3 `(i)` 白名单抉择**）+ **§9**（两向钉子）。

### 2026-09-19-2. ⚠ **【能力缺口·漏分支】`res_call_type` 无 `EXPR_PTRTYPE` 分支 ⇒ `*T` 形参在调用点解析成 `TI_UNIT`**（批 8 (乙) 复核实测；取号基准 = develop 当天已有最大 N = 0）

- **实测**：`fn h(a: *int) -> int { return 0; }` ← `h(p)`（`p := &x`）⇒ S1P 台账 `9917 <line> <col> 0 4 10 h`：**`p_ti = 4 = TI_UNIT`**（常量序：`TI_INT=0 · TI_DEX=1 · TI_BOOL=2 · TI_STR=3 · TI_UNIT=4`）。
- **性质**：`*T` 形参在**调用点**的类型解析落空 ⇒ 退化为 unit = **纯漏分支**（非设计）。
- **⚠ 仪器限定（必读，避免后人重蹈）**：**`*T` 形参的 `p_ti` 是垃圾值（unit）** ⇒ **凡用 `*T` 形参做的台账读数，都不可作 type 推理**。
- **附（行号 face 局部性）**：**不同 face 的行号不可跨比**——同一探针 `check` 面台账给 `p=11 / a=10`，而 `ccr --dump-types` 的行表是 `9/10/11`；两面行号空间不同，**不可互推**。
...
### 2026-09-19-3. 【读数纪律·两条】**S1P 台账的扫描口径 + 「台账 never 跳过」**（批 8 (乙) 实测立；取号基准 = develop 当天已有最大 N = 0）

- **覆盖面（下界声明）**：S1P 只覆盖**直调实参**，**返回位不覆盖**（同族见 `#2026-09-18-19`(A)）⇒ **台账行数是下界**，不得当「错点总数」。
- **台账只记不匹配**（当日控制组实证）：`fn g2(a: int)` ← `g2(1)` ⇒ **0 行**；同型 `REF→REF` 经调用 ⇒ **0 行**。
- **「0 行」有歧义 ⇒ 必须先跑活性自证**：0 行在「**干净**」与「**仪器没跑 / 该面未覆盖**」之间不可区分（即「覆盖为零不会红、只会腐烂」那一类）⇒ 读 0 之前先跑一个**已知必然产生台账行**的探针，证明打点活着。
- **never 跳过**：扫描时**不得因「该档无台账行」跳过它**；须记录「**探过 + 活性自证通过**」才可写 0。**当日实例**：`tests/suite/go_test.cr` 台账 0 行，**未跑该档活性自证** ⇒ 其状态是「**未定性**」而非「干净」。
- **行号 face 局部性**：台账行号与 `--dump-types` 行号**只在同一次运行内可比**（同 `#2026-09-19-2` 附）。
- **覆盖面第二盲区（2026-09-19 补跑实测）**：**整型字面量实参不入账**——`f(y)`（int 变量）**报 1 行**，**`f(7)`（int 字面量）不报**；对照三形参（`string` / `bool` / `*int`）**皆不报** ⇒ 与实参**形式**相关、与形参无关。另两形实测**入账**：`f("x")`（string 字面量）· `g(&x)`（取址表达式）。⇒ 口径精确化为：**直调实参 ∧ 非整型字面量 ∧ 非返回位**（三处盲区：返回位 · 整型字面量 · 内建回退面见个人 memory 文件的「判据命令陷阱」条目（**仓内不可解析的跨仓链接已按 2026-09-19 裁定移除**））。
...
- **两档零台账已证清**（同档内容 + 注入点 ⇒ 命中数 = 1 = 注入点本身 ⇒ 该档自身 0 行）：`tests/suite/go_test.cr`（`go 42`，无直调实参）· `tests/suite/fnaddr_test.cr`。
- **扫描面自更正**：并发面语料实测 **7 档**（非 5）——多出 `tests/suite/conc_test.cr`（5 点）；**总台账 = 29 点**（25× `sched_enqueue` + 4× `chan_recv`，恒 `p=3 (TI_STR) / a=0 (TI_INT)`），分布 = `chan_test` 5 · **`conc_test` 5** · `go_e2e` 6 · `go_final` 6 · `go_parallel` 7 · `go_test` 0\* · `fnaddr_test` 0\*（\* = 已活性证清）。**「24」是漏扫 `conc_test.cr` 的旧读数 ⇒ 作废；A 类条目请用 29。**

- **〔2026-09-19 · `b8-e1` 追加（只增不删）〕术语澄清：本条说的是「扫描流程」，不是「打点代码的跳过条件」**
  本条的「**never 跳过**」= **扫描流程**规则：**不得因「该档无台账行」就默认跳过**；须记「**探过 + 活性自证通过**」才可写 0。
  与之**名字相近但不是一件事**的，是**打点代码内部**的跳过条件（拟增 `ati == TI_NEVER`，见本文末〔打点 skip 规则〕段）——**一个是流程、一个是代码**，**勿合并、勿按「已有一条 never 跳过」处理**。
- **〔同上追加〕权威扫描口径（2026-09-19 确立，替代逐档 standalone）**：① **project 单元优先**——`corec check src/compiler`（**40 档全闭包**）⇒ **0 点** · `corec check src/targets/x86_64-linux`（24 档闭包）⇒ **6 点**；② 闭包外只采**未定义诊断为零**的单档（`Undefined (name|function)` 双零）。**凡读数必须声明扫描单元**。
- **〔同上追加〕真面读数（全语料，project 口径）**：**63 点 / 6 类** = A 并发句柄产出侧 **49** · B 裸字表写串值 **2** · C 裸地址当 string **4** · D `@typeInfo` 两面分歧 **4** · E dyn/泛型形参 **2** · F 指针 REF/PTR **2**。
...
- **〔同上追加〕第三个伪影维度 = 内建回退（真机理，已用对照钉死）**：`checker.cr` 的 `bi_add(name, ret_ti)` **只有返回型、没有参数型**；`EXPR_CALL` 分支的**内建回退路径上没有 `FuncInfo`** ⇒ 打点环 `fi < 0` 早退 ⇒ **该 callee 的实参面一条都不报**。
  - **三组对照**：`w64` 同文件真定义 + 异型实参 ⇒ **报**；同形只剩内建（`ld.cr` 单档）⇒ **0 点**；跨文件 callee（`str_len` ← `import fmt`）异型 ⇒ **报**；单行多语句形状 ⇒ **报**。⇒ 边界钉死。
...
- **〔同上追加〕「仪器在前置条件不满足时静默跳过整块」（独立登记）**：`s1p_arg_ring` 开头 `if fi < 0 { return; }` ⇒ 台账在 standalone 语境下**对整个文件失明**，且**不报错、不提示**。⇒ 这是「**覆盖静默退化为零**」，与「**覆盖为零**」是两回事：后者**你知道没测**，前者**你以为测了**。（机理锚 = 该行；实测 = `ld.cr` 单档 0 点 / 拼接单元 2 点。）
- **〔同上追加〕打点 skip 规则 × 钉子对照表（本表整张进刀 4 判据档）**：
  | # | skip 规则（`s1p_arg_one`） | 会让哪类钉子**恒真空绿** | 钉子必须换成什么形式 |
  |---|---|---|---|
  | ① | `pti < 0`（形参型不可得） | 形参侧不可得的钉子 | 换形参 |
  | ② | `ati == TI_UNIT` | unit 实参的钉子 | 换实参 |
  | ③ | `ast_kind(arg_n) == EXPR_INT \| EXPR_DEX`（**整型字面量**） | **`chan_recv(42)` 形** | **改用 int 变量**（实测：`chan_recv(n)` 报 · `chan_recv(42)` 不报） |
  | ④ | `None → T?`（② 号规则） | `None` 实参的钉子 | 换实参 |
  | ⑤ | **拟增 `ati == TI_NEVER`**（未知 ≠ 异型） | 「未定义名」类**阴性**钉子 | **阳性钉子另立**（未定义名 ⇒ 台账**可记**） |
  | ⑥ | **内建回退（无 FuncInfo）** | 内建 callee 的钉子 | 用**真定义在场**的同名 callee（实测：真定义 ⇒ 报） |
  ⇒ **纪律**：写每颗钉子时**逐条问「我这颗会不会落在某条 skip 规则里」**；落在里面就换形式。
- **〔同上追加〕五条「哨兵/不可判定值」族与两族划分**：`TI_NEVER` / `TI_DYN` / 泛型形参型 = **哨兵/不可判定值**族（**未知 ≠ 异型** ⇒ 应 skip）；`TI_UNIT` 与整型字面量 = **另一种**（形式盲区）。**两族理由不同，勿并成一条。**
...
- **第三个真源**：`bi_add("sched_go", TI_INT)`（`checker.cr:299`）· `bi_add("g_get_curg", TI_INT)`（`:302`）——与 A₂ 把 `sched_go` 声明对齐成 `-> string` **是两个真源**。**用户 `FuncInfo` 优先**（`find_func` 先于内建表查名）⇒ **内建只在无真定义时生效**。
...
- **读数（`plan-optdex` 只读扫：5 档并发语料 × `check` + `CORE_S1P=1`，逐档 clean-cache）**：**24 点 / 2 形，全部静默（rc=0）** = 20 × `p_ti=3(TI_STR) a_ti=0(TI_INT) sched_enqueue` + 4 × 同形 `chan_recv`。
  ...
  - ⇒ **24 是下界**：S1P 只覆盖**直调实参**（**返回位不覆盖**，已知边界）⇒ 台账行数**不是错点总数**。
  - **未测（「没测到」≠「测到没有」）**：`go` 结果**跨函数返回** · `sched_dequeue` 返回值的调用点逐点 · `examples/` · stdlib 自身非独立入口（不可单独 `check`）。
- **血缘（引原文）**：`#118` = `cc8f1b836669`「**并发 handle 类型模型对齐 ⇒ TF01 真冲突收口（3 返回型 + 5 形参）+ 撤 TF01 豁免条**」；其裁定原文「**(A)（lead 2026-09-18）：语料/stdlib 向模型对齐**（**(C) handle 兼容明令禁止；(B) 改 `alloc` 模型亦否**）」。
...
- **(甲) 刀 4 的三条前置**：**① 台账跳 never · ② 本条（显式排除 + 钉子写进刀 4 触发条件）· ③ 刀 2 基点**。**并写明「`(甲)` 与 A 类不能各做各的」**——硬错一开，这 49 点会**由「静默」转「红」**，4–5 个并发语料档（含 CI 门内 `test_chan_handle_model.py` 覆盖的那批）**当场失败**，而它们**今天是绿的**。
- **验收判据 = 修后两向钉子（写进刀 4 触发条件本体）**：
  - 条件 = 「实参型 ≠ 形参型」**∧ 实参不属 handle 产出通道**；handle 产出通道 = **逐条列出**（`EXPR_GO` · `sched_go` · `chan_make` · `g_new` 的结果位）。
  - **正向钉子（防误杀）**：`chan_recv(go work())` 形 ⇒ **不报**（**现成 5 个语料载体**：`go_e2e_test.cr:17` · `go_final_test.cr:16` · `go_parallel_test.cr:18/19`）。
  - **反向钉子（防漏放）**：非 handle 的 **int 变量**传进 `chan_recv` ⇒ **仍须被硬错**：`n : ., mut = 42; chan_recv(n);`
  - **⚠ 反向钉子不得用整型字面量**：`chan_recv(42)` 落进 skip 规则 ③（`ast_kind(arg_n) == EXPR_INT || EXPR_DEX`）⇒ **恒绿 = 真空通过**（实测：`chan_recv(n)` 报 1 行 · `chan_recv(42)` 不报）。
...
- **〔2026-09-19 留痕：已裁（只增不删）〕状态变更——本条目的「显式排除 + 立项」处置已作废。**
  ...
  - **由此作废的旧处置**：「**本批显式排除该面**」、以及「(甲) 刀 4 三条前置」里的
    ...
    ② **实测证明「排除」这条出路不可实现**——A 类 49 点与真异型**逐字段同形**（`p=3(string) ← a=0(int)`，
       ...
       ⇒ 唯一能抑制 49 点的实现「按 (callee, param) 列全」**必然杀死反向钉子**（按构造不可能兼得）。
...
- **口径三条（**必须原样写，否则后人会重复误判**）**：
  1. **遮蔽面**：22 条里 **6 条有 `.cr` 真定义**（`r64`@`dyn_arr.cr:40` · `w32`@`dyn_arr.cr:9`/`elf.cr:17` · `w64`@`dyn_arr.cr:28`/`elf.cr:22` · `_dyncpy`@`dyn_arr.cr:252` · `fiber_init`@`stdlib/scheduler.cr:48` · `sched_go`@`stdlib/sched.cr:171`）⇒ 在相应单元里它们走**用户 `FuncInfo`**、**本就受校验**。⇒ 口径 = 「**未被 `.cr` 遮蔽的内建条目**」，**禁止**写「22 个 callee 实参面零覆盖」。
  2. **术语面**：22 = **这张表**的口径，**不是「内建」的口径**（`str_len` 全仓高频却**不在表里**，它是普通 `.cr` 函数）⇒ **禁止**写「22 个内建」。
  3. **反预期**：类型信息在**返回面**（`alloc`/`get_arg`/`load_str_ptr` 三条返回 `string`），**不在参数面**。
- **另**：`sched_call_0..4` **零 `.cr` 调用点**（只在后端被注册/发射）⇒ 补参数型**没有可保护的调用点**。
- **裁决记录**：取 **(乙) 明确登记「内建族参数面免检」这个契约**（**有契约 = 有意设计** ⇒ 按本仓口径**登记**，不是修；与「看起来要豁免 ⇒ 先查是不是某次迁移漏了尾」是**两步**：先查同形先例，本例**查不到漏尾**、是设计）。**否 (甲)**（给内建合成 `FuncInfo`/参数符号）的**主因**（**本代理已自查源码，非转述**）：`checker.cr` 的 builtin 注册环（注释原文 `Register runtime builtins as proper SYM_FN`，`if find_gsym(ni2) < 0 { … sym_set_kind(si2, SYM_FN); … }`，**符号锚**）**今天已经把每个未被 `.cr` 遮蔽的内建建成 `SYM_FN`** ⇒ (甲) 是在此之上**再增参数符号** ⇒ `g_sym_count` 继续增长 ⇒ **符号编号顺移 ⇒ `.ccr` SYM 段字节面可能变 ⇒ canary 需重锁声明**（还要过 D1 闸门），**为 2 条付此代价不成立**。
- **数据源（引用，不抄全表）**：`docs/superpowers/specs/2026-09-19-builtin-table-recon.md`（`plan-optdex` 产出，已在 develop；**保留其 §6 口径限定**）。
- **状态**：**登记**（口径 + 裁决记录）。
...
### 2026-09-19-7. ⚠ **【同一注解在形参位与局部位行为不同】`dyn` 局部变量注解 ⇒ 台账记；`dyn` 形参 ⇒ 不记**（批 8 (甲) 刀 3 + `plan-optdex` 实测；取号基准 = 3）

- **实测**：同一档同时含 `f[T](x: T)` 与 `h(x: dyn)` ⇒ **只记泛型那条**；`dyn` 作**局部变量**注解（有初值 + 再赋值）⇒ **台账记**（`a=7`）；`dyn` 作**形参** ⇒ **0 条**。
...
- **附带口径**：**ti ≥ 10 的是 per-unit 分配行 ⇒ 跨档不可比**（只能比「是否 > 9」）；与「台账行号是单元局部的」同族。
...
- **D 的两条现成对照（本代理实核，直接引）**：
  1. **「同一档里 `@fields` 不报」**：`tests/suite/at_test_struct.cr:10` 的 `str_len(flds)`（`flds := @fields(Point)`）**不报** ⇒ 正是「**不是整条通道坏了**」的现成钉子（D 不是「内建整体失配」，是**一个内建**的分歧）。
...
- **E 的落法（归属写死，不许悬空）**：E 的两条都**有契约**（`dyn` = ⊤ `ty_shadow.cr` 的 `TYP_DYN` 分支注记；泛型形参 = 既有策略「注解为泛型形参 ⇒ 声明期不可验证 ⇒ 不判」）⇒ **处置 = 排除**；但**排除必须落在 `s1p_arg_one` 的 skip 规则上**——**硬错与台账共用同一条环，只登记不改环 ⇒ 硬错打开时这两类会红**。
  - **由谁在哪一步做 = `b8-e1`，在 `(甲)` 刀 4 前置①「台账跳 never」那一小刀里一并做**：三条**哨兵族 skip**（`ati == TI_NEVER` + `ati == TI_DYN` + **泛型形参型**）**一次落**，并**同批**在判据档带齐**整张「skip 规则 × 钉子对照表」**（六条，见 `#2026-09-19-3` 追加段）。
  - **两族划分（勿并）**：`TI_NEVER` / `TI_DYN` / 泛型形参型 = **哨兵/不可判定值**族（**未知 ≠ 异型**）；`TI_UNIT` 与整型字面量 = 另一族（**形式盲区**）。
- **状态**：**本表即 `(甲)` 刀 4 前置② 的正式措辞**（与 `#2026-09-19-4` 的「显式排除 + 钉子写进触发条件」配套）。
...
- **附带（已闭环）**：该档旧断言 `'B010' in out or 'error' in out` 的**第一支从来没有内容**，长期靠**第二支**命中 **TF01**（`&x` 当时是 `PTR`、返回型声明 `&int` ⇒ 返回型不符）而绿——**是裁定二把它照出来的**，不是改判弄坏的；该断言已由 PR #153 重定为「钉真实成立的性质」（`&x` 与 `&int` 同 kind ⇒ 无 TF01/TA02），并在 docstring 内**逐字保留原断言**。
- **⑥ 同面第三形态（2026-09-20 收口时新增）**：**返回点在区域「内」的逃逸仍漏检**——`while i < 1 { arr := [7, 8]; return &arr[0]; }`（RETURN 节点在区内 ⇒ `ni < EXIT`）本判定形态**看不见**。**修前同样漏（rc=0），非本批引入**；本批的边界修复只覆盖「使用点在区域关闭之后」一格 ⇒ **不修**、并入本条目作为同一面的第 ⑥ 项（与 ①–⑤ 同面：都是 escape/lifetime 的漏检与零覆盖形态；拆条会让「这个面到底多烂」看不出来）。
- **状态（2026-09-20 收口）**：**①–⑤ 本批已修并落判据**；**⑥** 与下段三条**保留登记**。
  ...
  - **① 收口**：`EC_B_ESCAPE`（渲染为 `B10`）**现有 raise 点**——返回逃逸改报 B10（按 `docs/developer/errors.md:245`「Reference to local escapes the function」归位；修前误用 B11 = 码位错配）。
  - **② 收口**：B11 首次有语料之外的**活**判据——正钉 7 断言到**具体码 + rc**并配反向对照 7（必须 rc=0）；③ 的手造反例（循环体内取址后返回）现 rc=1。
  - **④ 收口**：`SG_FUNC` 单点特例被「归还区域」谓词（出口发 `IR_ARENA_RESET` 者）**一般化**——函数级分配自然放行，不再依赖一句特判。
  - **⑤ 收口**：判定值接回 `check_error`（B11）——该点从「连 raise 点都没有」变为可触发。
