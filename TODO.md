...
- **裁决记录**：取 **(乙) 明确登记「内建族参数面免检」这个契约**（**有契约 = 有意设计** ⇒ 按本仓口径**登记**，不是修；与「看起来要豁免 ⇒ 先查是不是某次迁移漏了尾」是**两步**：先查同形先例，本例**查不到漏尾**、是设计）。**否 (甲)**（给内建合成 `FuncInfo`/参数符号）的**主因** = `checker.cr:4276-4288` 会为每个未被遮蔽的内建建 `SYM_FN` ⇒ **符号编号变 ⇒ `.ccr` SYM 段字节面可能变 ⇒ canary 需重锁声明**（还要过 D1 闸门），**为 2 条付此代价不成立**。
