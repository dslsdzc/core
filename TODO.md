...
- **覆盖面第二盲区（2026-09-19 补跑实测）**：**整型字面量实参不入账**——`f(y)`（int 变量）**报 1 行**，**`f(7)`（int 字面量）不报**；对照三形参（`string` / `bool` / `*int`）**皆不报** ⇒ 与实参**形式**相关、与形参无关。另两形实测**入账**：`f("x")`（string 字面量）· `g(&x)`（取址表达式）。⇒ 口径精确化为：**直调实参 ∧ 非整型字面量 ∧ 非返回位**（三处盲区：返回位 · 整型字面量 · 内建回退面见 [[verify-command-traps]] 第 14 条）。
