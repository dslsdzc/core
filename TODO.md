...
### 73. (A) 批（表示位随存储走）**收官** + 四项边界登记（2026-09-16——独立批 T1–T5 全阶段收官）
- **批交付**：裁-REP-1 (iii)「取址槽恒装箱」实现（W1 取址预扫 · W2/W3 写点装箱+表示位钉 1 · W4 形参序言条件装箱 · W5 `IR_STORE_PTR` 按 pointee 装箱 · W6 零足迹门）· **第二根因** ELF `IR_REF` 全局取址修复（`src/arch/x86_64/instr.cr:1095`）· T3 写点完备性审计 + 裁-T3-1 修复（推断可选数组/切片元素写点，7 形态）· 用例 `tests/selfhost/test_optional.py` **80 例**（48→66→80）。
- **判据（两轮全绿；证据 `/tmp/capt6/crit/` 与 `/tmp/capt7/crit2/`）**：构建确定性 ×2 IDENTICAL · `selftest-types` 415/415 · 枚举 61/61 · 五 CI job 5/5 · **ELF canary `95084e7b…d475`（28822B）IDENTICAL** · `.ccr` 两口径四条命中锁定值 · 腿① 72 档零差异 · 探针 29 档冷态零差异 · 自举链 `093ba230…` + N06=0 + `--help` rc=1 + 冒烟 42 · 突变 3+3 条全红 + 精确回滚。
- **边界登记（本批不覆盖，逐条留痕）**：
  - **REP-2（指针算术伪造地址）**：`p ± n`（`ir_gen.cr:1208` `OP_PTR_ADD`），地址由运行期派生 ⇒ 不可静态分类；**登记 + 探针**（`a_rep2_ptradd_boundary` 记录现实行为：当前布局下不落相邻可选槽，双路径同 6）。
  - **REP-3（`&s.a` 写入丢失）**：`UOP_REF` 无字段分支 ⇒ 取**临时量**地址（`*p = 5` 静默无效）；标 0/0 双路径同；**须新增 `IR_ADDR_FIELD` 面**（后端/解释器同改）才能修 ⇒ 维持登记；用例 `a_field_addr_registered` 只钉「双路径同 rc，不静默分歧」。
  - **B04（`&x` 借出后再读 x）**：既有软诊断（`Cannot use 'x' while it is borrowed`，build 仍 rc=0；根因 = 无 NLL/liveness 收窄）——本批新增用例以 `case_dual_softdiag` 钉「**恰 {B04}**」，**未修**。
  - **`IR_REF` 目标侧**是否可能为全局：现 IR 形态下 dest 恒为临时量（**未实测断言** ⇒ 不写进结论）。
- **过程事故与纪律（本批特有，已入 (A) 计划文档）**：工作副本 stale（**他方推进 op log** ⇒ 别的 workspace stale）时 `jj workspace update-stale` 会**以 op log 重建工作目录**、**未快照文件不留痕**（2026-09-16 两次实证）⇒ 纪律：**stale 前先 `jj status` 快照，或先把未快照文件备份到仓库外**；产出即路径限定提交。
- **关联**：计划 `docs/superpowers/plans/2026-09-16-opt-rep-follows-storage.md` · 报告 `/tmp/capt6/task2-report.md`（T2）· `/tmp/capt7/task3-report.md`（T3）· 提交 `1e11353c`（#74）· `5d36af98`（#75）。
