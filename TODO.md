...
寄存器分配导致大程序 SIGSEGV。`load64` inline 在 0x46c818 崩（`mov rax,[rdi+rsi]` -- rdi/rsi 含无效地址）。
**可能根因：** 寄存器分配的变量从未从栈加载值；或被调用者保存寄存器在函数调用中被破坏。
**当前状态：** 简单程序 O1 正确，大程序崩。

### 2. corec2 前端性能（~1000x 慢于 build/corec）
10 行程序 build/corec 0.02s -> corec2 8.0s。ELF 后端全栈操作无寄存器分配是直接原因。
**瓶颈：** 寄存器分配只作用于用户 IR，不影响编译器自身代码生成。

### 3. AST 优化禁用
`ast_optimize_body` 常量折叠导致 `ir_gen_all` 崩溃。`EXPR_INT` 处理不匹配。

### 4. pass_stack_share（O2 崩溃）
`pass_stack_share()` 在 O2 下崩。需要修复或重写。

### 5. pass_cse 大 IC 崩溃
`ic > 50000` 时可能崩。当前是占位实现。
