...
fn res_labels() {
    ...
    loop {
        ...
        loop {
            if ii >= ic { break; }
            inst_idx := ist + ii;
            iop2 := iri_op(inst_idx);
            if iop2 == IR_BRANCH {
                d := iri_dest(inst_idx);
                s1 := iri_s1(inst_idx);
                true_off := r64(g_label_poses, iri_s2(inst_idx) * 8);
                false_off := r64(g_label_poses, iri_s3(inst_idx) * 8);
                iri_set_op(inst_idx, IR_BRANCH);
                iri_set_dest(inst_idx, d);
                iri_set_s1(inst_idx, s1);
                iri_set_s2(inst_idx, true_off);
                iri_set_s3(inst_idx, false_off);
                iri_set_tk(inst_idx, IR_RESOLVED);
            }
            if iop2 == IR_JUMP {
                d := iri_dest(inst_idx);
                s2 := iri_s2(inst_idx);
                ...
                iri_set_op(inst_idx, IR_JUMP);
                iri_set_dest(inst_idx, d);
                iri_set_s1(inst_idx, tgt_off);
                iri_set_s2(inst_idx, s2);
                iri_set_s3(inst_idx, s3);
                iri_set_tk(inst_idx, IR_RESOLVED);
            }
            ii = ii + 1;
        }
